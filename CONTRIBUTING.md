...
Local CI streak (`docs/ci-streak.json`, `#36401`): after those gates are green on the tip, record the day with `php script/status/ci-streak.php --record-green --sha=$(git rev-parse HEAD) --day=$(date -u +%F) --write`. Never invent a streak day without a gate receipt. `release-readiness --json` exposes `ci_green_streak_days` / `last_green_master_sha` from that file. Remote Actions required-checks stay deferred while billing keeps GHA off (`.cursor/rules/local-ci-only.mdc`); master still requires a PR (no direct push / force-push).
