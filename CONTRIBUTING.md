...
- `./script/check-size-budgets.sh` when growing `lib/Compiler.php` / `lib/JIT.php` / `lib/VM.php` — budgets ratchet down only (#36403); check mode is part of `./script/check-generated-docs.sh`
...
Local CI streak (`docs/ci-streak.json`, `#36401`): after those gates are green on the tip, record the day with `php script/status/ci-streak.php --record-green --sha=$(git rev-parse HEAD) --day=$(date -u +%F) --gates=apply-patches,bootstrap-inventory,aot-smoke,north-star5-verify-fast --write`. Never invent a streak day without a gate receipt. `release-readiness --json` exposes `ci_green_streak_days` / `last_green_master_sha` from that file. Remote Actions required-checks stay deferred while billing keeps GHA off (`.cursor/rules/local-ci-only.mdc`). Master is protected by the `master-local-ci-protection` ruleset (PR required, no force-push / deletion); do not add a GHA “Compiler gate” required check until billing re-enables Actions.
