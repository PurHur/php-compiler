...
| Self-host **M0–M1** | ✅ |
| Self-host **M2** spine | 🚧 **582** / 586 units ([#1492](https://github.com/PurHur/php-compiler/issues/1492)) |
...
| **M2 — Full top-level `lib/` + spine** | All **14** top-level `lib/*.php` lint ✅; **`compiler_lib_spine_smoke`** (**582** / **586** inventory units) native link ✅; grow toward full `bin/vm.php` path | 🚧 ~99% of inventory |
...
| Gate | Command | Status |
|------|---------|--------|
| Phase A inventory | `php script/bootstrap-inventory.php --check` | ✅ **586** files; **0** blockers |
