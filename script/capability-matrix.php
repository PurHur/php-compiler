...
function analyzeInternal(PHPCompiler\Func\Internal $fn): array
{
    ...
    if (str_starts_with($fn->getName(), 'finfo_') && preg_match('/VmFinfo|VmMime|JitFinfoFile|JitFinfoBuffer|FinfoFileRuntime|FinfoBufferRuntime|FinfoConstruct|ReflectionSetup/i', $source)) {
        if (preg_match('/JitFinfoFile|JitFinfoBuffer|FinfoFileRuntime|FinfoBufferRuntime|FinfoConstruct|ReflectionSetup/i', $source)) {
            $notes[] = 'ext/fileinfo MIME sniff via VmMime + FinfoFileRuntime/FinfoBufferRuntime AOT (#3366,#27196,#28660,#34688; FILEINFO_NONE/RAW still VM-rich)';
        } else {
