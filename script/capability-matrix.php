...
require __DIR__ . '/capability-matrix-lib.php';
...
function collectCapabilities(string $root): array
{
    ...
    $modules = capabilityMatrixModules();
    ...
    foreach ($modules as $moduleLabel => $module) {
        foreach ($module->getFunctions() as $fn) {
            ...
            $capabilities[$name] = [
                'vm' => true,
                'jit' => $analysis['jit'],
                'aot' => analyzeInternalAot($fn, $analysis['jit'], $analysis['notes']),
...
function runCapabilityMatrixCli(array $argv): int
{
    ...
    $classMethods = collectClassMethodCapabilities($root, buildJitClassMethodProxyIndex($root));
    $phpt = collectPhptCoverage($root);
    $markdown = renderCapabilityMatrixMarkdown($capabilities, $classMethods, $phpt);
    ...
    if ($check) {
        ...
        fwrite(STDOUT, 'docs/capabilities.md is up to date ('.count($capabilities).' functions, '
            .count($classMethods)." class methods).\n");
    ...
    fwrite(STDOUT, 'Wrote '.$outFile.' ('.count($capabilities).' functions, '
        .count($classMethods)." class methods).\n");
