...
apply_php_types_resolver_worklist_overlay() {
  ...
  if grep -q '$legacyResolver' "$vendor" 2>/dev/null \
    && { [[ ! -f "$prelinked" ]] || grep -q '$legacyResolver' "$prelinked" 2>/dev/null; }; then
    echo "Skip php-types-resolver-worklist.patch (already applied)"
    return 0
  fi
  python3 - "${targets[@]}" <<'PY'
...
WORKLIST_BODY = """            // Dependency-driven worklist: the round-based loop rescanned every
            // unresolved variable per round — O(rounds × vars), minutes on
            // 30k-line files (#16077).
            $dependents = new SplObjectStorage();
            foreach ($unresolved as $var) {
...
FULL_BLOCK = """        // Worklist is the default (#16077 / #36225): dependency-driven instead of
        // O(rounds × vars) rescans. Opcode dumps match legacy on a 10-file corpus.
        // Opt out with PHPTYPES_RESOLVER_WORKLIST=0 or PHPTYPES_RESOLVER_LEGACY=1.
        $worklistFlag = getenv('PHPTYPES_RESOLVER_WORKLIST');
        $legacyResolver = ('0' === $worklistFlag)
...
OLD_PARTIAL_IF = "if ('1' !== getenv('PHPTYPES_RESOLVER_WORKLIST')) {"
OLD_PARTIAL_COMMENT = """            // Dependency-driven worklist: the round-based loop rescanned every
            // unresolved variable per round — O(rounds × vars), minutes on
            // 30k-line files (#16077). Resolution ORDER differs from the round
            // loop and the fixpoint is order-sensitive for codegen, so this is
            // opt-in for lint workloads only (bin/lint.php sets the flag);
...
PRISTINE_PAT = re.compile(
    r"        \$round = 1;\n        do \{.*?        \} while \(count\(\$unresolved\) > 0 && \$start < count\(\$resolved\)\);",
    re.DOTALL,
)
...
for arg in sys.argv[1:]:
    path = Path(arg)
    text = path.read_text()
    if "$legacyResolver" in text:
        continue
    new_text = text
    if OLD_PARTIAL_IF in text:
        preamble = """        // Worklist is the default (#16077 / #36225): dependency-driven instead of
        // O(rounds × vars) rescans. Opcode dumps match legacy on a 10-file corpus.
        // Opt out with PHPTYPES_RESOLVER_WORKLIST=0 or PHPTYPES_RESOLVER_LEGACY=1.
        $worklistFlag = getenv('PHPTYPES_RESOLVER_WORKLIST');
        $legacyResolver = ('0' === $worklistFlag)
            ...
            || ('false' === strtolower((string) $worklistFlag));
        if ($legacyResolver) {"""
        new_text = new_text.replace("        " + OLD_PARTIAL_IF, preamble, 1)
    ...
    elif PRISTINE_PAT.search(text):
        new_text = PRISTINE_PAT.sub(FULL_BLOCK, text, count=1)
    else:
        sys.stderr.write(f"php-types-resolver-worklist: no anchor in {path}\n")
        raise SystemExit(1)
    if new_text == text:
        sys.stderr.write(f"php-types-resolver-worklist: flip did not apply in {path}\n")
        raise SystemExit(1)
    if "$legacyResolver" not in new_text:
        sys.stderr.write(f"php-types-resolver-worklist: missing legacyResolver in {path}\n")
        raise SystemExit(1)
