...
#   FILE_UPLOAD_WEB_DEPLOY_SMOKE_GATE=1 SESSIONS_WEB_DEPLOY_SMOKE_GATE=1 THROWSWEB_DEPLOY_SMOKE_GATE=1 FASTCGI_WEB_DEPLOY_SMOKE_GATE=1 make deploy-smoke-all
#
# Docker:
...
skip_009() {
  echo "deploy-smoke-all: 009-FastCGIWeb: skip (FASTCGI_WEB_DEPLOY_SMOKE_GATE=0 — set FASTCGI_WEB_DEPLOY_SMOKE_GATE=1 or make examples-fastcgiweb-deploy-smoke #2359)" >&2
}
