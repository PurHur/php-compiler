...
if [[ "${BOOTSTRAP_M3_LINK_COMPILE_DRIVER:-0}" == "1" ]]; then
  ...
  set +e
  "${m3_link_env[@]}" php bin/compile.php -o build/selfhost-compiler-unit-probe-emit "${m3_emit_source}" >/dev/null 2>&1
  m3_link_code=$?
  set -e
  if [[ -x "${EMIT_HELPER}" ]]; then
    ...
    if [[ "${BOOTSTRAP_M3_RUNTIME_COMPILE:-1}" == "1" ]]; then
      ...
      if [[ "${USE_INVENTORY_EMIT_DRIVER}" == "1" ]]; then
        m3_run_env+=(PHP_COMPILER_M3_INVENTORY_EMIT_DRIVER=1 BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1)
      fi
      compile_out="$(
...
if ! grep -q 'compiler_unit_probe bundle OK' <<< "${bundle_out}"; then
  echo "bootstrap-selfhost-compiler-unit-probe: unexpected bundle stdout (want compiler_unit_probe bundle OK)" >&2
  printf '%s\n' "${bundle_out}" >&2
  exit 1
fi
...
if [[ "${M3_NATIVE_COMPILE}" -eq 1 ]]; then
  ...
  echo "bootstrap-selfhost-compiler-unit-probe: OK emit_path=zend partial ${PROBE} -> ${AOT_OUT} (native run)"
fi
