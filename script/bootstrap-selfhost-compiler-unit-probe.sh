...
if [[ "${BOOTSTRAP_M3_LINK_COMPILE_DRIVER:-0}" == "1" ]]; then
  ...
  if [[ -x "${EMIT_HELPER}" ]]; then
    ...
    if [[ "${BOOTSTRAP_M3_RUNTIME_COMPILE:-1}" == "1" ]]; then
      ...
      native_compile_code=$?
      set -e
      if [[ "${native_compile_code}" -eq 0 ]] && grep -qE '(compile_smoke_m3_emit|compiler_unit_probe_m3_emit): compile OK' <<< "${compile_out}"; then
        M3_NATIVE_COMPILE=1
        M3_EMIT_PATH="native"
        M3_BLOCK_REASON=""
...
emit_requested=0
if [[ "${BOOTSTRAP_M3_LINK_COMPILE_DRIVER:-0}" == "1" || "${BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT:-0}" == "1" ]]; then
  emit_requested=1
fi

if [[ "${emit_requested}" -eq 1 ]]; then
  if [[ "${M3_NATIVE_COMPILE}" -eq 0 ]]; then
    ...
    M3_EMIT_PATH="zend"
    echo "bootstrap-selfhost-compiler-unit-probe: emit_path=zend (bin/compile.php) — compiler unit probe AOT until native emit TU is stable" >&2
    rm -f "${AOT_OUT}"
  ...
  run_out="$("${AOT_OUT}" 2>&1)"
  if ! grep -q 'compiler_unit_probe fixture OK' <<< "${run_out}"; then
    echo "bootstrap-selfhost-compiler-unit-probe: unexpected AOT stdout (want compiler_unit_probe fixture OK, emit_path=${M3_EMIT_PATH})" >&2
    printf '%s\n' "${run_out}" >&2
    exit 1
  fi
  ...
  if [[ "${M3_NATIVE_COMPILE}" -eq 1 ]]; then
    ...
    echo "bootstrap-selfhost-compiler-unit-probe: OK emit_path=zend partial ${OUT} -> ${AOT_OUT} (native run)"
  fi
