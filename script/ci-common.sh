...
ci_run_stdlib_jit_deferred_sync_check() {
  if [[ "${STDLIB_JIT_DEFERRED_SYNC_GATE:-1}" != "1" ]]; then
    return 0
  fi
...
# 003-MiniWebApp VM OOP lint + phpc serve curls (issues #2059, #2189, #2293); default MINIWEBAPP_VM_OOP_GATE=1.
ci_run_miniwebapp_vm_oop() {
  if [[ "${MINIWEBAPP_VM_OOP_GATE:-1}" != "1" ]]; then
    echo "MiniWebApp VM OOP: skipped (MINIWEBAPP_VM_OOP_GATE=0; default 1 in ci-local, issue #2293)"
    return 0
  fi
  if [[ -n "${PHP_COMPILER_SKIP_SERVE_TESTS:-}" ]]; then
    echo "MiniWebApp VM OOP: skipped (PHP_COMPILER_SKIP_SERVE_TESTS set; unset to run VM serve OOP curls, #2293)"
    return 0
  fi
  ...
  echo "MiniWebApp VM OOP (MINIWEBAPP_VM_OOP_GATE=1 default, issues #2189, #2293)..."
...
# M3 compiler unit probe: lib/Compiler.php bundle native link + run (issue #2216); default-on (#2221 follow-up).
ci_run_bootstrap_compiler_unit_probe() {
  if [[ "${BOOTSTRAP_COMPILER_UNIT_PROBE_GATE:-1}" != "1" ]]; then
    return 0
  fi
  if ! ci_llvm_ready; then
    echo "bootstrap-selfhost-compiler-unit-probe: skipped (LLVM 9 not available)"
    return 0
  fi
  echo "bootstrap-selfhost-compiler-unit-probe (BOOTSTRAP_COMPILER_UNIT_PROBE_GATE=1, issue #2216)..."
  "$_CI_SCRIPT_DIR/bootstrap-selfhost-compiler-unit-probe.sh"
}
...
# M5 fast presenter in ci-fast (issue #1492); default on — opt-out with NORTH_STAR5_VERIFY_FAST_GATE=0.
# PRs already get inventory + spine + VM driver probes via individual gates; this is the consolidated M5 presenter.
ci_run_north_star5_verify_fast() {
  if [[ "${NORTH_STAR5_VERIFY_FAST_GATE:-1}" != "1" ]]; then
    echo "north-star5-verify: skipped (NORTH_STAR5_VERIFY_FAST_GATE=0 opt-out; issue #1492)"
    return 0
  fi
  ...
  echo "north-star5-verify --fast (NORTH_STAR5_VERIFY_FAST_GATE=1, issue #1492)..."
  "$ns5_script" --fast
}
...
# PHPUnit FastCGI record + TCP adapter (issue #173; opt-in FASTCGI_SMOKE_GATE=1, #1899).
ci_run_fastcgi_smoke() {
  if [[ "${FASTCGI_SMOKE_GATE:-0}" != "1" ]]; then
    return 0
  fi
  if [[ -n "${PHP_COMPILER_SKIP_SERVE_TESTS:-}" ]]; then
    echo "PHPUnit FastCGI (FastCgi*): skipped (PHP_COMPILER_SKIP_SERVE_TESTS is set)"
    return 0
  fi
  if ! ci_can_bind_loopback; then
    echo "PHPUnit FastCGI (FastCgi*): skipped (cannot bind loopback TCP)"
    return 0
  fi
