...
ci_docker_run() {
  docker run --rm \
    ...
    -e "REBUILD_EXAMPLES_005_SYNC_GATE=${REBUILD_EXAMPLES_005_SYNC_GATE:-0}" \
    ...
    -e "COALESCE_COMPLIANCE_GATE=${COALESCE_COMPLIANCE_GATE:-0}" \
    -e "STRING_KEY_JIT_COMPLIANCE_GATE=${STRING_KEY_JIT_COMPLIANCE_GATE:-1}" \
    "$@"
}
