...
final class Doctor
{
    ...
    private static function printFastCGIWebSection(string $repoRoot): void
    {
        ...
        $deployStatus = $deployOn && $llvmReady ? '✅' : '📋';
        $deployNote = $llvmReady
            ? ($deployOn ? '#2359 · FASTCGI_WEB_DEPLOY_SMOKE_GATE=1' : '#2359 · opt-in default 0')
            : 'LLVM required; #2359 when gate=1';
        ...
        fwrite(STDOUT, "      Or:      FASTCGI_WEB_DEPLOY_SMOKE_GATE=1 ./script/deploy-smoke.sh --example 009\n");
        fwrite(STDOUT, "      Ladder:  make deploy-smoke-all (skips 009 with hint when gate=0; #2359)\n");
        fwrite(STDOUT, "\n  Related:\n");
