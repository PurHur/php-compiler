...
final class Doctor
{
    ...
    private static function printSessionsWebSection(string $repoRoot): void
    {
        ...
        if ($linkOn && $llvmReady) {
            fwrite(STDOUT, "  [✅] SESSIONS_WEB_AOT_LINK_GATE=1 — phpc build --project link (#1946)\n");
        } else {
            $linkNote = $linkOn && !$llvmReady
                ? "{$llvmDetail}; export SESSIONS_WEB_AOT_LINK_GATE=1 when LLVM ready"
                : 'opt-in SESSIONS_WEB_AOT_LINK_GATE=0 until #1954';
            fwrite(STDOUT, "  [📋] AOT link — ExamplesCompileTest::test005SessionsWebAotLink (#1946; {$linkNote})\n");
        }
        ...
        fwrite(STDOUT, "    SESSIONS_WEB_AOT_LINK_GATE   default 0 until #1954 (#1946)\n");
