...
/**
 * Residual valueSlot resolvers after encapsed/concat/arithmetic early slots
 * (#36387 / #36403): direct dim fetch, coalesce, inline New_, hoisted empty,
 * compile-time fold, runtime enum prefetch, preceding-inline producer match,
 * outer sibling / adjacent nested / adjacent assign.
 *
 * Extracted from {@see CompileCallArgSends} so gen-0 split-TU can hollow a smaller
 * Concern TU. Mutates `$valueSlot` / `$sends` by-ref when `!$tookDimOrInlineArrayBranch`.
 ...
 */
trait CallArgResidualDimCoalesceNewHoistedFoldAndAdjacentProducerValueSlots
{
    /**
     * @param list<mixed> $inlineProducerCfgChildren
     * @param list<\PHPCompiler\OpCode> $sends already-emitted ARG_SENDs (compileExpr may append)
     * @param-out mixed $valueSlot
     */
    private function resolveCallArgResidualDimCoalesceNewHoistedFoldAndAdjacentProducerValueSlots(
        mixed $arg,
        int $argIndex,
        Block $block,
        ?string $calleeName,
