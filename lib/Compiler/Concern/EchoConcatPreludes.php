...
/**
 * Echo/concat prelude helpers (#36387 / #36403).
 *
 * Extracted from {@see CoalesceLeftAndEchoConcatPreludes} so gen-0 split-TU can
 * hollow a smaller Concern TU. Coalesce-left skip detectors stay in the parent;
 * this trait owns binary-concat → ConcatList materialization and
 * "lowered by following echo/concat" prelude detection used from compileOps.
 *
 * Companion to {@see EchoCoalesceCallArgCompile} (echo/?? → FuncCall arg wiring).
 * php-src: Zend/zend_compile.c (zend_compile_expr concat / echo).
 *
 * Note: no declare(strict_types=1) — parent Compiler.php is weak-types; call-arg
 * and coalesce slot wiring relies on coercion (same as EchoCoalesceCallArgCompile).
 */
trait EchoConcatPreludes
{
    /**
