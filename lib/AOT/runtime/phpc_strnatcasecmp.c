/*
 * strnatcasecmp() runtime for VM/JIT/AOT (issue #2372).
 * Byte-oriented natural order, ASCII case-insensitive; no PHP internal wrappers.
 */
...
int strnatcasecmp(const char *a, const char *b)
{
    ...
    while (*pa != '\0' && *pb != '\0') {
        ...
        unsigned char ca = to_lower(*pa);
        unsigned char cb = to_lower(*pb);
        if (ca != cb) {
            return ca < cb ? -1 : 1;
        }
        ++pa;
