 ...
 */
require_once __DIR__.'/LinkerOutputArtifacts.php';
...
final class Linker
{
    ...
    public static function link(string $objectFile, string $executable): void
    {
        ...
        if (
            ...
            $cmd = implode(' ', [
                ...
                CompileTarget::linkBuildIdFlag(false),
        ...
        if (is_executable($clang)) {
            ...
            $cmd = escapeshellarg($clang).' '.AotDebugSymbols::linkFlag().self::sanitizerLinkFlags().AotGcSections::linkStripFlag().AotGcSections::linkGcSectionsFlagForHelperLink(true, $helperGcLinkPaths).self::helperMuldefsFlag(' -Wl,-z,muldefs').CompileTarget::linkBuildIdFlag(true).self::libcNameHideFlag(true).$objects.' '.self::hostLibSearchFlag().' -lm '.self::runtimeLinkLibs($linkObjectFiles).' -o '.escapeshellarg($executable);
            self::run($cmd, $env);
            self::unlinkIfTemp($runtimeObjects);
    ...
    private static function toolchainEnvironment(string $llvmDir): array
    {
        ...
        return CompileTarget::applySourceDateEpochToEnv($env) ?? $env;
    }
    ...
    private static function runCaptured(string $command, ?array $env): array
    {
        ...
        if (null === $env) {
            ...
            $env = CompileTarget::applySourceDateEpochToEnv($env) ?? $env;
        }
    ...
    /** @see LinkerOutputArtifacts::prelinkedVendorObjectPaths() */
    public static function prelinkedVendorObjectPaths(string $projectRoot): array
    {
        return LinkerOutputArtifacts::prelinkedVendorObjectPaths($projectRoot);
    }

    /** @see LinkerOutputArtifacts::resolveEffectiveOutputPath() */
    public static function resolveEffectiveOutputPath(string $requestedOut): string
    {
        return LinkerOutputArtifacts::resolveEffectiveOutputPath($requestedOut);
    }

    /** Inventory argv emit must materialize a non-empty regular file (#8709). */
    public static function assertNonEmptyOutputFile(string $path): void
    {
        LinkerOutputArtifacts::assertNonEmptyOutputFile($path);
    }
    ...
    {
        LinkerOutputArtifacts::assertNonEmptyRequestedOutput($requestedOut);
    }
