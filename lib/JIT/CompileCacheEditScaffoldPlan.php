 ...
 *
 * Extracted from {@see CompileCacheEditScaffold} so which user LLVM symbols to
 * strip vs keep (changed-member / per-function attribution) stays a separate TU
 * from the LLVM mutation helpers in {@see CompileCacheEditScaffoldStrip}.
 *
 * Move-only — no new C ABI. php-src analogy: Zend opcache / file-cache deciding
 * which units are invalidated before rewriting the cached image
