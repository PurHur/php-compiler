...
/**
 * Pure LLVM array_udiff()/array_uintersect() for thin standalone AOT (#27533 / #26976).
 *
 * NestedJIT of {@see \PHPCompiler\ext\standard\ArrayUserSetOpsJitHelper} value filters aborts
 * under thin AOT (cross-HT NestedClosureInvoke / duplicate+reorder — handoff on #26976).
 * Walk exported key/value pairs and compare **values** with spaceship-equivalent strcmp
 * (strings) / int64 icmp (otherwise), mirroring {@see ArrayUserSetOpsKeyLlvm}.
 *
 * php-src: ext/standard/array.c — php_array_udiff / php_array_uintersect
 */
final class ArrayUserSetOpsValueLlvm
{
    private static int $seq = 0;

    public static function filterByValue(
        Context $context,
        bool $intersect,
        Value $firstHt,
        Value $othersPackedHt
    ): Value {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'array_uvalue_llvm_cont');
        $tag = (string) (++self::$seq);
        $prefix = ($intersect ? 'uintersect' : 'udiff').'_'.$tag;
        ...
        $present = self::valuePresentInOthers(
            $context,
            $valBox,
            $othersPackedHt,
            $otherCount,
    ...
    /** @return Value int1 — true if value is in all others ($requireAll) or any other (!$requireAll) */
    private static function valuePresentInOthers(
        Context $context,
        Variable $needleVal,
        Value $othersPackedHt,
        Value $otherCount,
    ...
    /** @return Value int1 */
    private static function valueInPairList(
        Context $context,
        Variable $needleVal,
        Value $pairs,
        Value $pairCount,
        ...
        $head = BasicBlockHelper::append($context, $prefix.'_val_head');
        $body = BasicBlockHelper::append($context, $prefix.'_val_body');
        $done = BasicBlockHelper::append($context, $prefix.'_val_done');
        $context->builder->branch($head);
        ...
        $cmp = ArrayUserSetOpsKeyLlvm::compareValueBoxesPublic($context, $needleVal, $otherVal);
        $eq = $context->builder->icmp(Builder::INT_EQ, $cmp, $i64->constInt(0, false));
        $hit = BasicBlockHelper::append($context, $prefix.'_val_hit');
        $miss = BasicBlockHelper::append($context, $prefix.'_val_miss');
