...
/**
 * Function-proxy resolve / NestedJIT kernel whitelist / external-stub
 * report for {@see Context} (#36387).
 *
 * Extracted from {@see Context} so resolveFunctionProxy, spine-chunk /
 * NestedJIT builtin lookup, and ExternalMethod stub reporting stay a
 * separate TU from the Context construction / compile hub (split-TU /
 * size-budget ratchet toward Context ≤ 4k lines, #36199 / #36403).
 *
 * Used via {@code use ContextFunctionProxyResolve;} on {@see Context}.
 *
 * No new C ABI. php-src analogy: zend_is_callable / zend_fetch_function /
 * zend_hash_find on EG(function_table) live beside the executor
 * (Zend/zend_execute_API.c, Zend/zend_API.c) rather than inside the
 * compiler front-end.
 */
trait ContextFunctionProxyResolve
{
