...
use PHPCompiler\JIT\Builtin\ReflectionClassGetMethodsRuntime;
 ...
 * ReflectionMethod list via {@see ReflectionClassGetMethodsRuntime} dispatched
...
final class ReflectionClassGetMethods implements Call
{
    ...
    public function call(Context $context, Variable ...$args): Value
    {
        ...
        if ($userArgCount < 0 || $userArgCount > 1) {
            ExceptionBridge::emitArgumentCountErrorAndAbort(
                $context,
                'ReflectionClass::getMethods() expects at most 1 argument, '.$userArgCount.' given'
            );
        ...
        if (1 === $userArgCount) {
            ...
            // ReflectionMethod::IS_* paths set compileTimeLong.
        }
    ...
    private function dispatchByClassId(Context $context, Value $classId, int $filter): Value
    {
        ...
        if ([] === $ids) {
            return ReflectionClassGetMethodsRuntime::emitEmpty($context);
        }
        ...
        foreach ($ids as $i => $id) {
            ...
            $raw = ReflectionClassGetMethodsRuntime::emitForClassId($context, $id, $filter);
        ...
        $empty = ReflectionClassGetMethodsRuntime::emitEmpty($context);
        $context->builder->store(
