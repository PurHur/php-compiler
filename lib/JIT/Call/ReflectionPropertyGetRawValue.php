...
use PHPCompiler\JIT\Builtin\ReflectionPropertyRawValueRuntime;
...
/** ReflectionProperty::getRawValue($object) — JIT/AOT (#27598, #6451, php_reflection.c). */
final class ReflectionPropertyGetRawValue implements Call
{
    public function call(Context $context, Variable ...$args): Value
    {
        $rp = ReflectionSetup::loadObjectFromArg($context, $args[0]);
        $classStr = self::readStoredStringBox($context, $rp, ReflectionSupport::PROP_DECLARING_CLASS_NAME);
        $propStr = self::readStoredStringBox($context, $rp, ReflectionSupport::PROP_PROPERTY_NAME);
        $target = ReflectionSetup::loadObjectFromArg($context, $args[1]);
        $resultSlot = JitValueBox::alloc($context);
        ReflectionPropertyRawValueRuntime::invokeGet($context, $target, $classStr, $propStr, $resultSlot);
    ...
    private static function readStoredStringBox(Context $context, Value $obj, string $propName): Value
    {
        ...
        $readBlock = $fn->appendBasicBlock('refl_prop_raw_get_str_read');
        $emptyBlock = $fn->appendBasicBlock('refl_prop_raw_get_str_empty');
        $doneBlock = $fn->appendBasicBlock('refl_prop_raw_get_str_done');
