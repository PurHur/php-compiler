...
use PHPCompiler\JIT\Builtin\ReflectionClassGetStaticPropertiesRuntime;
 ...
 * name=>value map via {@see ReflectionClassGetStaticPropertiesRuntime} dispatched
 * by ReflectionClass name → class_id (peer getDefaultProperties #34091).
 *
...
final class ReflectionClassGetStaticProperties implements Call
{
    ...
    private function dispatchByClassId(Context $context, Value $classId): Value
    {
        ...
        if ([] === $ids) {
            return ReflectionClassGetStaticPropertiesRuntime::emitEmpty($context);
        }
        ...
        foreach ($ids as $i => $id) {
            ...
            $raw = ReflectionClassGetStaticPropertiesRuntime::emitForClassId($context, $id);
        ...
        $empty = ReflectionClassGetStaticPropertiesRuntime::emitEmpty($context);
        $context->builder->store(
