...
/**
 * Pure LLVM for HashTable internal-pointer ops (#4967 / #5504 / #27484).
 *
 * Call-site emission for thin AOT (peer {@see HashTablePopLastLlvm} / {@see ArrayPopRuntime}).
 * NestedJIT of {@see \PHPCompiler\ext\standard\ArrayPointerJitHelper} left
 * HashTable::pointer* unresolved under standalone AOT.
 *
 * php-src: ext/standard/array.c — php_array_key / current / next / prev / reset / end
 */
final class HashTablePointerLlvm
{
    ...
    public static function key(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_key_cont');
    ...
    public static function current(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_cur_cont');
        $result = JitValueBox::alloc($context);
        $resultPtr = JitValueBox::pointer($context, $result);
    ...
    public static function next(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_next_cont');
        $result = JitValueBox::alloc($context);
        $resultPtr = JitValueBox::pointer($context, $result);
    ...
    public static function prev(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_prev_cont');
        $result = JitValueBox::alloc($context);
        $resultPtr = JitValueBox::pointer($context, $result);
    ...
    public static function reset(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_reset_cont');
        $result = JitValueBox::alloc($context);
        $resultPtr = JitValueBox::pointer($context, $result);
    ...
    public static function end(Context $context, Value $ht): Value
    {
        BasicBlockHelper::ensureOpenInsertBlock($context, 'ht_ptr_end_cont');
        $result = JitValueBox::alloc($context);
        $resultPtr = JitValueBox::pointer($context, $result);
    ...
    {
        // Match Zend bool false via type+payload stores (peer JitValueBox::writeBool).
        $map = $context->structFieldMap['__value__'];
        $i8 = $context->getTypeFromString('int8');
        $i32 = $context->getTypeFromString('int32');
        $i64 = $context->getTypeFromString('int64');
        $context->builder->store(
            $i8->constInt(\PHPCompiler\JIT\Variable::TYPE_NATIVE_BOOL, false),
            $context->builder->structGep($resultPtr, $map['type'])
        );
        $valueField = $context->builder->structGep($resultPtr, $map['value']);
