...
/** Link phpc_gc.c into MCJIT modules (#3160). */
final class GcCollectCyclesRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_gc.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read gc JIT bitcode: '.$bitcode);
        }
        ...
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link gc JIT runtime');
        }
    ...
    private static function ensureBitcode(string $sourcePath): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('GC runtime source not found: '.$sourcePath);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile gc JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for gc JIT runtime bitcode');
    }
