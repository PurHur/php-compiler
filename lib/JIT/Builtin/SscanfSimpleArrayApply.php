 ...
 * Array ABI builds a packed hashtable; assign ABI writes by-ref `__value__*` outs.
 *
 * ABI: `phpc_sscanf_simple_array(__string__* line, __string__* specs) -> __hashtable__*`
 * ABI: `phpc_sscanf_simple_assign(__string__* line, __string__* specs, int64 n, __value__** outs) -> int64`
...
final class SscanfSimpleArrayApply
{
    ...
    /**
     * @param list<'d'|'s'> $specs
     * @param list<\PHPCompiler\JIT\Variable> $outArgs
     */
    public static function invokeAssign(Context $context, Value $line, array $specs, array $outArgs): Value
    {
        self::ensureAssignLinked($context);
        ...
        for ($i = 0; $i < $outCount; ++$i) {
            ...
            $context->builder->store(
                \PHPCompiler\JIT\JitValueBox::valuePtrFromVariable($context, $outArgs[$i]),
                $slot
            );
        }
        $specsStr = $context->builder->load(
    ...
    private static function ensureDecls(Context $context): void
    {
        ...
        // size_t may be needed for GEP; keep declared via LibcExtern paths.
        unset($sizeT);
    }
    ...
    private static function emitAssignBody(Context $context, ?LlvmFunction $existing): void
    {
        ...
        $entry = $fn->appendBasicBlock('ssas_entry');
        $fail = $fn->appendBasicBlock('ssas_fail');
        $ready = $fn->appendBasicBlock('ssas_ready');
        $loop = $fn->appendBasicBlock('ssas_loop');
        $body = $fn->appendBasicBlock('ssas_body');
        $skipWs = $fn->appendBasicBlock('ssas_skip_ws');
        $afterWs = $fn->appendBasicBlock('ssas_after_ws');
        $advWs = $fn->appendBasicBlock('ssas_adv_ws');
        $dispatch = $fn->appendBasicBlock('ssas_dispatch');
        $doD = $fn->appendBasicBlock('ssas_do_d');
        $storeD = $fn->appendBasicBlock('ssas_store_d');
        $doS = $fn->appendBasicBlock('ssas_do_s');
        $tokLoop = $fn->appendBasicBlock('ssas_tok_loop');
        $tokAdv = $fn->appendBasicBlock('ssas_tok_adv');
        $tokDone = $fn->appendBasicBlock('ssas_tok_done');
        $storeS = $fn->appendBasicBlock('ssas_store_s');
        $next = $fn->appendBasicBlock('ssas_next');
        $done = $fn->appendBasicBlock('ssas_done');
        ...
        $context->builder->branchIf(
            $context->builder->or($context->builder->or($nullLine, $nullSpecs), $badCount),
            $fail,
            $ready
        );
        ...
        $context->builder->call($context->lookupFunction('__value__writeString'), $outVarPtrS, $tokStr);
