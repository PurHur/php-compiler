...
/**
 * JIT/AOT link for ReflectionClass::isFinal() (#34043, #18297, #28135).
 *
 * Thin AOT previously used {@see StringCaseCompare} which always "matched"
 * under thin AOT, so every class reported isFinal()===true (#34043). Peer of
 * #34027 isInstantiable: ASCII-fold + length-checked {@see memcmp} against a
 * compile-unit + builtin ZEND_ACC_FINAL name table.
 *
 * php-src: ext/reflection/php_reflection.c zim_ReflectionClass_isFinal
 * (ce->ce_flags & ZEND_ACC_FINAL; enums are final — #26531).
 */
final class ReflectionClassIsFinalRuntime
{
    ...
    public static function ensureLinked(Context $context): void
    {
        ...
        // Over-long names are not in the table → not final.
    ...
    /**
     * Lowercase class names that must report isFinal()===true.
     *
     * @return list<string>
     */
    private static function finalLowerNames(Context $context): array
    {
        ...
        foreach (self::knownBuiltinFinalClassNames() as $builtin) {
            $add($builtin);
        }
     ...
     * @return list<string>
     */
    private static function knownBuiltinFinalClassNames(): array
    {
        $names = [
            'AddressInfo',
