...
/**
 * JIT MCJIT body for __compiler_memory_get_usage / __compiler_memory_get_peak_usage.
 *
 * Links {@see lib/AOT/runtime/phpc_memory.c}.
 */
final class StringMemory
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_memory.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_memory_get_usage');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read memory JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_memory.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link memory JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (['__compiler_memory_get_usage', '__compiler_memory_get_peak_usage'] as $symbol) {
            ...
            $context->registerFunction($symbol, $fn);
        }
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('memory runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile memory JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for memory JIT runtime bitcode');
    }
