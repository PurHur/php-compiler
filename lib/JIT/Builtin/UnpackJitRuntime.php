...
/**
 * JIT/AOT link for __compiler_unpack (issue #5442).
 *
 * Bitcode from {@see unpack_jit_runtime.c} until fully lowered from {@see UnpackEngine}.
 */
final class UnpackJitRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/unpack_jit_runtime.c';
    ...
    public static function ensureLinked(Context $context): void
    {
        ...
        $existing = $context->module->getNamedFunction('__compiler_unpack');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read unpack JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'unpack_jit_runtime.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link unpack JIT runtime bitcode');
        }

        $fn = $context->module->getNamedFunction('__compiler_unpack');
        if (null === $fn) {
            throw new \LogicException('__compiler_unpack missing after unpack bitcode link');
        }
        $context->registerFunction('__compiler_unpack', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('unpack runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile unpack JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for unpack JIT runtime bitcode');
    }

    private static function hostLibcIncludeFlags(): string
    {
        $flags = '';
        $dirs = self::discoverSystemIncludeDirs();
        foreach ($dirs as $dir) {
