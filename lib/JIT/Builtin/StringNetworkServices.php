...
/**
 * JIT MCJIT body for __compiler_getprotobynumber / __compiler_getservbyport.
 *
 * Links {@see lib/AOT/runtime/phpc_network_services.c}.
 */
final class StringNetworkServices
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_network_services.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read network services JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_network_services.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link network services JIT runtime bitcode');
        }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('network services runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile network services JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for network services JIT runtime bitcode');
    }
