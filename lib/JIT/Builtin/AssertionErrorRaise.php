...
/**
 * Pending AssertionError for assert() JIT/AOT path (issue #3316).
 */
final class AssertionErrorRaise
{
    ...
    private static function registerPendingGlobals(Context $context): void
    {
        ...
        if (null === $context->module->getNamedGlobal('phpc_jit_assertion_error_pending_flag')) {
            $flag = $context->module->addGlobal($i8, 'phpc_jit_assertion_error_pending_flag');
            $flag->setInitializer($i8->constInt(0, false));
        }
        if (null === $context->module->getNamedGlobal('phpc_jit_assertion_error_pending_msg')) {
            $msgGlobal = $context->module->addGlobal($msgTy, 'phpc_jit_assertion_error_pending_msg');
    ...
    {
        $fn = $context->lookupFunction('__compiler_jit_raise_assertion_error');
        ...
        $msgGlobal = $context->module->getNamedGlobal('phpc_jit_assertion_error_pending_msg');
        $flagGlobal = $context->module->getNamedGlobal('phpc_jit_assertion_error_pending_flag');
        $msgPtr = $context->builder->pointerCast($msgGlobal, $i8p);
        $max = $context->constantFromInteger(511, 'size_t');
        $lenOk = $fn->appendBasicBlock('len_ok');
        $lenClamp = $fn->appendBasicBlock('len_clamp');
        $done = $fn->appendBasicBlock('raise_done');
    ...
    private static function implementPendingHelpers(Context $context): void
    {
        ...
        if (null === $context->module->getNamedFunction('phpc_jit_assertion_error_clear_pending')
            || 0 === $context->module->getNamedFunction('phpc_jit_assertion_error_clear_pending')->countBasicBlocks()
        ) {
            $clear = $context->lookupFunction('phpc_jit_assertion_error_clear_pending');
            $block = $clear->appendBasicBlock('entry');
            $context->builder->positionAtEnd($block);
            $flag = $context->module->getNamedGlobal('phpc_jit_assertion_error_pending_flag');
            $context->builder->store(
                $i8->constInt(0, false),
        ...
        if (null === $context->module->getNamedFunction('phpc_jit_assertion_error_has_pending')
            || 0 === $context->module->getNamedFunction('phpc_jit_assertion_error_has_pending')->countBasicBlocks()
        ) {
            $has = $context->lookupFunction('phpc_jit_assertion_error_has_pending');
        ...
        if (null === $context->module->getNamedFunction('phpc_jit_assertion_error_copy_pending')
            || 0 === $context->module->getNamedFunction('phpc_jit_assertion_error_copy_pending')->countBasicBlocks()
        ) {
            $copyFn = $context->lookupFunction('phpc_jit_assertion_error_copy_pending');
    ...
    public static function registerDeclarations(Context $context): void
    {
        ...
        $decls = [
            '__compiler_jit_raise_assertion_error' => [$void, false, [$i8p, $sizeT]],
            'phpc_jit_assertion_error_clear_pending' => [$void, false, []],
            'phpc_jit_assertion_error_has_pending' => [$i32, false, []],
            'phpc_jit_assertion_error_copy_pending' => [$void, false, [$i8p, $sizeT]],
        ];
    ...
    {
        self::$hasPendingAddress = $engine->getFunctionAddress('phpc_jit_assertion_error_has_pending');
        self::$copyPendingAddress = $engine->getFunctionAddress('phpc_jit_assertion_error_copy_pending');
        self::$clearPendingAddress = $engine->getFunctionAddress('phpc_jit_assertion_error_clear_pending');
    }
