...
/**
 * JIT MCJIT body for substr_compare (haystack slice vs needle).
 *
 * Links {@see lib/AOT/runtime/phpc_substr_compare.c}.
 */
final class StringSubstrCompare
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_substr_compare.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('substr_compare');
            if (null !== $fn) {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read substr_compare JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_substr_compare.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link substr_compare JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('substr_compare');
        if (null === $fn) {
            throw new \LogicException('substr_compare missing after substr_compare bitcode link');
        }
        $context->registerFunction('substr_compare', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('substr_compare runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile substr_compare JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for substr_compare JIT runtime bitcode');
    }
