...
/**
 * JIT/AOT link for __compiler_hex2bin via Hex2binJitHelper PHP (#14627).
 *
...
final class StringHex2bin
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after Hex2binJitHelper compile (#14627)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'Hex2binJitHelper.php');
            if (null === $block) {
                throw new \LogicException('Hex2binJitHelper.php parseAndCompile failed (#14627)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            if (!isset($context->functions[\strtolower($logical)])) {
                throw new \LogicException($logical.' was not compiled for JIT (#14627)');
            }
        }
    }
