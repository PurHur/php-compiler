...
/**
 * JIT MCJIT body for strnatcasecmp (case-insensitive natural-order compare).
 *
 * Links {@see lib/AOT/runtime/phpc_strnatcasecmp.c}.
 */
final class StringStrnatcasecmp
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_strnatcasecmp.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('strnatcasecmp');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read strnatcasecmp JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_strnatcasecmp.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link strnatcasecmp JIT runtime bitcode');
        }
    ...
    {
        $fn = $context->module->getNamedFunction('strnatcasecmp');
        if (null === $fn) {
            throw new \LogicException('strnatcasecmp missing after strnatcasecmp bitcode link');
        }
        $context->registerFunction('strnatcasecmp', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('strnatcasecmp runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile strnatcasecmp JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for strnatcasecmp JIT runtime bitcode');
    }
