...
/**
 * JIT MCJIT body for str_getcsv (CSV line parse).
 *
 * Links {@see lib/AOT/runtime/phpc_str_getcsv.c}.
 */
final class StringStreamCsv
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_str_getcsv.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read str_getcsv JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_str_getcsv.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link str_getcsv JIT runtime bitcode');
        }
    ...
    {
        $fn = $context->module->getNamedFunction('__compiler_str_getcsv');
        if (null === $fn) {
            throw new \LogicException('__compiler_str_getcsv missing after str_getcsv bitcode link');
        }
        $context->registerFunction('__compiler_str_getcsv', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('str_getcsv runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile str_getcsv JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for str_getcsv JIT runtime bitcode');
    }
