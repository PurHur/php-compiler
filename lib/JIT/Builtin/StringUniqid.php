...
/**
 * JIT MCJIT body for __compiler_uniqid.
 *
 * Links {@see lib/AOT/runtime/phpc_uniqid.c}.
 */
final class StringUniqid
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_uniqid.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_uniqid');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read uniqid JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_uniqid.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link uniqid JIT runtime bitcode');
        }
    ...
    {
        $fn = $context->module->getNamedFunction('__compiler_uniqid');
        if (null === $fn) {
            throw new \LogicException('__compiler_uniqid missing after uniqid bitcode link');
        }
        $context->registerFunction('__compiler_uniqid', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('uniqid runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile uniqid JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for uniqid JIT runtime bitcode');
    }
