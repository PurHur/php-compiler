...
/**
 * JIT/AOT link for array_intersect() via ArrayIntersectJitHelper PHP (#12529).
 *
 * Standalone AOT keeps LLVM in {@see ArrayBuiltinHelper::arrayIntersect()}.
...
final class ArrayIntersectRuntime
{
    ...
    public static function intersect(Context $context, JITVariable $first, JITVariable ...$others): Value
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            return ArrayBuiltinHelper::arrayIntersect($context, $first, ...$others);
        }
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after ArrayIntersectJitHelper compile (#12529)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'ArrayIntersectJitHelper.php');
            if (null === $block) {
                throw new \LogicException('ArrayIntersectJitHelper.php parseAndCompile failed (#12529)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT (#12529)');
            }
        }
    }
