...
/**
 * JIT/AOT link for gz stream ABI via GzStreamJitHelper PHP (#13420).
 *
...
final class GzStreamRuntime
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after GzStreamJitHelper compile (#13420)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'GzStreamJitHelper.php');
            if (null === $block) {
                throw new \LogicException('GzStreamJitHelper.php parseAndCompile failed (#13420)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT gz stream (#13420)');
            }
        }
    }
