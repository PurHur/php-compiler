...
final class ChownRuntime
{
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'ChownJitHelper.php');
            if (null === $block) {
                throw new \LogicException('ChownJitHelper.php parseAndCompile failed (#9585)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT chown/chgrp (#9585)');
            }
        }
    }
