...
/**
 * JIT/AOT link for stream read ABI via StreamReadJitHelper PHP (#9393, #20982, #33104, #33106, #33155, #33164).
 *
 * Owns `__compiler_flock` / `__compiler_fpassthru` / `__compiler_ftruncate` / `__compiler_ftell`
 * (and peer fgetc/…) ABI module-locally: {@see getNamedFunction} first, then {@see addFunction}
 * if absent via {@see JitStreamReadBridgeKernel}. Do not re-add empty always-on shells in
 * {@see Type} — leftover decls mint flock.1 / fpassthru.1 / ftruncate.1 / ftell.1 (#31894 / #32122).
