...
/**
 * JIT/AOT link for stream read ABI via StreamReadJitHelper PHP (#9393, #20982, #33104, #33106, #33155, #33164, #33166, #33168, #33170).
 *
 * Owns `__compiler_flock` / `__compiler_fpassthru` / `__compiler_ftruncate` / `__compiler_ftell` /
 * `__compiler_fgetc` / `__compiler_fgets` / `__compiler_stream_get_line` ABI module-locally:
 * {@see getNamedFunction} first, then {@see addFunction} if absent via {@see JitStreamReadBridgeKernel}.
 * Do not re-add empty always-on shells in {@see Type} — leftover decls mint flock.1 / fpassthru.1 /
 * ftruncate.1 / ftell.1 / fgetc.1 / fgets.1 / stream_get_line.1 (#31894 / #32122).
