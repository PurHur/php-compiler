...
/**
 * LLVM stream read helpers for standalone AOT — flock/fpassthru/fseek/gets/get_contents (#5343, #9393).
 *
 * Embed JIT uses {@see StreamReadRuntime} + {@see StreamReadJitHelper} PHP instead.
 * php-src: ext/standard/flock.c, ext/standard/streams.c, ext/standard/file.c
 */
final class StreamReadStandaloneLlvm
{
    ...
    private static function declareFunction(Context $context, string $name): LlvmFunction
    {
        ...
        $ft = match ($name) {
            ...
            default => throw new \LogicException('StreamReadStandaloneLlvm: unknown '.$name),
        };
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after StreamReadStandaloneLlvm implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
