...
/**
 * JIT MCJIT body for strnatcmp (natural-order compare).
 *
 * Links {@see lib/AOT/runtime/phpc_strnatcmp.c}.
 */
final class StringStrnatcmp
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_strnatcmp.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('strnatcmp');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read strnatcmp JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_strnatcmp.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link strnatcmp JIT runtime bitcode');
        }
    ...
    {
        $fn = $context->module->getNamedFunction('strnatcmp');
        if (null === $fn) {
            throw new \LogicException('strnatcmp missing after strnatcmp bitcode link');
        }
        $context->registerFunction('strnatcmp', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('strnatcmp runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile strnatcmp JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for strnatcmp JIT runtime bitcode');
    }
