...
/**
 * LLVM phpc_similar_text (mirrors VmString::similar_text / former phpc_similar_text.c, #5173).
 *
...
final class StringSimilarTextJit
{
    ...
    private static function emitSimilarText(Context $context, LlvmFunction $fn): void
    {
        ...
        $s1Use = $context->builder->select(
            $context->builder->icmp(Builder::INT_EQ, $s1, $empty),
            $empty,
            $s1
        );
        $s2Use = $context->builder->select(
            $context->builder->icmp(Builder::INT_EQ, $s2, $empty),
            $empty,
            $s2
        );
        ...
        $tooLong = $context->builder->or(
            $context->builder->icmp(Builder::INT_UGT, $len1, $maxLen),
            $context->builder->icmp(Builder::INT_UGT, $len2, $maxLen)
        );
        ...
        $tooLongBb = $fn->appendBasicBlock('sim_too_long');
        $checkZeroBb = $fn->appendBasicBlock('sim_check_zero');
        ...
        $context->builder->branchIf($tooLong, $tooLongBb, $checkZeroBb);

        $context->builder->positionAtEnd($tooLongBb);
        $context->builder->store($negOne, $resultSlot);
        $context->builder->branch($doneBb);

        $context->builder->positionAtEnd($checkZeroBb);
