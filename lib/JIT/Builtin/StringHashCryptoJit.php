...
{
    private const RUNTIME_SOURCE = __DIR__.'/hash_crypto_jit_runtime.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $hashFn = $context->module->getNamedFunction('__compiler_hash');
        if (null !== $hashFn && $hashFn->countBasicBlocks() > 0) {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read hash JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'hash_crypto.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link hash JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after hash crypto JIT link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('hash runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile hash JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for hash JIT runtime bitcode');
    }
