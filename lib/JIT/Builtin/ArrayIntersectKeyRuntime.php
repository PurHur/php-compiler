...
/**
 * JIT/AOT link for array_intersect_key() via ArrayIntersectKeyJitHelper PHP (#12551).
 *
 * Standalone AOT keeps LLVM in {@see ArrayBuiltinHelper::arrayIntersectKey()}.
...
final class ArrayIntersectKeyRuntime
{
    ...
    public static function intersectKey(Context $context, JITVariable $first, JITVariable ...$others): Value
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            return ArrayBuiltinHelper::arrayIntersectKey($context, $first, ...$others);
        }
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after ArrayIntersectKeyJitHelper compile (#12551)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'ArrayIntersectKeyJitHelper.php');
            if (null === $block) {
                throw new \LogicException('ArrayIntersectKeyJitHelper.php parseAndCompile failed (#12551)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT (#12551)');
            }
        }
    }
