...
/**
 * JIT MCJIT body for phpc_substr_count (substr_count with offset/length).
 *
 * Links {@see lib/AOT/runtime/phpc_substr_count.c}.
 */
final class StringSubstrCount
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_substr_count.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_substr_count');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('phpc_substr_count', $fn);
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_substr_count JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_substr_count.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_substr_count JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        return $context->module->addFunction('phpc_substr_count', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_substr_count');
        if (null === $fn) {
            throw new \LogicException('phpc_substr_count missing after phpc_substr_count bitcode link');
        }
        $context->registerFunction('phpc_substr_count', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_substr_count runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_substr_count JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_substr_count JIT runtime bitcode');
    }
