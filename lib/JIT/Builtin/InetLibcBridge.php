...
/**
 * Standalone AOT libc bridge for inet_* / ip2long / long2ip (#8969).
 *
 * Embedded JIT uses {@see InetRuntime} PHP helper; standalone links libc inet directly.
 * php-src: ext/standard/basic_functions.c — PHP_FUNCTION(ip2long), long2ip, inet_ntop, inet_pton
 */
final class InetLibcBridge
{
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (['__compiler_ip2long', '__compiler_long2ip', '__compiler_inet_pton', '__compiler_inet_ntop'] as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after InetLibcBridge LLVM implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
