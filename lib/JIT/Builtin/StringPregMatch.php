...
/**
 * JIT MCJIT body for __compiler_preg_match — link shared AOT runtime (issue #93).
 *
...
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/preg_match.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read preg_match JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'preg_match.bc');
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('preg_match runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        $cache = $cacheDir.'/preg_match-'.substr(sha1($source.filemtime($source).$compiler), 0, 16).'.bc';
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile preg_match JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for preg_match JIT runtime bitcode');
    }
