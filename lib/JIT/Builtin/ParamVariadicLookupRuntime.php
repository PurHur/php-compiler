...
/** JIT/AOT link for __compiler_param_*_is_variadic via embedded tables (#24461 / peer #23593). */
final class ParamVariadicLookupRuntime
{
    ...
    {
        $abiName = '__compiler_param_func_is_variadic';
        ...
        $entry = $fn->appendBasicBlock('param_func_is_variadic_entry');
        ...
        $merge = BasicBlockHelper::append($context, 'param_func_is_variadic_merge');
        ...
        foreach ($functionParams as $funcLc => $indices) {
            foreach ($indices as $paramIndex) {
                $check = BasicBlockHelper::append($context, 'param_func_is_variadic_check_'.$seq);
                $match = BasicBlockHelper::append($context, 'param_func_is_variadic_match_'.$seq);
                ...
                $next = $merge;
                $merge = BasicBlockHelper::append($context, 'param_func_is_variadic_merge_'.$seq);
                ++$seq;
            }
        }
    ...
    {
        $abiName = '__compiler_param_method_is_variadic';
        $probe = $context->module->getNamedFunction($abiName);
        if (null !== $probe && $probe->countBasicBlocks() > 0) {
            $context->registerFunction($abiName, $probe);
        ...
        $fn = null !== $probe ? $probe : $context->module->addFunction($abiName, $ft);
        $entry = $fn->appendBasicBlock('param_method_is_variadic_entry');
        $context->builder->positionAtEnd($entry);
        ...
        $merge = BasicBlockHelper::append($context, 'param_method_is_variadic_merge');
        $resultSlot = BasicBlockHelper::entryAlloca($context, $i1);
        $context->builder->store($falseVal, $resultSlot);
        $next = $entry;
        $seq = 0;
        ...
        foreach ($methodParams as $classLc => $methods) {
            foreach ($methods as $methodLc => $positions) {
                foreach ($positions as $position) {
                    $check = BasicBlockHelper::append($context, 'param_method_is_variadic_check_'.$seq);
                    $match = BasicBlockHelper::append($context, 'param_method_is_variadic_match_'.$seq);
                    $context->builder->positionAtEnd($next);
                    $context->builder->branch($check);
                    $context->builder->positionAtEnd($check);
                    ...
                    $next = $merge;
                    $merge = BasicBlockHelper::append($context, 'param_method_is_variadic_merge_'.$seq);
                    ++$seq;
                }
            }
        }
