...
/**
 * JIT/AOT link for stream buffer/chunk/timeout ABI via StreamBufferJitHelper PHP (#14462).
 *
 * SSOT: {@see \PHPCompiler\ext\standard\StreamBufferJitHelper}
...
final class StreamBufferRuntime
{
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            $fn = $context->module->getNamedFunction($name);
            if (null === $fn || 0 === $fn->countBasicBlocks()) {
                throw new \LogicException($name.' missing after StreamBufferRuntime bridge (#14462)');
            }
            $context->registerFunction($name, $fn);
        }
    }
