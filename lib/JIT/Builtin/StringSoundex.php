...
/**
 * JIT MCJIT body for __compiler_soundex (Soundex encode).
 *
 * Links {@see lib/AOT/runtime/phpc_soundex.c}.
 */
final class StringSoundex
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_soundex.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('__compiler_soundex');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('__compiler_soundex', $fn);

            return;
        }

        $probe = $context->module->getNamedFunction('__compiler_soundex');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read __compiler_soundex JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_soundex.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link __compiler_soundex JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        $ft = $context->context->functionType($strPtr, false, $charPtr);

        return $context->module->addFunction('__compiler_soundex', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('__compiler_soundex');
        if (null === $fn) {
            throw new \LogicException('__compiler_soundex missing after __compiler_soundex bitcode link');
        }
        $context->registerFunction('__compiler_soundex', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('__compiler_soundex runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile __compiler_soundex JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for __compiler_soundex JIT runtime bitcode');
    }
