...
/**
 * JIT MCJIT body for phpc_soundex (phonetic encoding).
 *
...
final class StringSoundex
{
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_soundex');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('phpc_soundex', $fn);

            return;
        }

        $probe = $context->module->getNamedFunction('phpc_soundex');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_soundex JIT bitcode: '.$bitcode);
        }
        ...
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_soundex JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        $ft = $context->context->functionType($strPtr, false, $ptr);

        return $context->module->addFunction('phpc_soundex', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_soundex');
        if (null === $fn) {
            throw new \LogicException('phpc_soundex missing after phpc_soundex bitcode link');
        }
        $context->registerFunction('phpc_soundex', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_soundex runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_soundex JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_soundex JIT runtime bitcode');
    }
