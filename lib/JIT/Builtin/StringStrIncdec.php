...
/**
 * JIT MCJIT body for phpc_str_increment / phpc_str_decrement (issue #3102).
 *
 * Links {@see lib/AOT/runtime/phpc_str_incdec.c}.
 */
final class StringStrIncdec
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_str_incdec.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            self::declareStandalone($context, 'phpc_str_increment');
            self::declareStandalone($context, 'phpc_str_decrement');

            return;
        }

        $probe = $context->module->getNamedFunction('phpc_str_increment');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_str_incdec JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_str_incdec.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_str_incdec JIT runtime bitcode');
        }

        self::registerLinkedRuntime($context);
    }

    private static function declareStandalone(Context $context, string $name)
    {
    ...
    {
        foreach (['phpc_str_increment', 'phpc_str_decrement'] as $name) {
            $fn = $context->module->getNamedFunction($name);
            if (null === $fn) {
                throw new \LogicException($name.' missing after phpc_str_incdec bitcode link');
            }
            $context->registerFunction($name, $fn);
        }
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_str_incdec runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_str_incdec JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_str_incdec JIT runtime bitcode');
    }
