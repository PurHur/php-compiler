...
/**
 * JIT/AOT link for __phpc_stream_mode via StreamModeJitHelper PHP (#13021).
 *
...
final class StreamModeRuntime
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after StreamModeJitHelper compile (#13021)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'StreamModeJitHelper.php');
            if (null === $block) {
                throw new \LogicException('StreamModeJitHelper.php parseAndCompile failed (#13021)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT (#13021)');
            }
        }
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::ABI_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after StreamModeRuntime bridge (#13021)');
            }
