...
/**
 * JIT MCJIT bodies for addcslashes/stripcslashes/substr_replace (issue #3356).
 *
 * Links {@see lib/AOT/runtime/phpc_string_cslashes.c}.
 */
final class StringCslashes
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_string_cslashes.c';
    ...
    public static function ensureLinked(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_addcslashes');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_string_cslashes JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_string_cslashes.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_string_cslashes JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after phpc_string_cslashes bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_string_cslashes runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_string_cslashes JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_string_cslashes JIT runtime bitcode');
    }
