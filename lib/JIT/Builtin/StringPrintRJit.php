...
/**
 * LLVM __compiler_print_r (mirrors ext/standard/print_r.php / former phpc_print_r.c).
 *
 * php-src: ext/standard/var.c — php_print_r_ex()
 * Issue #6709.
 */
final class StringPrintRJit
{
    ...
    public static function implement(Context $context): void
    {
        ...
        if (null !== $probe && $probe->countBasicBlocks() > 0) {
            $context->registerFunction('__compiler_print_r', $probe);
        ...
        foreach ([
            '__phpc_pr_export_array',
            '__phpc_pr_export_value',
            '__compiler_print_r',
        ] as $recursiveName) {
        ...
        foreach ([
            '__phpc_pr_buf_append_bytes' => self::emitBufAppendBytes(...),
            '__phpc_pr_buf_append_cstr' => self::emitBufAppendCstr(...),
            '__phpc_pr_buf_append_char' => self::emitBufAppendChar(...),
            '__phpc_pr_buf_append_indent' => self::emitBufAppendIndent(...),
            '__phpc_pr_buf_append_ll' => self::emitBufAppendLl(...),
            '__phpc_pr_buf_append_double' => self::emitBufAppendDouble(...),
            '__phpc_pr_buf_append_raw_string' => self::emitBufAppendRawString(...),
            '__phpc_pr_buf_append_spaces' => self::emitBufAppendSpaces(...),
            '__phpc_pr_export_array' => self::emitExportArray(...),
            '__phpc_pr_export_value' => self::emitExportValue(...),
            '__compiler_print_r' => self::emitCompilerPrintR(...),
        ] as $name => $emit) {
    ...
    private static function declareFunction(Context $context, string $name): LlvmFunction
    {
        ...
        return match ($name) {
            '__phpc_pr_buf_append_bytes' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8p, $sizeT)
            ),
            '__phpc_pr_buf_append_cstr' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8p)
            ),
            '__phpc_pr_buf_append_char' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8)
            ),
            '__phpc_pr_buf_append_indent' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i32)
            ),
            '__phpc_pr_buf_append_ll' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i64)
            ),
            '__phpc_pr_buf_append_double' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $dbl)
            ),
            '__phpc_pr_buf_append_raw_string' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $strPtr)
            ),
            '__phpc_pr_buf_append_spaces' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i32)
            ),
            '__phpc_pr_export_array' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $htPtr, $i32)
            ),
            '__phpc_pr_export_value' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $valuePtr, $i32)
            ),
            '__compiler_print_r' => $context->module->addFunction(
                $name,
                $context->context->functionType($strPtr, false, $valuePtr)
            ),
            default => throw new \LogicException('Unknown print_r JIT helper: '.$name),
        };
    }
    ...
    private static function emitCompilerPrintR(Context $context, LlvmFunction $fn): void
    {
        ...
        $bodyBb = $fn->appendBasicBlock('pr_body');
        $finishBb = $fn->appendBasicBlock('pr_finish');
        $context->builder->branchIf($nullV, $finishBb, $bodyBb);

        $context->builder->positionAtEnd($bodyBb);
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_export_value'),
            $bufVoid,
            $v,
    ...
    private static function emitBufAppendSpaces(Context $context, LlvmFunction $fn): void
    {
        ...
        $count = $fn->getParam(1);
        $i32 = $context->getTypeFromString('int32');
        $idxSlot = $context->builder->alloca($i32, 1, 'pr_sp_i');
        $context->builder->store($i32->constInt(0, false), $idxSlot);
        ...
        $context->builder->positionAtEnd($body);
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_char'),
            $buf,
    ...
    private static function emitBufAppendRawString(Context $context, LlvmFunction $fn): void
    {
        $entry = $fn->appendBasicBlock('entry');
        $context->builder->positionAtEnd($entry);
        ...
        $done = $fn->appendBasicBlock('rs_done');
        $body = $fn->appendBasicBlock('rs_body');
    ...
    private static function emitBufAppendQuotedString(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call($context->lookupFunction('__phpc_pr_buf_append_char'), $buf, $ch);
    ...
    private static function emitExportArray(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString("Array\n"), $context->getTypeFromString('int8*'))
        );
        $zeroLevel = $context->builder->icmp(Builder::INT_EQ, $level, $i32->constInt(0, false));
        $openIndent = $fn->appendBasicBlock('pr_open_indent');
        $openParen = $fn->appendBasicBlock('pr_open_paren');
        $context->builder->branchIf($zeroLevel, $openParen, $openIndent);
        $context->builder->positionAtEnd($openIndent);
        $openCount = $context->builder->mul(
        ...
        $context->builder->branch($openParen);
        $context->builder->positionAtEnd($openParen);
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString("(\n"), $context->getTypeFromString('int8*'))
        );
        $keyIndentSlot = $context->builder->alloca($i32, 1, 'pr_key_indent');
        $keyAtRoot = $fn->appendBasicBlock('pr_key_root');
        $keyNested = $fn->appendBasicBlock('pr_key_nested');
        $keyDone = $fn->appendBasicBlock('pr_key_done');
        $context->builder->branchIf($zeroLevel, $keyAtRoot, $keyNested);
        $context->builder->positionAtEnd($keyAtRoot);
        $context->builder->store($i32->constInt(4, false), $keyIndentSlot);
        $context->builder->branch($keyDone);
        $context->builder->positionAtEnd($keyNested);
        $context->builder->store(
            $context->builder->mul(
                $context->builder->add($level, $i32->constInt(2, false)),
                $i32->constInt(4, false)
            ),
            $keyIndentSlot
        );
        ...
        $keyIndent = $context->builder->load($keyIndentSlot);
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_ll'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_raw_string'),
            $buf,
            $keyStr
        );
        $context->builder->call(
        ...
        $context->builder->positionAtEnd($done);
        $closeIndent = $fn->appendBasicBlock('pr_close_indent');
    ...
    private static function emitExportValue(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString('1'), $context->getTypeFromString('int8*'))
        );
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_buf_append_double'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_pr_export_array'),
            $buf,
