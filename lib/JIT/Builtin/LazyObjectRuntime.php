...
/** Link phpc_lazy.c into MCJIT modules (#4940). */
final class LazyObjectRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_lazy.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('phpc_lazy_is_pending');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read lazy object JIT bitcode: '.$bitcode);
        }
        ...
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link lazy object JIT runtime');
        }

        foreach (
            [
                'phpc_lazy_register',
                'phpc_lazy_is_pending',
                'phpc_lazy_is_ghost',
                'phpc_lazy_init_index',
                'phpc_lazy_mark_done',
                'phpc_lazy_unregister',
            ] as $fnName
        ) {
    ...
    private static function ensureBitcode(string $sourcePath): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('Lazy object runtime source not found: '.$sourcePath);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile lazy object JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for lazy object JIT runtime bitcode');
    }
