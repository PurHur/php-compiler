...
/**
 * JIT MCJIT bodies for opendir/readdir/closedir/rewinddir in phpc_dir.c (issue #3235).
 */
final class StringDir
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_dir.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read dir JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_dir.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link dir JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_SYMBOLS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after phpc_dir bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('dir runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile dir JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for dir JIT runtime bitcode');
    }
    ...
    private static function hostLibcIncludeFlags(): string
    {
        ...
        if ('' === $flags && is_file('/usr/include/dirent.h')) {
