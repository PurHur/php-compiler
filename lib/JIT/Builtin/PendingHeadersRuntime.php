...
/**
 * LLVM pending HTTP header queue for JIT/AOT (issue #5344, #311, #1170).
 *
...
final class PendingHeadersRuntime
{
    ...
    private static function globalPtr(Context $context, string $name, $llvmType): Value
    {
        ...
        if (null === $global) {
            throw new \LogicException('PendingHeadersRuntime global missing: '.$name);
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (
            ...
            if (null === $fn || 0 === $fn->countBasicBlocks()) {
                throw new \LogicException($name.' missing after PendingHeadersRuntime LLVM implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
