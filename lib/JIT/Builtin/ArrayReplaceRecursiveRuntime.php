...
final class ArrayReplaceRecursiveRuntime
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after ArrayReplaceRecursiveJitHelper compile (#12638)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'ArrayReplaceRecursiveJitHelper.php');
            if (null === $block) {
                throw new \LogicException('ArrayReplaceRecursiveJitHelper.php parseAndCompile failed (#12638)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT (#12638)');
            }
        }
    }
