...
/**
 * LLVM __compiler_var_dump (mirrors ext/standard/var_dump.php / former phpc_var_dump.c).
 *
 * php-src: ext/standard/var.c — php_var_dump_ex()
 * Issue #6709.
 */
final class StringVarDumpJit
{
    ...
    public static function implement(Context $context): void
    {
        ...
        if (null !== $probe && $probe->countBasicBlocks() > 0) {
            $context->registerFunction('__compiler_var_dump', $probe);
        ...
        foreach ([
            '__phpc_vd_dump_array',
            '__phpc_vd_dump_value',
            '__compiler_var_dump',
        ] as $recursiveName) {
        ...
        foreach ([
            '__phpc_vd_buf_append_bytes' => self::emitBufAppendBytes(...),
            '__phpc_vd_buf_append_cstr' => self::emitBufAppendCstr(...),
            '__phpc_vd_buf_append_char' => self::emitBufAppendChar(...),
            '__phpc_vd_buf_append_indent' => self::emitBufAppendIndent(...),
            '__phpc_vd_buf_append_ll' => self::emitBufAppendLl(...),
            '__phpc_vd_buf_append_double' => self::emitBufAppendDouble(...),
            '__phpc_vd_buf_append_spaces' => self::emitBufAppendSpaces(...),
            '__phpc_vd_buf_append_var_string' => self::emitBufAppendVarString(...),
            '__phpc_vd_dump_array' => self::emitDumpArray(...),
            '__phpc_vd_dump_value' => self::emitDumpValue(...),
            '__compiler_var_dump' => self::emitCompilerVarDump(...),
        ] as $name => $emit) {
    ...
    private static function declareFunction(Context $context, string $name): LlvmFunction
    {
        ...
        return match ($name) {
            '__phpc_vd_buf_append_bytes' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8p, $sizeT)
            ),
            '__phpc_vd_buf_append_cstr' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8p)
            ),
            '__phpc_vd_buf_append_char' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i8)
            ),
            '__phpc_vd_buf_append_indent' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i32)
            ),
            '__phpc_vd_buf_append_ll' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i64)
            ),
            '__phpc_vd_buf_append_double' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $dbl)
            ),
            '__phpc_vd_buf_append_spaces' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $i32)
            ),
            '__phpc_vd_buf_append_var_string' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $strPtr)
            ),
            '__phpc_vd_dump_array' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $htPtr, $i32)
            ),
            '__phpc_vd_dump_value' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $voidPtr, $valuePtr, $i32)
            ),
            '__compiler_var_dump' => $context->module->addFunction(
                $name,
                $context->context->functionType($void, false, $valuePtr)
            ),
            default => throw new \LogicException('Unknown var_dump JIT helper: '.$name),
        };
    }
    ...
    private static function emitCompilerVarDump(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $bufVoid,
            $context->builder->pointerCast($context->constantFromString("NULL\n"), $i8p)
        );
        $finishBb = $fn->appendBasicBlock('vd_finish');
        $context->builder->branch($finishBb);

        $context->builder->positionAtEnd($bodyBb);
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_dump_value'),
            $bufVoid,
            $v,
    ...
    private static function emitBufAppendCstr(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_bytes'),
            $buf,
            $cstr,
            $len
        );
    ...
    private static function emitBufAppendSpaces(Context $context, LlvmFunction $fn): void
    {
        $entry = $fn->appendBasicBlock('entry');
        $context->builder->positionAtEnd($entry);
        $buf = $fn->getParam(0);
        $count = $fn->getParam(1);
        $i32 = $context->getTypeFromString('int32');
        $idxSlot = $context->builder->alloca($i32, 1, 'vd_sp_i');
        $context->builder->store($i32->constInt(0, false), $idxSlot);
        $head = $fn->appendBasicBlock('sp_head');
        $body = $fn->appendBasicBlock('sp_body');
        $done = $fn->appendBasicBlock('sp_done');
        $context->builder->branch($head);
        $context->builder->positionAtEnd($head);
        $i = $context->builder->load($idxSlot);
        ...
        $context->builder->positionAtEnd($body);
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_char'),
            $buf,
    ...
    private static function emitBufAppendVarString(Context $context, LlvmFunction $fn): void
    {
        $entry = $fn->appendBasicBlock('entry');
        $context->builder->positionAtEnd($entry);
        $buf = $fn->getParam(0);
        ...
        $done = $fn->appendBasicBlock('vs_done');
        $body = $fn->appendBasicBlock('vs_body');
        $context->builder->branchIf($nullStr, $done, $body);
        $context->builder->positionAtEnd($body);
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString('string('), $i8p)
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_ll'),
            $buf,
    ...
    private static function emitBufAppendQuotedString(Context $context, LlvmFunction $fn): void
    {
        ...
        $context->builder->call($context->lookupFunction('__phpc_vd_buf_append_char'), $buf, $ch);
    ...
    private static function emitDumpArray(Context $context, LlvmFunction $fn): void
    {
        $entry = $fn->appendBasicBlock('entry');
        $context->builder->positionAtEnd($entry);
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString('array('), $context->getTypeFromString('int8*'))
        );
        $count = $context->builder->load($context->builder->structGep($ht, $map['numElements']));
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_ll'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_spaces'),
            $buf,
            $level
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_char'),
            $buf,
            $context->getTypeFromString('int8')->constInt(91, false)
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_ll'),
            $buf,
        ...
        $context->builder->call(
            ...
            $context->builder->pointerCast(
                $context->builder->structGep($keyStr, self::stringFieldMap($context)['value']),
                $context->getTypeFromString('int8*')
            ),
            $context->builder->load(
                $context->builder->structGep($keyStr, self::stringFieldMap($context)['length'])
            )
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
        ...
        $context->builder->positionAtEnd($done);
        $gtOne = $context->builder->icmp(Builder::INT_SGT, $level, $i32->constInt(1, false));
        $closeIndent = $fn->appendBasicBlock('vd_close_indent');
        $closeBrace = $fn->appendBasicBlock('vd_close_brace');
        $context->builder->branchIf($gtOne, $closeIndent, $closeBrace);
        $context->builder->positionAtEnd($closeIndent);
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_spaces'),
            $buf,
            $context->builder->sub($level, $i32->constInt(1, false))
        );
    ...
    private static function emitDumpValue(Context $context, LlvmFunction $fn): void
    {
        $entry = $fn->appendBasicBlock('entry');
        $context->builder->positionAtEnd($entry);
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString("bool(true)\n"), $context->getTypeFromString('int8*'))
        );
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString("bool(false)\n"), $context->getTypeFromString('int8*'))
        );
        $context->builder->branch($boolDone);
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString('int('), $context->getTypeFromString('int8*'))
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_ll'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString('float('), $context->getTypeFromString('int8*'))
        );
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_double'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_var_string'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_dump_array'),
            $buf,
        ...
        $context->builder->call(
            $context->lookupFunction('__phpc_vd_buf_append_cstr'),
            $buf,
            $context->builder->pointerCast($context->constantFromString("unknown()\n"), $context->getTypeFromString('int8*'))
        );
        $context->builder->branch($done);
