...
/**
 * JIT/AOT link for __compiler_pack (issue #5231).
 *
 * Bitcode from {@see pack_jit_runtime.c} until fully lowered from {@see PackEngine}.
 */
final class PackJitRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/pack_jit_runtime.c';
    ...
    public static function ensureLinked(Context $context): void
    {
        ...
        $existing = $context->module->getNamedFunction('__compiler_pack');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read pack JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'pack_jit_runtime.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link pack JIT runtime bitcode');
        }

        $fn = $context->module->getNamedFunction('__compiler_pack');
        if (null === $fn) {
            throw new \LogicException('__compiler_pack missing after pack bitcode link');
        }
        $context->registerFunction('__compiler_pack', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('pack runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile pack JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for pack JIT runtime bitcode');
    }
