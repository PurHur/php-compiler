...
/**
 * Thin-AOT ReflectionExtension::getFunctions() (#34178).
 *
 * Extension name → name-indexed ReflectionFunction map, or empty array when unknown.
 * Filters phantom `__compiler_*` builtins via {@see VmReflection::functionIsVisibleInReflection}
 * (#18362).
 *
 * Peer memcmp tables: {@see ReflectionExtensionGetClassesRuntime} (#34169).
...
final class ReflectionExtensionGetFunctionsRuntime
{
    ...
    public static function emit(Context $context, Value $nameCstr, Value $nameLen): Value
    {
        ...
        $hitIdx = 0;
        foreach (self::extensionLcToFunctionNames() as $lcExt => $names) {
    ...
    /**
     * Same name set as {@see VmReflection::reflectionExtensionFunctionsTable} keys.
     *
     * @return array<string, list<string>> lowercase extension → display function names
     */
    private static function extensionLcToFunctionNames(): array
    {
        /** @var array<string, list<string>> $byExt */
        $byExt = [];
        foreach (ModuleRegistry::extensionFunctionMap() as $ext => $bucketFuncs) {
            $lcExt = strtolower((string) $ext);
            $funcs = ModuleRegistry::getExtensionFunctions($lcExt) ?? [];
            if ('standard' === $lcExt) {
                foreach (ModuleRegistry::extensionFunctionMap() as $bucket) {
    ...
    private static function storeFunctionMap(Context $context, Value $resultSlot, array $names): void
    {
        ...
        $rfClassId = $object->lookup('ReflectionFunction');
        foreach ($names as $name) {
            $name = (string) $name;
            if ('' === $name) {
                continue;
            }
            $rfObj = $object->allocate($rfClassId);
            ReflectionSetup::markConstructed($context, $rfObj);
            ...
            HashTableHelper::setAtStringKey(
                ...
                $ht,
                $keyStr,
                new Variable($context, Variable::TYPE_OBJECT, Variable::KIND_VALUE, $rfObj)
            );
        }
        $context->builder->call(
