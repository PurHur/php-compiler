...
/**
 * JIT MCJIT body for __compiler_gethostname.
 *
 * Links {@see lib/AOT/runtime/phpc_gethostname.c}.
 */
final class StringGethostname
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_gethostname.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_gethostname');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read gethostname JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_gethostname.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link gethostname JIT runtime bitcode');
        }
    ...
    {
        $fn = $context->module->getNamedFunction('__compiler_gethostname');
        if (null === $fn) {
            throw new \LogicException('__compiler_gethostname missing after gethostname bitcode link');
        }
        $context->registerFunction('__compiler_gethostname', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('gethostname runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile gethostname JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for gethostname JIT runtime bitcode');
    }
