...
/**
 * JIT MCJIT bodies for upload temp helpers in phpc_fs_dir.c.
 *
 * Links {@see lib/AOT/runtime/phpc_upload_temp.c} for __compiler_is_uploaded_file (issue #2204).
 */
final class StringFsDir
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_upload_temp.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read fs_dir JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_fs_dir.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link fs_dir JIT runtime bitcode');
        }
    ...
    {
        foreach (['__compiler_is_uploaded_file', '__compiler_move_uploaded_file'] as $name) {
            $fn = $context->module->getNamedFunction($name);
            if (null === $fn) {
                throw new \LogicException($name.' missing after fs_dir bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('fs_dir runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile fs_dir JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for fs_dir JIT runtime bitcode');
    }
