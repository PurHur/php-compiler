...
/**
 * JIT MCJIT bodies for count_chars() in phpc_count_chars.c (issue #3310).
 */
final class StringCountChars
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_count_chars.c';

    public static function ensureLinked(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            self::declareStandalone($context, '__phpc_count_chars_array_bytes');
            self::declareStandalone($context, '__phpc_count_chars_string_bytes');

            return;
        }

        $probe = $context->module->getNamedFunction('__phpc_count_chars_array_bytes');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_count_chars JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_count_chars.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_count_chars JIT runtime bitcode');
        }

        self::registerLinkedRuntime($context);
    }

    private static function declareStandalone(Context $context, string $name): void
    {
        ...
        $ret = str_contains($name, '_string') ? $strPtr : $htPtr;
        $ft = $context->context->functionType($ret, false, $charPtr, $sizeT, $i64);
    ...
    {
        foreach (['__phpc_count_chars_array_bytes', '__phpc_count_chars_string_bytes'] as $name) {
            $fn = $context->module->getNamedFunction($name);
            if (null === $fn) {
                throw new \LogicException($name.' missing after phpc_count_chars bitcode link');
            }
            $context->registerFunction($name, $fn);
        }
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_count_chars runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_count_chars JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_count_chars JIT runtime bitcode');
    }
