...
/**
 * JIT MCJIT body for nl2br (php-src ext/standard/string.c).
 *
 * Links {@see lib/AOT/runtime/phpc_nl2br.c}.
 */
final class StringNl2brRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_nl2br.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            ...
            if (null !== $fn) {
                $context->registerFunction('__compiler_nl2br', $fn);
            }

            return;
        }

        $probe = $context->module->getNamedFunction('__compiler_nl2br');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read nl2br JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_nl2br.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link nl2br JIT runtime bitcode');
        }
        self::registerLinkedRuntime($context);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('__compiler_nl2br');
        if (null === $fn) {
            throw new \LogicException('__compiler_nl2br missing after nl2br bitcode link');
        }
        $context->registerFunction('__compiler_nl2br', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('nl2br runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile nl2br JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for nl2br JIT runtime bitcode');
    }
