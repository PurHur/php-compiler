...
/**
 * Thin-AOT ReflectionExtension::getClassNames() (#34150).
 *
 * Extension name (from ReflectionExtension::$name) → indexed string[] of
 * internal classes for that module, or empty array when unknown.
 *
 * Peer memcmp tables: {@see ReflectionClassNameListRuntime} (#34110).
 * VM: {@see \PHPCompiler\VM\Builtin\ReflectionExtensionGetClassNames} /
 * {@see VmReflection::reflectionExtensionClassNamesTable} (#22247).
 *
 * php-src: zim_ReflectionExtension_getClassNames
 */
final class ReflectionExtensionGetClassNamesRuntime
{
    ...
    public static function emit(Context $context, Value $nameCstr, Value $nameLen): Value
    {
        ...
        foreach (self::extensionLcToClassNames($context) as $lcExt => $names) {
            ...
            self::storeStringList($context, $resultSlot, $lcExt, $names);
    ...
    private static function storeStringList(
        ...
        $cacheKey = 'refl_ext_gcn_'.md5($lcExt.'|'.implode("\0", $names));
