...
class Object_ extends Type {
    ...
    /**
     * Record a trait {@code as} alias for ReflectionClass::getTraitAliases() (#34129).
     *
     * Value format matches Zend / VM {@see \PHPCompiler\VM\ClassEntry::$traitAliases}:
     * {@code "TraitName::method"}.
     */
    public function recordClassTraitAlias(string $classLc, string $alias, string $traitMethod): void
    {
        $lc = strtolower(ltrim($classLc, '\\'));
        if ('' === $alias || '' === $traitMethod) {
            return;
        }
    ...
    private function registerExternalClass(string $lcname, string $displayName): void
    {
        ...
        if ('domdocument' === $lcname) {
            ...
            // loadXML document meta props — same late-define OOB class (#34894 / peer #34887).
            $this->defineProperty($id, \PHPCompiler\ext\dom\VmDom::PROP_XML_VERSION, Variable::TYPE_VALUE);
