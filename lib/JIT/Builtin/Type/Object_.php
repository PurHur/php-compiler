...
class Object_ extends Type {
     ...
     *
     * Thin AOT: {@see StringCaseCompare} strncasecmp always "matched", so the
     * first registered class won (e.g. ErrorLastJitHelper). ASCII-fold the
     * needle and length-checked {@see memcmp} against each compile-unit name
     * (peer {@see ReflectionClassGetParentClassRuntime}).
     */
    public function classIdFromRuntimeName(PHPLLVM\Value $namePtr, PHPLLVM\Value $nameLen): PHPLLVM\Value
    {
        ...
        LibcExtern::ensureMemcmpDecl($this->context);
        ...
        $check = $afterFold;
        $hasCase = false;
        $names = $this->allClassNamesById();
        $n = \count($names);
        $i = 0;
        foreach ($names as $id => $className) {
            $hasCase = true;
            $this->context->builder->positionAtEnd($check);
            $lcName = strtolower(ltrim($className, '\\'));
            $wantLenInt = \strlen($lcName);
            $wantLen = $sizeT->constInt($wantLenInt, false);
            $wantGlobal = $this->context->constantStringFromString($lcName);
            ...
            $lenEq = $this->context->builder->icmp(PHPLLVM\Builder::INT_EQ, $nameLen, $wantLen);
            ...
            $nameEq = $this->context->builder->icmp(
                PHPLLVM\Builder::INT_EQ,
                $cmp,
                $i32->constInt(0, false)
            );
            $match = $this->context->builder->and($lenEq, $nameEq);
            $case = $fn->appendBasicBlock('class_name_id_'.$id);
            $next = ($i === $n - 1)
                ? $fail
                : $fn->appendBasicBlock('class_name_id_try_'.$id);
     ...
     * PROFILE≤8.2 throws ValueError (#29150). Alias-of-alias to canonical (#11639).
     * Duplicate names warn + false (#29084 / re-#18290).
     */
    public function registerClassAlias(string $original, string $alias): bool
    {
        ...
        if (isset($this->externalOnlyClassIds[$classId])
            && !\PHPCompiler\CompilerVersion::allowsClassAliasOfInternalClass()) {
    ...
    /**
     * ReflectionClass::getProperties() specs — skip parent privates; honour filter (#34113).
     *
     * @return list<array{display: string, declaringClass: string, isStatic: bool}>
     */
    public function allPropertiesForReflection(int $classId, int $filter = 0): array
    {
        ...
        $seen = [];
        /** @var list<array{display: string, declaringClass: string, isStatic: bool}> $result */
        $result = [];

        $currentId = $classId;
        for ($depth = 0; $depth < 64; ++$depth) {
            foreach ($this->instancePropertySets($currentId) as $propset) {
                $propName = $propset[1];
                $lc = strtolower($propName);
                if (isset($seen[$lc])) {
                    continue;
                ...
                $vis = $this->propertyVisibility($currentId, $propName);
                $declName = $this->instancePropertyDeclaringClassName($currentId, $propName);
                $declLc = strtolower(ltrim($declName, '\\'));
                // php-src: parent-private properties hidden on child (#7191 / #34091).
        ...
        foreach (array_keys($globals) as $name) {
            ...
            $vis = $this->staticPropertyVisibility[$classId][$name]
            ...
            $result[] = [
                'display' => (string) $name,
                'declaringClass' => $this->classNameForId($declId),
                'isStatic' => true,
            ];
        }
