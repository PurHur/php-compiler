...
class HashTable extends Type
{
    ...
    public function register(): void
    {
        ...
        // SORT_STRING|SORT_FLAG_CASE — NestedJIT SortJitHelper is a no-op under thin AOT (#34702).
        $this->registerFn('__hashtable__sortPackedCase', 'void', ['__hashtable__*']);
        $this->registerFn('__hashtable__sortPackedReverseCase', 'void', ['__hashtable__*']);
    ...
    /**
     * Bubble-sort packed list values in place (sort / rsort SORT_REGULAR or SORT_*|SORT_FLAG_CASE) (#24010, #34702).
     *
     ...
     * Case-insensitive uses {@see StringCaseCompare::ABI_STRCASECMP} (peer natural case).
     */
    private function implementSortPacked(bool $reverse, bool $caseInsensitive = false): void
    {
        if ($caseInsensitive) {
            $abi = $reverse ? '__hashtable__sortPackedReverseCase' : '__hashtable__sortPackedCase';
            $tag = $reverse ? 'rsort_case' : 'sort_case';
        } else {
        ...
        // Length-prefixed strcasecmp — not __compiler_strcasecmp (strlen on non-NUL payloads) (#34702).
        $cmp = $caseInsensitive
