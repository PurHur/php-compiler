...
/**
 * JIT MCJIT bodies for __compiler_microtime_string / __compiler_microtime_float.
 *
 * Links {@see lib/AOT/runtime/phpc_microtime.c}.
 */
final class StringMicrotime
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_microtime.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_microtime_string');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read microtime JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_microtime.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link microtime JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (['__compiler_microtime_string', '__compiler_microtime_float'] as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after microtime bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('microtime runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile microtime JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for microtime JIT runtime bitcode');
    }
