...
/**
 * JIT MCJIT body for strtok (php-src ext/standard/string.c).
 *
 * Links {@see lib/AOT/runtime/phpc_strtok.c}.
 */
final class StringStrtok
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_strtok.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_strtok');
            if (null !== $fn) {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read strtok JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_strtok.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link strtok JIT runtime bitcode');
        }
        self::registerLinkedRuntime($context);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_strtok');
        if (null === $fn) {
            throw new \LogicException('phpc_strtok missing after strtok bitcode link');
        }
        $context->registerFunction('phpc_strtok', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('strtok runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile strtok JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for strtok JIT runtime bitcode');
    }
