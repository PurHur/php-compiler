...
/**
 * JIT/AOT link for strspn()/strcspn() via StrspnJitHelper PHP (#14700).
 *
...
final class StringStrspn
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after StrspnJitHelper compile (#14700)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        NestedJitCompileScope::run($context, static function () use ($context, $runtime, $path): void {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'StrspnJitHelper.php');
            if (null === $block) {
                throw new \LogicException('StrspnJitHelper.php parseAndCompile failed (#14700)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        });
        foreach (self::COMPILED_HELPERS as $logical) {
            if (!isset($context->functions[\strtolower($logical)])) {
                throw new \LogicException($logical.' was not compiled for JIT (#14700)');
            }
        }
    }
