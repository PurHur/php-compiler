...
/**
 * JIT MCJIT body for phpc_strspn_ex (strspn/strcspn with offset/length).
 *
 * Links {@see lib/AOT/runtime/phpc_strspn.c}.
 */
final class StringStrspn
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_strspn.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_strspn_ex');
            if (null === $fn) {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_strspn_ex JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_strspn_ex.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_strspn_ex JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        return $context->module->addFunction('phpc_strspn_ex', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_strspn_ex');
        if (null === $fn) {
            throw new \LogicException('phpc_strspn_ex missing after phpc_strspn_ex bitcode link');
        }
        $context->registerFunction('phpc_strspn_ex', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_strspn_ex runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_strspn_ex JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_strspn_ex JIT runtime bitcode');
    }
