...
/**
 * JIT/AOT link for __compiler_filter_validate_url via FilterUrlJitHelper PHP (#11274).
 *
...
final class StringFilterUrl
{
    ...
    private static function helperFunction(Context $context, string $logical): LlvmFunction
    {
        ...
        if (null === $fn) {
            throw new \LogicException($logical.' missing after FilterUrlJitHelper compile (#11274)');
        }
    ...
    private static function ensureJitHelperCompiled(Context $context): void
    {
        ...
        try {
            $block = $runtime->parseAndCompile((string) \file_get_contents($path), 'FilterUrlJitHelper.php');
            if (null === $block) {
                throw new \LogicException('FilterUrlJitHelper.php parseAndCompile failed (#11274)');
            }
            $jit = new JIT($context);
            $jit->compile($block);
        } finally {
        ...
        foreach (self::COMPILED_HELPERS as $logical) {
            ...
            if (!isset($context->functions[$lc])) {
                throw new \LogicException($lc.' was not compiled for JIT (#11274)');
            }
        }
    }
