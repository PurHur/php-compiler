...
/**
 * JIT MCJIT bodies for __compiler_time_nanosleep / __compiler_time_sleep_until.
 *
 * Links {@see lib/AOT/runtime/phpc_time_sleep.c}.
 */
final class StringTimeSleep
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_time_sleep.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_time_nanosleep');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read time_sleep JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_time_sleep.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link time_sleep JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (['__compiler_time_nanosleep', '__compiler_time_sleep_until'] as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after time_sleep bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('time_sleep runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile time_sleep JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for time_sleep JIT runtime bitcode');
    }
