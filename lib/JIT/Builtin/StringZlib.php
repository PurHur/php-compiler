...
/**
 * JIT MCJIT bodies for gzcompress/gzuncompress/gzdeflate/gzinflate/gzencode/gzdecode.
 *
 * Links {@see lib/AOT/runtime/zlib_compress.c} (libz).
 */
final class StringZlib
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/zlib_compress.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read zlib JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'zlib_compress.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link zlib JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after zlib bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('zlib runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        $cache = $cacheDir.'/zlib_compress-'.substr(
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile zlib JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for zlib JIT runtime bitcode');
    }
