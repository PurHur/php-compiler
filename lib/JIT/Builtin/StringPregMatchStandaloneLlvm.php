...
/**
 * LLVM preg_* runtime for standalone AOT (#5289, #9542).
 *
 * Embed JIT uses {@see PregMatchRuntime} + {@see PregJitHelper} PHP instead.
...
final class StringPregMatchStandaloneLlvm
{
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after StringPregMatchStandaloneLlvm LLVM implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
