...
/**
 * JIT MCJIT body for phpc_metaphone (phonetic encoding).
 *
 * Links {@see lib/AOT/runtime/phpc_metaphone.c}.
 */
final class StringMetaphone
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_metaphone.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_metaphone');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('phpc_metaphone', $fn);

            return;
        }

        $probe = $context->module->getNamedFunction('phpc_metaphone');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_metaphone JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_metaphone.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_metaphone JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        return $context->module->addFunction('phpc_metaphone', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_metaphone');
        if (null === $fn) {
            throw new \LogicException('phpc_metaphone missing after phpc_metaphone bitcode link');
        }
        $context->registerFunction('phpc_metaphone', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_metaphone runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_metaphone JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_metaphone JIT runtime bitcode');
    }
