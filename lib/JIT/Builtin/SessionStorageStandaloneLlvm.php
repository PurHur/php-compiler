...
/**
 * Standalone AOT LLVM for session file I/O (#9495, #6968).
 *
 * Embed/MCJIT routes through {@see SessionStorageJitHelper} via {@see SessionStorageRuntime}.
 * php-src: ext/session/mod_files.c
 */
final class SessionStorageStandaloneLlvm
{
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn || 0 === $fn->countBasicBlocks()) {
                throw new \LogicException($name.' missing after SessionStorageStandaloneLlvm implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
