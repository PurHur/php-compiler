...
/**
 * JIT/AOT link for convert_uuencode/convert_uudecode (issue #5277).
 *
 * Bitcode from {@see uuencode_jit_runtime.c} — removed from lib/AOT/Linker.php.
 * VM uses ext/standard/VmString.php; full PHP LLVM lowering is follow-up on #5277.
 */
final class ConvertUuRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/uuencode_jit_runtime.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $probe = $context->module->getNamedFunction('__compiler_convert_uuencode');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read uuencode JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'uuencode_jit_runtime.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link uuencode JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (self::RUNTIME_FUNCTIONS as $name) {
            ...
            if (null === $fn) {
                throw new \LogicException($name.' missing after uuencode bitcode link');
            }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('uuencode runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile uuencode JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for uuencode JIT runtime bitcode');
    }
