...
/**
 * JIT MCJIT body for levenshtein (edit distance between two strings).
 *
...
final class StringLevenshtein
{
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('levenshtein');
            if (null !== $fn) {
        ...
        $probe = $context->module->getNamedFunction('levenshtein');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read levenshtein JIT bitcode: '.$bitcode);
        }
        ...
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link levenshtein JIT runtime bitcode');
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('levenshtein');
        if (null === $fn) {
            throw new \LogicException('levenshtein missing after levenshtein bitcode link');
        }
        $context->registerFunction('levenshtein', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('levenshtein runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile levenshtein JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for levenshtein JIT runtime bitcode');
    }
