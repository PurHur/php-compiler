...
/**
 * JIT MCJIT body for phpc_levenshtein (edit distance).
 *
 * Links {@see lib/AOT/runtime/phpc_levenshtein.c}.
 */
final class StringLevenshtein
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_levenshtein.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_levenshtein');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('phpc_levenshtein', $fn);
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_levenshtein JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_levenshtein.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_levenshtein JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        $ft = $context->context->functionType($i32, false, $ptr, $ptr, $i64, $i64, $i64);

        return $context->module->addFunction('phpc_levenshtein', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_levenshtein');
        if (null === $fn) {
            throw new \LogicException('phpc_levenshtein missing after phpc_levenshtein bitcode link');
        }
        $context->registerFunction('phpc_levenshtein', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_levenshtein runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_levenshtein JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_levenshtein JIT runtime bitcode');
    }
