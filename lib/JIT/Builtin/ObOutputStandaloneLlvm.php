...
/**
 * LLVM output-buffer stack for JIT/AOT (issue #5314, #118, #1056).
 *
 * Replaces lib/AOT/runtime/phpc_ob.c. php-src: ext/standard/output.c
 */
final class ObOutputStandaloneLlvm
{
    ...
    private static function storageRowPtr(Context $context, Value $idx): Value
    {
        ...
        if (null === $storage) {
            throw new \LogicException('ObOutputRuntime: storage global missing');
        }
    ...
    private static function globalPtr(Context $context, string $name, $llvmType): Value
    {
        ...
        if (null === $global) {
            throw new \LogicException('ObOutputRuntime global missing: '.$name);
        }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        foreach (
            ...
            if (null === $fn || 0 === $fn->countBasicBlocks()) {
                throw new \LogicException($name.' missing after ObOutputRuntime LLVM implement');
            }
            $context->registerFunction($name, $fn);
        }
    }
