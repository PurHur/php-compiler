...
/**
 * JIT/AOT openssl_pbkdf2() via HMAC built from {@see __compiler_hash} (#32410, #32869).
 *
 ...
 * empty `__compiler_openssl_pbkdf2` shells (#32869 / peer #32866) — leftover Type decls
 * mint openssl_pbkdf2.1 (#31894 / #32122).
 *
...
final class OpensslPbkdf2Runtime
{
    ...
    private static function declareAbi(Context $context, string $name, ?LlvmFunction $probe): LlvmFunction
    {
        ...
        $ft = match ($name) {
            '__compiler_openssl_pbkdf2' => $context->context->functionType(
                $strPtr,
                false,
                $strPtr,
                $strPtr,
            ...
            default => throw new \LogicException(
                'OpensslPbkdf2Runtime unknown ABI '.$name.' (#32869)'
            ),
        };
