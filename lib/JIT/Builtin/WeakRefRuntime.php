...
/** Link phpc_weakref.c into MCJIT modules (#3667). */
final class WeakRefRuntime
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_weakref.c';
    ...
    public static function implement(Context $context): void
    {
        ...
        $bitcode = self::ensureBitcode(self::RUNTIME_SOURCE);
        $data = file_get_contents($bitcode);
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read weakref JIT bitcode: '.$bitcode);
        }
        ...
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link weakref JIT runtime');
        }
    ...
    private static function ensureBitcode(string $sourcePath): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('Weakref runtime source not found: '.$sourcePath);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile weakref JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No clang compiler found for weakref JIT runtime bitcode');
    }
