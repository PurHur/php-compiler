...
/**
 * JIT MCJIT body for phpc_htmlspecialchars_decode (web entity decode subset).
 *
 * Links {@see lib/AOT/runtime/phpc_htmlspecialchars_decode.c}.
 */
final class StringHtmlspecialcharsDecode
{
    private const RUNTIME_SOURCE = __DIR__.'/../../AOT/runtime/phpc_htmlspecialchars_decode.c';
    ...
    public static function implement(Context $context): void
    {
        if (Builtin::LOAD_TYPE_STANDALONE === $context->loadType) {
            $fn = $context->module->getNamedFunction('phpc_htmlspecialchars_decode');
            if (null === $fn) {
                $fn = self::declareStandalone($context);
            }
            $context->registerFunction('phpc_htmlspecialchars_decode', $fn);

            return;
        }

        $probe = $context->module->getNamedFunction('phpc_htmlspecialchars_decode');
        ...
        if (false === $data || '' === $data) {
            throw new \LogicException('Failed to read phpc_htmlspecialchars_decode JIT bitcode: '.$bitcode);
        }
        $buffer = $context->llvm->createMemoryBufferWithString($data, 'phpc_htmlspecialchars_decode.bc');
        $runtimeModule = $buffer->parseBitcode($context->context);
        if (!$context->module->link($runtimeModule)) {
            throw new \LogicException('Failed to link phpc_htmlspecialchars_decode JIT runtime bitcode');
        }
    ...
    private static function declareStandalone(Context $context)
    {
        ...
        $ft = $context->context->functionType($strPtr, false, $ptr, $i64);

        return $context->module->addFunction('phpc_htmlspecialchars_decode', $ft);
    }
    ...
    private static function registerLinkedRuntime(Context $context): void
    {
        $fn = $context->module->getNamedFunction('phpc_htmlspecialchars_decode');
        if (null === $fn) {
            throw new \LogicException('phpc_htmlspecialchars_decode missing after runtime bitcode link');
        }
        $context->registerFunction('phpc_htmlspecialchars_decode', $fn);
    }
    ...
    private static function ensureBitcode(): string
    {
        ...
        if (false === $source || !is_file($source)) {
            throw new \LogicException('phpc_htmlspecialchars_decode runtime source not found: '.self::RUNTIME_SOURCE);
        }
        ...
        if (!is_file($cache)) {
            throw new \LogicException(
                'Failed to compile phpc_htmlspecialchars_decode JIT bitcode: '.trim((string) $output)
            );
        }
    ...
    private static function resolveCompiler(): string
    {
        ...
        throw new \LogicException('No C compiler found for phpc_htmlspecialchars_decode JIT runtime bitcode');
    }
