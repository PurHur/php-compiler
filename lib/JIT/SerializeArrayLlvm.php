 ...
 *
 * NestedJIT {@see \PHPCompiler\ext\standard\SerializeNestedJitHelper::encodeHashtable}
 * SIGABRTs on non-empty HTs under thin AOT (flat `[1]`, assoc, nested). Peer of
 * {@see JsonEncodeArrayLlvm}: walk export pairs in LLVM and build PHP serialize wire.
 *
 * php-src: ext/standard/var.c — php_var_serialize
 */
final class SerializeArrayLlvm
{
    ...
    public static function encode(Context $context, Value $ht, Value $flags): Value
    {
        ...
        // Header: a:N:{
        $aColon = $context->builder->load($context->constantStringFromString('a:'));
        ...
        $header = JitStringConcat::concat(
            $context,
            JitStringConcat::concat($context, $aColon, $numDigits),
            $openBrace
        );
        ...
        $keyStrBlock = BasicBlockHelper::append($context, 'ser_ht_enc_key_str_'.$tag);
        $keyLongBlock = BasicBlockHelper::append($context, 'ser_ht_enc_key_long_'.$tag);
        $keyDone = BasicBlockHelper::append($context, 'ser_ht_enc_key_done_'.$tag);
