...
trait DiscardedPureCallElisionNativeLongPowFolds
{
     ...
     * - {@code $n ** 86} / {@code pow($n, 86)} → chained smul overflow→float (eightyfifth × n)
     * - {@code $n ** 87} / {@code pow($n, 87)} → chained smul overflow→float (eightysixth × n)
     * - {@code $n ** 88} / {@code pow($n, 88)} → chained smul overflow→float (eightyseventh × n)
     * - {@code $n ** 89} / {@code pow($n, 89)} → chained smul overflow→float (eightyeighth × n)
     ...
     * - {@code $n ** 95} / {@code pow($n, 95)} → chained smul overflow→float (ninetyfourth × n)
     * - {@code $n ** 96} / {@code pow($n, 96)} → chained smul overflow→float (ninetyfifth × n)
     * - {@code $n ** 97} / {@code pow($n, 97)} → chained smul overflow→float (ninetysixth × n)
     * - {@code $n ** 98} / {@code pow($n, 98)} → chained smul overflow→float (ninetyseventh × n)
     *
    ...
    public static function nativeLongPowCompileTimeExponentFold(
        ...
        if (102 === $e) {
            return 'hundredsecond';
        }
        if (103 === $e) {
            return 'hundredthird';
        }
        if (104 === $e) {
            return 'hundredfourth';
        }
        if (105 === $e) {
            return 'hundredfifth';
        }
        ...
        if (107 === $e) {
            return 'hundredseventh';
        }
        if (108 === $e) {
            return 'hundredeighth';
        }
