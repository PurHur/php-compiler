...
/**
 * Call-site / helper LLVM print_r() on {@see __hashtable__*} (#34497).
 *
 * Thin standalone AOT {@see Builtin\StringPrintR} scalar bridge aborted on arrays;
 * walk export pairs in LLVM (peer {@see VarExportArrayLlvm} / {@see SerializeArrayLlvm}).
 *
 * php-src: ext/standard/var.c — zend_print_zval_r / php_array_element_dump
 */
final class PrintRArrayLlvm
{
    public const HELPER = '__compiler_print_r_hashtable';
    ...
    public static function ensureHelper(Context $context): void
    {
        ...
        BasicBlockHelper::scopeLoweringToFunction($context, $fn, self::HELPER, static function () use ($context, $fn): void {
            $entry = $fn->appendBasicBlock('pr_ht_entry');
    ...
    private static function encodeBody(Context $context, Value $ht, Value $level): Value
    {
        ...
        // openSpaces = level==0 ? 0 : 4*(level+1); keySpaces = 4*(level==0 ? 1 : level+2)
        $isTop = $context->builder->icmp(Builder::INT_EQ, $level, $i64->constInt(0, false));
        ...
        $keyCount = $context->builder->select(
            ...
            $i64->constInt(4, false),
            $context->builder->mul(
                $context->builder->add($level, $i64->constInt(2, false)),
                $i64->constInt(4, false)
            )
        );
        $openSpaces = self::spaces($context, $openCount);
        $keySpaces = self::spaces($context, $keyCount);

        $header = $context->builder->load($context->constantStringFromString("Array\n"));
        $openParen = JitStringConcat::concat(
            $context,
            $openSpaces,
            $context->builder->load($context->constantStringFromString("(\n"))
        );
        $acc0 = JitStringConcat::concat($context, $header, $openParen);
        ...
        $head = BasicBlockHelper::append($context, 'pr_ht_head_'.$tag);
        $body = BasicBlockHelper::append($context, 'pr_ht_body_'.$tag);
        $done = BasicBlockHelper::append($context, 'pr_ht_done_'.$tag);
        ...
        $keyStrBlock = BasicBlockHelper::append($context, 'pr_ht_key_str_'.$tag);
        $keyLongBlock = BasicBlockHelper::append($context, 'pr_ht_key_long_'.$tag);
        $keyDone = BasicBlockHelper::append($context, 'pr_ht_key_done_'.$tag);
        ...
        $lb = $context->builder->load($context->constantStringFromString('['));
        $rb = $context->builder->load($context->constantStringFromString(']'));
        $keyWireStr = JitStringConcat::concat(
            $context,
            JitStringConcat::concat($context, $lb, $rawKey),
            $rb
        );
        ...
        $lb2 = $context->builder->load($context->constantStringFromString('['));
        $rb2 = $context->builder->load($context->constantStringFromString(']'));
        $keyWireLong = JitStringConcat::concat(
            $context,
            JitStringConcat::concat($context, $lb2, $keyDigits),
            $rb2
        );
    ...
    private static function formatValueAtLevel(Context $context, Value $valPtr, Value $level): Value
    {
        ...
        $htBlock = BasicBlockHelper::append($context, 'pr_ht_val_ht_'.$tag);
        $scalarBlock = BasicBlockHelper::append($context, 'pr_ht_val_scalar_'.$tag);
        $done = BasicBlockHelper::append($context, 'pr_ht_val_done_'.$tag);
