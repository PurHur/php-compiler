...
/**
 * Operand-value assign and compile-time / property-backing meta sync (#36387).
 *
 * Extracted from {@see \PHPCompiler\JIT}: {@code assignOperandValue} through
 * {@code copyObjectPropertyBacking} so the hub keeps shrinking toward gen-0
 * split-TU iterability under the size-budget ratchet.
 *
 * php-src: Zend/zend_execute.c assignment / CV store paths and compile-time
 * constant folding metadata — move-only Concern extract; no new C ABI and no
 * opcode/IR shape change.
 */
trait AssignOperandValueAndCompileTimeMeta
{
