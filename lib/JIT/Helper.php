...
class Helper {
    ...
    public function unaryOp(OpCode $opcode, Variable $var): Variable {
        // object/array ~ : zend_type_error, not compiler abort (#32486).
        if (OpCode::TYPE_BITWISE_NOT === $opcode->type) {
            ...
            if (JitObjectNumericOperandGuard::guardUnary(
                $this->context,
                $var,
                static fn (string $class): string => 'Cannot perform bitwise not on '.$class
            )) {
                return $this->nativeLongResultVariable(
                    $this->context->getTypeFromString('int64')->constInt(0, false)
                );
