...
class JIT {
  ...
  /** LLVM/C symbols reserved for the AOT entry wrapper and runtime init (#2779). */
    ...
    /**
     * M3 compile-driver loadJitContext (#1402, #2846): separate FUNCDEF from loadJit — do not inline.
     * `jitContextForLoadJit` / `createJit` stay deny-listed while outer loadJit is real-lowered (#1495).
     */
    ...
    private function tryCompileM3EmitTuRuntimeSpineNative(
        ...
        if (str_ends_with($emitLc, '\\runtime::loadjit')
            ...
            if ($this->shouldUseM3CompileDriverRealLowering() && str_ends_with($emitLc, '\\runtime::loadjit')) {
                return null;
            }
