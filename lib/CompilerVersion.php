...
final class CompilerVersion
{
    ...
    /**
     * PHP 8.5+ parenthesized clone-with (`clone($obj, [...])`).
     *
     * Zend landed the array form in 8.5; PROFILE≤8.4 must reject like Zend (#23877, re-#16676/#12987).
     * Keyword `clone $obj with { }` / `with […]` is **not** in php-src — always rejected (#29187).
     * Named `clone($obj, with: [...])` is rejected on PROFILE≥8.5 like Zend 8.5.8 (#28182).
     * Forward profile via {@see languageProfileVersion()} enables the array form on 8.5+.
     ...
     *
     * Gated on {@see languageProfileVersion()} so 8.4.0-dev reference profile rejects like Zend 8.2
     * (#30483, re-#24818 / #22781 / #22371 / #18531). `version_compare` treats `8.4.0-dev` as below
     * `8.4.0`, so unset `PHP_COMPILER_PROFILE` keeps this false. Forward profile via
     * `PHP_COMPILER_PROFILE=8.4` (or stable 8.4.0+) enables hook syntax.
     * Do not use VERSION_ID / isForwardProfileAtLeast here — that re-enabled acceptance on default
     * after #30204/#30221 and broke Zend 8.2 parity (#30483).
     ...
     * Zend 8.2 (methods absent). Aligns with {@see supportsPropertyHooks()} (#30483, #24818, #24672).
 ...
 */
    ...
    /**
     * get_defined_constants() optional $category named filter — never shipped by Zend/php-src (#28522 / #12947).
     *
     * php-src Zend/zend_builtin_functions.stub.php keeps arity 1 through master
     * (`function get_defined_constants(bool $categorize = false): array {}`). Prior forward-profile
     * gate (#12947 / #17436) was wrong-direction; always withhold.
     */
    ...
    /**
     * PHP 8.3+ clarified libxml null entity-loader warning (ext/libxml/libxml.c; php-src #11952).
     *
     * Withheld on 8.4.0-dev reference / PROFILE=8.2 (Zend 8.2: Failed to load external entity "NULL").
     * Enable via stable 8.4.0+ or explicit `PHP_COMPILER_PROFILE=8.3` / `8.4` forward profile (#30424).
     */
    public static function supportsLibxmlNullEntityLoaderClarifiedMessage(): bool
    {
        if (version_compare(self::VERSION, '8.3', '<')) {
            return false;
        }
