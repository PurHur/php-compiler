 ...
 * getPathname/getPath/__toString reuse the same join / `__dir_path` (#33274).
 *
...
final class DirectoryIteratorJitHelper
{
     ...
     * php-src: ext/spl/spl_directory.c — zim_SplFileInfo_getPath
     */
    ...
    /**
     * DirectoryIterator::__toString → filename; SplFileInfo::__toString → pathname (#33274).
     * php-src: zim_DirectoryIterator___toString / zim_SplFileInfo___toString
     */
    public static function compileToString(Context $context, JITVariable $receiver, string $className): Value
    {
        if ('SplFileInfo' === $className) {
            return self::compileGetPathname($context, $receiver, $className);
        }

        // DirectoryIterator / FilesystemIterator — basename only (not pathname).
