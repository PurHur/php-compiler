...
iterations, skips JIT after a VM time-cap, reuses a verify sample when it already
took ≥ 5 s, and honors `// @bench-skip-vm: &lt;reason&gt;` so a known time-cap case cannot
burn the full cap. `binary-trees` uses `maxDepth = 7` and `fannkuch-redux` uses `n = 6`
so VM/JIT columns stay measurable under that cap (depth 10 / n=8 timed out). Fast gate:
`php script/check-bench-readme-sync.php --check-wall` projects wall from committed `RESULTS.json`.
...
Environment: 8.2.33 · LLVM 9 available · 3 iterations averaged, wall time per run.

| Test Name          | Zend       8.2 (s)| bin/vm.php (s) | bin/jit.php (s) | phpc build (s) | native run (s) |
|--------------------|-------------------|----------------|-----------------|----------------|----------------|
|        assoc-heavy |            0.0125 |         2.4465 |         2.5408 |         0.1881 |         0.0088 |
|       binary-trees |            0.0220 |        10.4585 |        10.1683 |         0.2830 |         0.0095 |
...
|     fannkuch-redux |            0.0176 |         7.6686 |         7.3140 |         0.2859 |         0.0064 |
