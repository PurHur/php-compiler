...
class Parser
{
    ...
    protected function parseExpr_NullsafePropertyFetch(Expr\NullsafePropertyFetch $expr)
    {
        return new Op\Expr\NullsafePropertyFetch(
            $this->readVariable($this->parseExprNode($expr->var)),
            $this->readVariable($this->parseExprNode($expr->name)),
            $this->mapAttributes($expr)
        );
