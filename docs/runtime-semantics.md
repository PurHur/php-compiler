...
| **M5** | Separate before writing a shared container (`rc > 1`) | `__ref__assert_exclusive` from `__hashtable__grow`, `lookupStringKeyForWriteBranch`, and object-key setters (`setObjectKeyLong` / `setObjectKeyObject`); inject probe | Arrays/strings COW; abort `PHPC_RUNTIME_ASSERT M5` |
...
Under ASSERT, packed writes (`__hashtable__grow`), string-key writes (`lookupStringKeyForWriteBranch`), and object-key writes (`__hashtable__setObjectKeyLong` / `setObjectKeyObject`, SplObjectStorage / WeakMap) call `__ref__assert_exclusive` on the container before mutating — so a skipped `separate()` on a shared array aborts with **M5** (php-src `SEPARATE_ARRAY` in [Zend/zend_variables.h](https://github.com/php/php-src/blob/master/Zend/zend_variables.h)). Exclusive containers (`rc <= 1`) and unclaimed `rc=0` headers pass.
