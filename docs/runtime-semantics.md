...
Under ASSERT, packed writes (`__hashtable__grow`), string-key writes (`lookupStringKeyForWriteBranch`), object-key writes (`__hashtable__setObjectKeyLong` / `setObjectKeyObject`, SplObjectStorage / WeakMap), unsets (`__hashtable__unsetLongAt` / `unsetStringKey` / `unsetObjectKey`), packed sorts (`__hashtable__sortPacked*`), and by-ref stdlib mutators (`array_push` / `array_unshift` / `array_pop` / `array_shift` / `array_splice` / `sort` / `rsort` / `asort` / `arsort` / `ksort` / `krsort` / `natsort` / `usort` / `array_walk` / `array_multisort` / `shuffle` — `HashTableHelper::separateContainerForWrite` then pop/shift/splice/sort/shuffle LLVM which also call `__ref__assert_exclusive`; `array_multisort` asserts at the MultisortRuntime call site before `packVariables` addrefs) call `__ref__assert_exclusive` on the container before mutating — so a skipped `separate()` on a shared array aborts with **M5** (php-src `SEPARATE_ARRAY` in [Zend/zend_variables.h](https://github.com/php/php-src/blob/master/Zend/zend_variables.h); `array_push` / `array_splice` / `php_array_sort` / `php_array_walk` / `php_array_multisort` / `php_shuffle` in [ext/standard/array.c](https://github.com/php/php-src/blob/master/ext/standard/array.c)). Exclusive containers (`rc <= 1`) and unclaimed `rc=0` headers pass.
