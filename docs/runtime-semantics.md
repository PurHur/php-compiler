...
| **M5** | Separate before writing a shared container (`rc > 1`) | `__ref__assert_exclusive` from `__hashtable__grow`, `lookupStringKeyForWriteBranch`, object-key setters (`setObjectKeyLong` / `setObjectKeyObject`), unset (`unsetLongAt` / `unsetStringKey` / `unsetObjectKey`), packed sort (`__hashtable__sortPacked*`), nested FETCH_DIM_W / `array_walk_recursive` descend (`HashTableWriteLlvm::exclusiveNestedChildHashtable`), foreach-by-ref (`VmIteratorForeach::compileValueByRefHashtable` → `separateContainerForWrite`, php-src `ZEND_FE_RESET_RW`), and by-ref mutators (`array_push` / `unshift` / `pop` / `shift` / `array_splice` / `sort` / `asort` / `ksort` / `natsort` / `usort` / `array_walk` / `array_multisort` / `shuffle` / `next` / `prev` / `reset` / `end` via `separateContainerForWrite` + pop/shift/splice/sort/shuffle/pointer LLVM / MultisortRuntime call-site assert before pack); inject probe | Arrays/strings COW; abort `PHPC_RUNTIME_ASSERT M5` |
...
Under ASSERT, packed writes (`__hashtable__grow`), string-key writes (`lookupStringKeyForWriteBranch`), object-key writes (`__hashtable__setObjectKeyLong` / `setObjectKeyObject`, SplObjectStorage / WeakMap), unsets (`__hashtable__unsetLongAt` / `unsetStringKey` / `unsetObjectKey`), packed sorts (`__hashtable__sortPacked*`), and by-ref stdlib mutators (`array_push` / `array_unshift` / `array_pop` / `array_shift` / `array_splice` / `sort` / `rsort` / `asort` / `arsort` / `ksort` / `krsort` / `natsort` / `usort` / `array_walk` / `array_multisort` / `shuffle` / `next` / `prev` / `reset` / `end` — `HashTableHelper::separateContainerForWrite` then pop/shift/splice/sort/shuffle/pointer LLVM which also call `__ref__assert_exclusive`; `array_multisort` asserts at the MultisortRuntime call site before `packVariables` addrefs) call `__ref__assert_exclusive` on the container before mutating — so a skipped `separate()` on a shared array aborts with **M5** (php-src `SEPARATE_ARRAY` in [Zend/zend_variables.h](https://github.com/php/php-src/blob/master/Zend/zend_variables.h); `array_push` / `array_splice` / `php_array_sort` / `php_array_walk` / `php_array_multisort` / `php_shuffle` / `next`/`prev`/`reset`/`end` (`a/`) in [ext/standard/array.c](https://github.com/php/php-src/blob/master/ext/standard/array.c)). Exclusive containers (`rc <= 1`) and unclaimed `rc=0` headers pass.
