...
Under ASSERT, packed writes (`__hashtable__grow`), string-key writes (`lookupStringKeyForWriteBranch`), object-key writes (`__hashtable__setObjectKeyLong` / `setObjectKeyObject`, SplObjectStorage / WeakMap), unsets (`__hashtable__unsetLongAt` / `unsetStringKey` / `unsetObjectKey`), and by-ref stdlib mutators (`array_push` / `array_unshift` / `array_pop` / `array_shift` / `array_splice` — `HashTableHelper::separateContainerForWrite` then pop/shift/splice LLVM which also call `__ref__assert_exclusive`) call `__ref__assert_exclusive` on the container before mutating — so a skipped `separate()` on a shared array aborts with **M5** (php-src `SEPARATE_ARRAY` in [Zend/zend_variables.h](https://github.com/php/php-src/blob/master/Zend/zend_variables.h); `array_push` / `array_splice` in [ext/standard/array.c](https://github.com/php/php-src/blob/master/ext/standard/array.c)). Exclusive containers (`rc <= 1`) and unclaimed `rc=0` headers pass.
