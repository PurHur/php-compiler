...
3. **Make the suites runnable.** ~~Sharding takes `VMTest` from ~4 h to ~20 min (24 shards, per-test
   ...
   Measured: 8,812 cases, round-robin sharding at `--shards=24` gives 368/368/367; a shard runs
   ~320 s, so 24 shards is ~2.1 h serial on one box and ~20 min across parallel runners. Note the
   ~20 min figure requires **parallel runners** — on a single box the shards must run sequentially,
   because concurrent containers sharing the bind mount corrupt each other's caches.
