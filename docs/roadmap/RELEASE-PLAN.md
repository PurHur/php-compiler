...
3. **Make the suites runnable.** ~~Sharding takes `VMTest` from ~4 h to ~20 min (24 shards, per-test
   ...
   script rather than leaving it in a scratchpad.~~ **LANDED 2026-07-29** —
   `script/shard-compliance.sh` (#24445) and `script/compliance-baseline.sh` (#24469), documented in
   AGENTS.md §2.

   Measured: 8,812 cases; membership is `crc32(name) % SHARDS` (#24498 — hash-stable; index
   round-robin reshuffled the corpus on any add/remove). At `--shards=24` counts stay near
   368±a few; a shard runs ~320 s, so 24 shards is ~2.1 h serial on one box and ~20 min across
   parallel runners. Note the ~20 min figure requires **parallel runners** — on a single box the
   shards must run sequentially, because concurrent containers sharing the bind mount corrupt each
   other's caches.

   Runnable is not yet a gate: that needs the committed failing-name baseline, which is generated by
   `--collect` from a complete run and refuses to build from partial results.
4. **Quarantine flaky tests explicitly.** `hrtime_*` (#23420), `proc_get_status_basic`,
   ...
   **Mechanism landed 2026-07-29** (`test/compliance/quarantine.txt`, excluded from both sides of
   the diff), **list deliberately empty.** The four names above are *suspected*, not measured, and
   quarantining a case on reputation hides any real regression in it. Each needs two full runs of one
   commit showing it on one side and not the other. Populate it from evidence.
