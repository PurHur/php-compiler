...
| Closures `function () { }` / `use ($var)` | yes | yes | no | [#72](https://github.com/PurHur/php-compiler/issues/72) | VM via ClosureState + __invoke; use() by-value on VM (#3081); JIT ClosureHelper lowers TYPE_CLOSURE without use() (#3081); use (&$x) deferred; Bootstrap / self-host AOT stubs null; compliance PHPT |
| Arrow functions `fn () =>` | yes | yes | no | [#142](https://github.com/PurHur/php-compiler/issues/142) | Same lowering as closures (#72); JIT without use() (#3081); compliance PHPT |
