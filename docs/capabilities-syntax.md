...
| `try` / `catch` / `throw` | yes | yes | yes | [#57](https://github.com/PurHur/php-compiler/issues/57) | php-cfg TryCatch overlay (#2084); VM TYPE_TRY/CATCH/THROW; JIT object throw dispatch via `TryCatchHelper` + `JitThrow` (#1056); TryCatchComplianceTest + try_* PHPT; `TryCatchJITTest` (@group llvm); finally after catch still #57; bootstrap AOT |
| readonly classes | yes | yes | yes | [#1360](https://github.com/PurHur/php-compiler/issues/1360) | php-cfg Class_::flags MODIFIER_READONLY; VM/JIT reject instance property writes after __construct (`__object__.constructed` + pending LogicException) |
