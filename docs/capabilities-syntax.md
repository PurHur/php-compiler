...
| Generators (`yield` / `foreach`) | yes | yes | no | [#167](https://github.com/PurHur/php-compiler/issues/167) | VM GeneratorState + foreach; keyed yield (#3085); MCJIT resume lowering for nested generator funcs (#3074); script-scope requiresVmLowering; linear yield only (no yield from / try in generator JIT yet); see docs/generators-jit-aot.md; `yield from` generator path supported on VM (#167); compliance PHPT |
