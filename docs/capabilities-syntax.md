...
| Closures `function () { }` / `use ($var)` | yes | yes | no | [#72](https://github.com/PurHur/php-compiler/issues/72) | VM ClosureState + __invoke; use() by-value and by-ref (#3081, #3108); array_map/filter/usort callbacks (#3086); JIT ClosureHelper: TYPE_CLOSURE + use() value/ref IR (#3092, #3108); indirect $arr[0]() via __closure_target (#3089, #3092); bootstrap / self-host AOT stubs null; bin/jit.php MCJIT execute still probe-dependent (#98); compliance PHPT |
| Arrow functions `fn () =>` | yes | yes | no | [#142](https://github.com/PurHur/php-compiler/issues/142) | Desugars to TYPE_CLOSURE (#142); VM + JIT same as closures (#3092, #3108); compliance PHPT |
