...
| Generators (`yield` / `foreach`) | yes | no | no | [#167](https://github.com/PurHur/php-compiler/issues/167) | VM GeneratorState + foreach; keyed yield (#3085); bin/jit.php VM fallback via Block::requiresVmLowering (#3085); see docs/generators-jit-aot.md; `yield from` generator path supported on VM (#167); compliance PHPT; VM-only lowering |
...
| Return-by-reference (`function &f()` / `$x = &f()`) | yes | no | no | [#3414](https://github.com/PurHur/php-compiler/issues/3414) | VM propagates reference cells via FLAG_RETURNS_REF; JIT deferred (#3161); VM-only lowering |
