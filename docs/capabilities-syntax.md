...
| Class constants with `new` object expressions (PHP 8.3) | yes | no | yes | [#3196](https://github.com/PurHur/php-compiler/issues/3196) | Zend zend_compile_const_expr / zend_constants.c — materialize at class definition; shared identity on fetch; VM: ClassConstMaterializer + defineClass const-init opcodes; JIT compile lowers metadata (execute follow-up); VM-only lowering |
