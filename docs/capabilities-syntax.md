...
| Generators (`yield` / `foreach`) | yes | yes | no | [#167](https://github.com/PurHur/php-compiler/issues/167) | VM GeneratorState + foreach; keyed yield (#3085); MCJIT resume lowering for nested generator funcs (#3074); script-scope requiresVmLowering; JIT linear `yield` + packed-array `yield from`; `yield from` generator + try/catch in generator JIT deferred; see docs/generators-jit-aot.md; compliance PHPT |
...
| By-reference parameters (`function f(&$x)`) | yes | yes | yes | [#140](https://github.com/PurHur/php-compiler/issues/140) | VM TYPE_INDIRECT; JIT aliases caller __value__* via paramByRef (#3161) |
...
| Simple `use Trait;` in class body | yes | yes | yes | [#2314](https://github.com/PurHur/php-compiler/issues/2314) | php-cfg-trait-use.patch; VM merges trait methods into class; JIT/AOT recompile trait methods on using class (#3789); TraitUseAdaptation (alias/insteadof) is #144; Horizontal trait method collision fatals at compile time (#3416) |
...
| Property hooks (`get` / `set` on properties) | yes | yes | yes | [#3145](https://github.com/PurHur/php-compiler/issues/3145) | SourcePreprocessor lowers hooks to `__phpc_property_*` methods (#3145); VM dispatch; JIT PropertyHookDispatch (#3723); AOT set-hook smoke in property_hook_set.phpt; MCJIT execute segfaults when hook body calls str_contains |
...
| WeakReference / WeakMap | yes | no | yes | [#3282](https://github.com/PurHur/php-compiler/issues/3282) | VM: WeakReference::create/get via indirect target slot; unset clears get immediately; GC-backed weak refs via WeakRefRegistry — referent collected by gc_collect_cycles() clears get(); WeakMap uses object-id string keys; entries removed when key object is collected; JIT may compile references but method bodies are VM-only; VM-only lowering |
