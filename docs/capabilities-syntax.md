...
| Magic methods `__get` / `__set` / `__call` / `__toString` | yes | no | yes | [#146](https://github.com/PurHur/php-compiler/issues/146) | Zend zend_object_handlers.c: zend_std_read_property, zend_std_write_property, zend_std_get_method, __toString cast; VM slow path on undeclared property read/write and missing method call; __callStatic (#3273); JIT deopt/fallback; compliance PHPT; VM-only lowering |
...
| Generators (`yield` / `foreach`) | yes | yes | yes | [#167](https://github.com/PurHur/php-compiler/issues/167) | VM GeneratorState + foreach; keyed yield (#3085); MCJIT/AOT resume lowering for nested generator funcs (#3074, #3115, #4014); script-scope yield blocked; JIT linear `yield` + `yield from` array/generator; try/catch in generator JIT deferred; see docs/generators-jit-aot.md; AOT fixture generator_yield.phpt + generator_yield_keys.phpt; compliance PHPT; AOT PHPT |
| Fibers (`Fiber`, `Fiber::suspend()`, start/resume) | yes | no | no | [#3130](https://github.com/PurHur/php-compiler/issues/3130) | VM FiberState + builtin Fiber class (#3130); php-src Zend/zend_fibers.c; JIT/AOT out of scope v1 (mirror generators #167); VM-only lowering |
...
| foreach over Iterator / IteratorAggregate objects | yes | no | no | [#3234](https://github.com/PurHur/php-compiler/issues/3234) | VM calls rewind/valid/current/key/next (Zend zend_iterators.c parity); IteratorAggregate::getIterator(); TypeError for non-iterable objects; VM-only lowering |
| ArrayAccess interface — `$obj[$key]` read/write/isset/unset | yes | yes | yes | [#3331](https://github.com/PurHur/php-compiler/issues/3331) | VM dispatches offsetGet/Set/Exists/Unset (Zend zend_object_handlers.c read_dimension); JIT ArrayAccessHelper + RuntimeIndirectInstanceMethodCall (#4012); Non-ArrayAccess objects keep Illegal offset; compliance PHPT |
| By-reference parameters (`function f(&$x)`) | yes | yes | no | [#140](https://github.com/PurHur/php-compiler/issues/140) | VM TYPE_INDIRECT; JIT aliases caller __value__* via paramByRef (#3161); Return-by-ref `function &f()` + `$x = &f()` on VM (#3414) |
| Return-by-reference (`function &f()` / `$x = &f()`) | yes | yes | no | [#3778](https://github.com/PurHur/php-compiler/issues/3778) | VM + JIT propagate reference cells via FLAG_RETURNS_REF (#3414, #3778); AOT deferred |
...
| DNF types (`(A&B)|null`, union of intersections) | yes | yes | yes | [#3094](https://github.com/PurHur/php-compiler/issues/3094) | php-cfg Union + Intersection; TypeReconstructor TYPE_INTERSECTION; VM DnfCheck on param/property/return; JIT/AOT DnfParamCheck at call sites + returns; __value__* param ABI (#4008); typed properties MCJIT assign still VM-only; Parenthesized DNF only (php-parser 4.x); ref Zend/zend_compile.c |
...
| PHP 8.4 asymmetric property visibility (public private(set), etc.) | yes | no | yes | [#3165](https://github.com/PurHur/php-compiler/issues/3165) | Ast\AsymmetricVisibilityRewriter normalizes private(set) for php-parser 4.x; VM enforces set visibility (#3165); php-src: Zend/zend_compile.c ZEND_ACC_*_SET; JIT/AOT follow-up; VM-only lowering |
