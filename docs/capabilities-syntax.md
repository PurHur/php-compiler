...
| Trait use adaptations (`as` rename, `insteadof` precedence) | yes | no | no | [#3238](https://github.com/PurHur/php-compiler/issues/3238) | Zend/zend_compile.c trait alias/precedence; VM applyTraitUsesWithAdaptations; Visibility `as private` deferred to #144 |
...
| readonly properties (per-property) | yes | partial | yes | [#3149](https://github.com/PurHur/php-compiler/issues/3149) | php-cfg readonly / propertyFlags MODIFIER_READONLY; VM rejects writes after __construct; JIT ReadonlyClassGuard IR + ReadonlyPropertyTest; MCJIT execute raise path harness-deferred (JITTest skip); AOT pending abort via phpc_jit_abort_if_pending_logic_exception; php-src: Zend/zend_object_handlers.c zend_std_write_property |
...
| `#[\Override]` attribute (PHP 8.3) | yes | yes | yes | [#3211](https://github.com/PurHur/php-compiler/issues/3211) | Compile-time validation via `OverrideValidator` + `ClassCompileRegistry` (Zend `zend_compile_override_attribute` parity); invalid override → `CompileError`; compliance PHPT |
