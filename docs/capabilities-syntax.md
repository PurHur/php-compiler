...
| Property hooks (`get` / `set` on properties) | yes | yes | yes | [#3145](https://github.com/PurHur/php-compiler/issues/3145) | SourcePreprocessor lowers hooks to __phpc_property_* methods (#3145); VM dispatches set/get on property access; JIT PropertyHookDispatch (#3723); AOT: user-class hook methods lower under PHP_COMPILER_SELFHOST_AOT; set-hook smoke in property_hook_set.phpt; MCJIT execute still segfaults when hook body calls str_contains (follow-up) |
