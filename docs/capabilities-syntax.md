...
| Generators (`yield` / `foreach`) | yes | yes | yes | [#167](https://github.com/PurHur/php-compiler/issues/167) | VM GeneratorState + foreach; keyed yield (#3085); MCJIT/AOT resume lowering for nested generator funcs (#3074, #3115); script-scope yield blocked; JIT linear `yield` + packed-array `yield from`; `yield from` generator + try/catch in generator JIT deferred; see docs/generators-jit-aot.md; AOT fixture generator_yield.phpt + generator_yield_keys.phpt; compliance PHPT; AOT PHPT |
...
| DNF types (`(A&B)|null`, union of intersections) | yes | no | no | [#3094](https://github.com/PurHur/php-compiler/issues/3094) | php-cfg Union + Intersection; TypeReconstructor TYPE_INTERSECTION; VM DnfCheck on param/property/return; Parenthesized DNF only (php-parser 4.x); ref Zend/zend_compile.c; VM-only lowering |
