...
| Simple `use Trait;` in class body | yes | yes | yes | [#2314](https://github.com/PurHur/php-compiler/issues/2314) | php-cfg-trait-use.patch; VM merges trait methods into class; JIT/AOT alias trait-merged methods onto using class (#3789); TraitUseAdaptation alias/insteadof on VM (#3238); visibility `as` deferred #144; Horizontal trait method collision fatals at compile time (#3416) |
| Trait use adaptations (`as` rename, `insteadof` precedence) | yes | yes | yes | [#3238](https://github.com/PurHur/php-compiler/issues/3238) | Zend/zend_compile.c trait alias/precedence; VM applyTraitUsesWithAdaptations; JIT/AOT batch trait use with insteadof/as (#2483, #3238); Visibility `as private|protected` on trait alias (#144) |
