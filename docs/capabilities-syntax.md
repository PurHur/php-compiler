...
| Enum declarations `enum Foo: string { case Bar = 'x'; }` | yes | yes | no | [#1356](https://github.com/PurHur/php-compiler/issues/1356) | Backed enum case objects with `->name` / `->value`; string context coerces to backed scalar (#3518); `Foo::Bar` singleton fetch; `enum_exists` registry; `implements` interface list + instance methods + `instanceof` (#3373); static methods (#2299); `Enum::cases()` VM (#3308) |
| Abstract enum `abstract enum E { case A; }` | yes | yes | no | [#3737](https://github.com/PurHur/php-compiler/issues/3737) | Source rewriter + php-cfg flags; `new E()` fatals; case fetch works on VM |
...
| readonly classes | yes | yes | no | [#1360](https://github.com/PurHur/php-compiler/issues/1360) | php-cfg Class_::flags MODIFIER_READONLY; VM rejects instance property writes after __construct |
