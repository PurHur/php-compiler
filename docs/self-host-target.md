...
| **Full inventory** | Honest bundle covers the `bin/vm.php` path (~**593** files; [`bootstrap-inventory.md`](bootstrap-inventory.md)) |
...
| **Bundle size** | **589** curated `require_once` in spine smoke (108 minimal overlap + **476** M2-only) | **593** inventory files |
| **Inventory coverage** | **~99%** of vm.php path in spine smoke (**589/593**) | **100%*
...
| `php script/bootstrap-inventory.php --check` | ✅ **593** files, **0** source blockers |
...
| **M2** | Spine grows toward full `bin/vm.php` inventory | 🚧 **589/593** units | **~99%*
...
|-----------|---------|----------|
| **M2 spine progress** | `require_once` units in `compiler_lib_spine_smoke` ÷ Phase A inventory file count | **589 / 593 ≈ 99%*
...
| `test/selfhost/compiler_lib_spine_smoke/main.php` | **589** | M2 growth (ext/standard + Vm* spine batch
