...
| **Bundle size** | **1297** curated `require_once` in spine smoke (`cli_spine_shim` for `src/cli.php` only) | **1626** Phase A inventory files (`php script/bootstrap-spine-count.php`) |
| **Inventory coverage** | **1297** / **1626** literal `require_once` with real **`bin/vm.php`** ([#2134](https://github.com/PurHur/php-compiler/issues/2134); union-type overlays [#5096](https://github.com/PurHur/php-compiler/pull/5096)) | **~95%** |
...
| M4 `make bootstrap-loop-gen1-full-spine-emit` | 🚧 gen-1→gen-2 **1297/1626** spine (`bootstrap-loop-gen2-full-spine`) — heavy |
| M4 `make bootstrap-loop-gen2-recompile-spine` | 🚧 gen-2→gen-3 **1297/1626** — native driver when wired; Zend fallback when blocked ([#2697](https://github.com/PurHur/php-compiler/pull/2697)) |
...
| `make bootstrap-inventory-check` | ✅ **1626** Phase A files (live collector), **0** source blockers; regenerate `bootstrap-inventory.md` when SSOT drifts |
...
| **M2** | Spine grows toward full `bin/vm.php` inventory | 🚧 **1297/1626** literal `require_once` ([#2868](https://github.com/PurHur/php-compiler/issues/2868)) | **~95%** |
...
|-----------|---------|----------|
| **M2 spine progress** | `require_once` units in `compiler_lib_spine_smoke` ÷ Phase A inventory file count | **1297 / 1626** (`php script/bootstrap-spine-count.php`) |
...
| `test/selfhost/compiler_lib_spine_smoke/main.php` | **1297** literal | M2 in progress ([#2868](https://github.com/PurHur/php-compiler/issues/2868)) |
