...
| **Full inventory** | Honest bundle covers the `bin/vm.php` path (~**605** files; [`bootstrap-inventory.md`](bootstrap-inventory.md)) |
...
| **Bundle size** | **600** curated `require_once` in spine smoke (108 minimal overlap + **490** M2-only) | **605** inventory files |
| **Inventory coverage** | **~99%** of vm.php path in spine smoke (**600/605**) | **100%*
...
| `php script/bootstrap-inventory.php --check` | ✅ **605** files, **0** source blockers |
...
| **M2** | Spine grows toward full `bin/vm.php` inventory | 🚧 **600/605** units | **~99%*
...
|-----------|---------|----------|
| **M2 spine progress** | `require_once` units in `compiler_lib_spine_smoke` ÷ Phase A inventory file count | **600 / 605 ≈ 99%*
...
| `test/selfhost/compiler_lib_spine_smoke/main.php` | **600** | M2 growth (ext/standard + Vm* spine batch
