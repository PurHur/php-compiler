...
| `lib/JIT/Context.php` + `ContextEditScaffoldModuleRebind.php` + `ContextDefineBuiltins.php` | JIT Context hub + edit-scaffold/bitcode module rebind + defineBuiltins proxies trait | `./script/phpunit.sh --filter AotCompileCacheTest`; aot-smoke | module.bc thin-boot SIGSEGV if seed/rebind stays on hub (#36387) |
