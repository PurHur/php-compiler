...
| `lib/JIT/NoThrowCallElision.php` + `NoThrowCallElisionRuntimeInfoPredicates.php` + `NoThrowCallElisionExistsConvertAndIntrospectOps.php` + `NoThrowCallElisionStringOps.php` | Skip exception-stack / throw-pending for proven no-throw callees | `./script/aot-smoke.sh`; differential | Wrong proof = missing frames on uncaught traces (#36386/#36403) |
