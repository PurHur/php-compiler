...
| `JIT_SERVER_SUPERGLOBAL_GATE` | `0` | `ci-local.sh` (LLVM tail) | PHPUnit `JitServerSuperglobal` — `bin/jit.php` `$_SERVER` / `PATH_INFO` refresh without recompile ([#2257](https://github.com/PurHur/php-compiler/issues/2257), [#2275](https://github.com/PurHur/php-compiler/issues/2275)); skipped when LLVM missing or MCJIT probe fails; set `1` to opt in |
