...
| `BOOTSTRAP_VM_DRIVER_EXECUTE_GATE` | `1` | `ci-local.sh` (LLVM tail, after lib-spine-vm-smoke) | `bootstrap-selfhost-vm-driver-execute-probe.sh` — M2 spine binary + `PHP_COMPILER_VM_DRIVER_EXECUTE=1` dispatches `bin/vm.php` `run()` ([#2201](https://github.com/PurHur/php-compiler/issues/2201), default-on [#2227](https://github.com/PurHur/php-compiler/issues/2227)); set `0` to skip during spine-only PRs |
