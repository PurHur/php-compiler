...
| `CAPABILITIES_THROWS_SYNC_GATE` | `0` | `ci-fast.sh` (`ci_run_inventory_checks`) | `script/check-capabilities-throws-sync.php` — `try` / `catch` / `throw` + **007-ThrowsWeb** rows in capability docs ([#2144](https://github.com/PurHur/php-compiler/issues/2144), [#2103](https://github.com/PurHur/php-compiler/issues/2103)); set `1` after matrix lands; flip default-on in follow-up |
