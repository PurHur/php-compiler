...
| `DOCS_HARNESS_HYGIENE_GATE` | `1` | `ci-fast.sh` (`ci_run_inventory_checks`) | `script/check-docs-harness-hygiene.php` — guard `README.md` / `docs/` against raw `docker run -v "$(pwd):/compiler"` recommended as positive commands ([#2485](https://github.com/PurHur/php-compiler/issues/2485)); set `0` to skip during doc-only iteration |
