...
| `BOOTSTRAP_INVENTORY_LINT_SYNC_GATE` | `0` | `ci-fast.sh` (`ci_run_inventory_checks`) | `script/check-bootstrap-inventory-lint-sync.php` — `phpc lint --bootstrap-inventory` vs `docs/bootstrap-inventory-lint-snapshot.json` ([#2210](https://github.com/PurHur/php-compiler/issues/2210)); set `1` when editing inventory lint report; regenerate with `php script/bootstrap-inventory-lint-snapshot.php --write` |
