...
| `SERVE_JIT_SMOKE_GATE` | `0` | `ci-common.sh` (`ci_run_examples_serve_jit_smoke`) | `examples-serve-jit-smoke.sh` — `phpc serve --jit` curls on 001 (+ 003 when `lint --all` green, + 007 caught POST when `lint` green — [#2274](https://github.com/PurHur/php-compiler/issues/2274), [#2478](https://github.com/PurHur/php-compiler/issues/2478), [#2408](https://github.com/PurHur/php-compiler/issues/2408)); respects `THROWSWEB_SERVE_JIT_SMOKE_GATE`; follow-on [#1900](https://github.com/PurHur/php-compiler/issues/1900) ci-local default |
