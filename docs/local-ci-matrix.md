...
| `MINIWEBAPP_JIT_PROJECT_GATE` | `0` | `ci-local.sh` (`ci_run_miniwebapp_jit_project`) | PHPUnit `@group miniwebapp-jit-project` / `MiniWebAppJitProjectTest` — `bin/jit.php` project entry ([#587](https://github.com/PurHur/php-compiler/issues/587), [#2183](https://github.com/PurHur/php-compiler/issues/2183)); set `1` when LLVM + JIT probe green |
| `SERVE_JIT_SMOKE_GATE` | `0` | `ci-common.sh` (`ci_run_examples_serve_jit_smoke`) | `examples-serve-jit-smoke.sh` — `phpc serve --jit` two-request curls on 001 (+ 003 when lint green) ([#2274](https://github.com/PurHur/php-compiler/issues/2274)); follow-on [#1900](https://github.com/PurHur/php-compiler/issues/1900) ci-local default |
...
| `THROWSWEB_SERVE_JIT_SMOKE_GATE` | `0` | `ci-fast.sh` (`ci_run_throws_web_serve_jit_smoke`) | `examples-web-smoke.sh --throws-only --jit` — 007 `phpc serve --jit` caught invalid POST ([#2408](https://github.com/PurHur/php-compiler/issues/2408)) |
...
| `BOOTSTRAP_M4_LOOP_PROBE` | `0` | `ci-local.sh` (LLVM tail, after M3 strict gates) | Same dry-run probe as `BOOTSTRAP_LOOP_PROBE_GATE`; ci-local placement after M3 HelloWorld/compile-smoke strict ([#2058](https://github.com/PurHur/php-compiler/issues/2058)) |
...
| `BOOTSTRAP_M3_COMPILE_SMOKE_STRICT_GATE` | `0` | `ci-local.sh` (LLVM tail, after compile-smoke probe) | same probe with `BOOTSTRAP_M3_COMPILE_SMOKE_STRICT=1` — no Zend fallback ([#1937](https://github.com/PurHur/php-compiler/issues/1937)); set `1` when `emit_path=native` stable ([#2165](https://github.com/PurHur/php-compiler/issues/2165)) |
...
| `BOOTSTRAP_INVENTORY_TRIAGE_SYNC_GATE` | `0` | `ci-fast.sh` (`ci_run_inventory_checks`) | `script/check-bootstrap-inventory-triage-sync.php` — `bootstrap-inventory-triage.php --json --top 50` vs `docs/bootstrap-inventory-triage-top50.json` ([#2265](https://github.com/PurHur/php-compiler/issues/2265)); set `1` when editing triage output; regenerate with `php script/bootstrap-inventory-triage.php --json --top 50 > docs/bootstrap-inventory-triage-top50.json` |
| `STDLIB_JIT_DEFERRED_SYNC_GATE` | `0` | `ci-fast.sh` (`ci_run_inventory_checks`) | `script/check-stdlib-jit-deferred-sync.php` — `docs/stdlib-jit-audit.md` + `docs/capabilities.md` vs `script/stdlib-jit-deferred-lib.php` open deferrals ([#2465](https://github.com/PurHur/php-compiler/issues/2465)); probe: `./phpc doctor --gates \| grep -i stdlib_jit`; set `1` when editing deferral allowlist or audit |
...
Progressive ladder (VM throw/catch → AOT link → AOT execute → deploy CGI). VM smoke default-on ([#2125](https://github.com/PurHur/php-compiler/issues/2125), [#2093](https://github.com/PurHur/php-compiler/issues/2093)); AOT gates default-on ([#2135](https://github.com/PurHur/php-compiler/issues/2135), [#2157](https://github.com/PurHur/php-compiler/issues/2157)). Copy-paste ladder: `./phpc doctor --gates` (**#2102**).
...
| JIT serve | `THROWSWEB_SERVE_JIT_SMOKE_GATE` | `0` | `THROWSWEB_SERVE_JIT_SMOKE_GATE=1 ./script/examples-web-smoke.sh --throws-only --jit` · `ci-fast.sh` ([#2408](https://github.com/PurHur/php-compiler/issues/2408)) |
