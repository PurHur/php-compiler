...
| **JIT (`bin/jit.php`)** | MCJIT resume (#3074) | Main script MCJIT when yield only in nested functions; `GeneratorHelper` switch-on-resume-ip; linear `yield`, packed-array `yield from`, nested `yield from inner()`, dynamic `yield from $g` |
...
4. ✅ Switch-on-IP lowering for generators with packed-array `yield from`, nested generator delegation, and dynamic `yield from $g` (#3074); try/catch inside generators still deferred
