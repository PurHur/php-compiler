...
| Compiler unit probe native emit | `make bootstrap-selfhost-compiler-unit-probe-strict` or `BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT=1` + compile-driver env | ✅ `emit_path=native` compiles `compiler_unit_probe_compile.php` via dedicated M3 emit helper ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` via `compiler_unit_probe/compile_driver.php` ([#2879](https://github.com/PurHur/php-compiler/issues/2879)); partial Zend emit when `BOOTSTRAP_M3_LINK_COMPILE_DRIVER=1` without strict |
...
| JIT unit probe native emit | `make bootstrap-selfhost-jit-unit-probe-strict` or `BOOTSTRAP_M3_JIT_UNIT_PROBE_STRICT=1` + compile-driver env | ✅ `emit_path=native` compiles `jit_unit_probe_compile.php` via dedicated M3 emit helper ([#2778](https://github.com/PurHur/php-compiler/issues/2778)); partial Zend emit when `BOOTSTRAP_M3_LINK_COMPILE_DRIVER=1` without strict |
...
| M2 lib spine smoke lint | `php bin/compile.php -l test/selfhost/compiler_lib_spine_smoke/main.php` | ✅ **725/725** literal units ([#2868](https://github.com/PurHur/php-compiler/issues/2868)) |
| M2 spine coverage drift | `php script/check-selfhost-spine-coverage-sync.php` | ✅ **725** inventory files covered (no ratio-deferred); `SELFHOST_SPINE_COVERAGE_SYNC_GATE=1` ([#1945](https://github.com/PurHur/php-compiler/issues/1945), [#2840](https://github.com/PurHur/php-compiler/issues/2840))
...
| M3 HelloWorld self-host probe | `./script/bootstrap-selfhost-helloworld-probe.sh` or `make bootstrap-selfhost-helloworld` | ✅ strict default — `emit_path=native` HelloWorld + `bin/compile.php` via emit-helper sidecars ([#1768](https://github.com/PurHur/php-compiler/issues/1768), [#2822](https://github.com/PurHur/php-compiler/pull/2822), [#2827](https://github.com/PurHur/php-compiler/issues/2827), [#2697](https://github.com/PurHur/php-compiler/issues/2697)); opt-in inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` — `emit_path=native` via `compile_driver.php` only (no `*_m3_emit_native_entry.php`, [#2843](https://github.com/PurHur/php-compiler/issues/2843)); see [bootstrap-m5-fast-path.md](bootstrap-m5-fast-path.md) |
...
| M3 compile-smoke self-host probe | `./script/bootstrap-selfhost-compile-smoke-probe.sh` | ✅ strict — `emit_path=native` via `compile_smoke_m3_emit_native_entry.php` ([#2610](https://github.com/PurHur/php-compiler/issues/2610), [#2827](https://github.com/PurHur/php-compiler/issues/2827)) |
...
| M3 compile-smoke strict probe | `BOOTSTRAP_M3_COMPILE_SMOKE_STRICT=1 ./script/bootstrap-selfhost-compile-smoke-probe.sh` or `make bootstrap-selfhost-compile-smoke-strict` | ✅ `emit_path=native` — strict auto-enables compile-driver link env ([#2610](https://github.com/PurHur/php-compiler/issues/2610); refuses `emit_path=zend_fallback_would_be_used`); inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` via `compiler_compile_smoke/compile_driver.php` ([#2879](https://github.com/PurHur/php-compiler/issues/2879)) |
...
| M3 Runtime compile smoke probe | `./script/bootstrap-selfhost-runtime-compile-smoke.sh` or `make bootstrap-selfhost-runtime-compile-smoke` | ✅ strict — `emit_path=native` via `runtime_m3_emit_native_entry.php` ([#2610](https://github.com/PurHur/php-compiler/issues/2610), [#2827](https://github.com/PurHur/php-compiler/issues/2827)) |
...
| M3 Runtime compile smoke strict probe | `BOOTSTRAP_M3_RUNTIME_COMPILE_SMOKE_STRICT=1 ./script/bootstrap-selfhost-runtime-compile-smoke.sh` or `make bootstrap-selfhost-runtime-compile-smoke-strict` | ✅ `emit_path=native` — strict auto-enables compile-driver link env ([#2610](https://github.com/PurHur/php-compiler/issues/2610); refuses `emit_path=zend_fallback_would_be_used`); inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` via `runtime_compile_smoke/compile_driver.php` ([#2879](https://github.com/PurHur/php-compiler/issues/2879)) |
...
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | ✅ **ladder** — M2 + **M3 HelloWorld strict** (same env as `make bootstrap-selfhost-helloworld`, #2612) + gen-1→gen-2 + gen-2→gen-3 spine; `--dry-run` skips gen-3 ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2611](https://github.com/PurHur/php-compiler/issues/2611)) |
| M4 full-spine loop probe | `make bootstrap-loop-full-spine-probe` or `BOOTSTRAP_M4_GEN1_COMPILE_FULL_SPINE=1 make bootstrap-loop-probe` | ✅ inventory-scale gen-1 emit (`compiler_lib_spine_smoke`, **725** units) + gen-2 smoke ([#2770](https://github.com/PurHur/php-compiler/issues/2770), [#2664](https://github.com/PurHur/php-compiler/issues/2664)); heavy — opt-in `BOOTSTRAP_M4_FULL_SPINE_PROBE_GATE=1` in `ci-local.sh` llvm tail |
