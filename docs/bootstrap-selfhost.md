...
| Phase B lib AOT lint | `php bin/compile.php -l lib/*.php` (with `script/php-env.sh`) | ✅ **7410/7412** top-level `lib/*.php` units ([#534](https://github.com/PurHur/php-compiler/pull/534)) |
...
| Phase C native run | `make bootstrap-aot-link` or `./script/bootstrap-aot-link.sh` | ✅ **7410/7412** link targets OK |
...
| M2 spine coverage drift | `php script/check-selfhost-spine-coverage-sync.php` | ✅ **3622** Phase A inventory files covered; `SELFHOST_SPINE_COVERAGE_SYNC_GATE=1` ([#1945](https://github.com/PurHur/php-compiler/issues/1945), [#2840](https://github.com/PurHur/php-compiler/issues/2840))
...
| M4 gen-1 full-spine emit | `./script/bootstrap-loop-gen1-full-spine-emit.sh` or `make bootstrap-loop-gen1-full-spine-emit` | ✅ inventory-scale gen-1 emit (**3622** Phase A files) — heavy opt-in; may need Docker + patches |
...
| M3/M4 **genuine native emit** gate | `BOOTSTRAP_M3_REQUIRE_NATIVE_EMIT=1` / `BOOTSTRAP_M4_REQUIRE_NATIVE_EMIT=1` | Refuse a **prelinked sidecar blob COPY** standing in for a native emit ([#21860](https://github.com/PurHur/php-compiler/issues/21860)). **M3 helloworld:** `REQUIRE_NATIVE_EMIT=1` is green with `emit_path=native` via gen-0 argv helper ([#22178](https://github.com/PurHur/php-compiler/issues/22178)). Default-off so existing gates keep running; **default-on under `north-star4-verify --strict`**. Without these, `emit_path=native-prelinked-sidecar` is accepted and the ladder still reports OK — and byte-comparing gen-0/gen-2/gen-3 then proves nothing, because they are the same blob. |
...
**Zend gen-0 fallback memory floor** ([#31714](https://github.com/PurHur/php-compiler/issues/31714)): when committed gen-0 is lowering-stale and the link gate falls back to `php bin/compile.php`, default Docker `PHP_COMPILER_DOCKER_MEM=10g` SIGKILLs (~137) mid-compile. The link/invoke path **fails fast** under cgroup `< 16GiB` with a `NEXT_LOWER_CMD`; remount:
...
| `bin/vm.php` | Real `require_once` in `compiler_lib_spine_smoke` (#2134); literal `src/cli_driver.php` + `cli_spine_shim` for `src/cli.php` (#2910) |
