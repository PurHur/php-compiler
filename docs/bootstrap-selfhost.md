...
| VM unit probe native link | `./script/bootstrap-selfhost-vm-unit-probe.sh` or `make bootstrap-selfhost-vm-unit-probe` | ✅ `build/selfhost-vm-unit-probe` prints `vm_unit_probe bundle OK`; optional execute via `make bootstrap-selfhost-vm-unit-probe-execute` / `BOOTSTRAP_VM_UNIT_PROBE_EXECUTE=1` → `vm_unit_probe_run OK` ([#2354](https://github.com/PurHur/php-compiler/issues/2354), [#2619](https://github.com/PurHur/php-compiler/issues/2619)); `BOOTSTRAP_VM_UNIT_PROBE_GATE=1` opt-in in `ci-local.sh` llvm tail ([#2368](https://github.com/PurHur/php-compiler/issues/2368)) |
