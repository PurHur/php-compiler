...
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | ✅ M2 + **M3 HelloWorld strict** + gen-1→gen-2 compile-smoke slice (`emit_path=native` when LLVM present — [#2611](https://github.com/PurHur/php-compiler/issues/2611)) |
...
| M4/M5 gen-2 spine recompile | `make bootstrap-loop-gen2-recompile-minimal` or `./script/bootstrap-loop-gen2-recompile-minimal.sh` | ✅ native `build/bin-compile-aot` compiles 718/718 `compiler_lib_spine_smoke` → `build/bootstrap-loop-gen3-full-spine` ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2697](https://github.com/PurHur/php-compiler/issues/2697)) |
