...
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | ✅ M2 + **M3 HelloWorld strict** (same env as `make bootstrap-selfhost-helloworld`, #2612) + gen-1→gen-2 `emit_path=native` ([#1498](https://github.com/PurHur/php-compiler/issues/1498)) |
| M4 full-spine gen-1 emit | `make bootstrap-loop-full-spine-probe` or `./script/bootstrap-loop-probe.sh --with-full-spine` | ✅ inventory-scale `compiler_lib_spine_smoke` gen-1→gen-2 native emit ([#2664](https://github.com/PurHur/php-compiler/issues/2664)); heavy — opt-in CI only |
| M5 wave-check driver smoke | `NATIVE_COMPILE_DRIVER_GATE=1 ./script/bootstrap-wave-check.sh` or `--with-driver-smoke` | ✅ gen-1 compiled driver compiles/runs `compiler smoke` without Zend on emit ([#1521](https://github.com/PurHur/php-compiler/issues/1521)) |
