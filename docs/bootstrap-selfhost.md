...
| Compiler unit probe native link | `./script/bootstrap-selfhost-compiler-unit-probe.sh` or `make bootstrap-selfhost-compiler-unit-probe` | ✅ bundle link + optional Zend emit/run; native emit when `BOOTSTRAP_M3_LINK_COMPILE_DRIVER=1` ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); `BOOTSTRAP_COMPILER_UNIT_PROBE_GATE=1` default-on in `ci-local.sh` llvm tail ([#2221](https://github.com/PurHur/php-compiler/issues/2221)); set `0` to skip |
| Compiler unit probe strict | `BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT=1 ./script/bootstrap-selfhost-compiler-unit-probe.sh` or `make bootstrap-selfhost-compiler-unit-probe-strict` | ✅ `emit_path=native` — requires compile-driver link env ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); refuses `emit_path=zend_fallback_would_be_used` |
