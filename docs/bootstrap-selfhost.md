...
| M4 full-spine loop probe | `make bootstrap-loop-full-spine-probe` or `BOOTSTRAP_M4_GEN1_COMPILE_FULL_SPINE=1 make bootstrap-loop-probe` | ✅ inventory-scale gen-1 emit (`compiler_lib_spine_smoke`, 717 units) + gen-2 smoke ([#2770](https://github.com/PurHur/php-compiler/issues/2770), [#2664](https://github.com/PurHur/php-compiler/issues/2664)); heavy — opt-in `BOOTSTRAP_M4_FULL_SPINE_PROBE_GATE=1` in `ci-local.sh` llvm tail |
| M4/M5 gen-2 spine recompile | `make bootstrap-loop-gen2-recompile-minimal` or `./script/bootstrap-loop-gen2-recompile-minimal.sh` | ✅ native `build/bin-compile-aot` compiles 717/717 `compiler_lib_spine_smoke` → `build/bootstrap-loop-gen3-full-spine` ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2697](https://github.com/PurHur/php-compiler/issues/2697)) |
