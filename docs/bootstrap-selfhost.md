...
| Compiler unit probe native link | `./script/bootstrap-selfhost-compiler-unit-probe.sh` or `make bootstrap-selfhost-compiler-unit-probe` | ✅ `build/selfhost-compiler-unit-probe` prints `compiler_unit_probe bundle OK` (link-only; native compile-driver emit deferred — **#2466**); `BOOTSTRAP_COMPILER_UNIT_PROBE_GATE=1` default-on in `ci-local.sh` llvm tail ([#2221](https://github.com/PurHur/php-compiler/issues/2221)); set `0` to skip |
