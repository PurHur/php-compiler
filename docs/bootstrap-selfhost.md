...
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | ✅ M2 + **M3 HelloWorld strict** + gen-1/gen-2 slice — full mode exit **0** when gen-2 native emit green ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2680](https://github.com/PurHur/php-compiler/issues/2680)) |
| M4 full-spine gen-1 emit probe | `make bootstrap-loop-full-spine-probe` or `BOOTSTRAP_M4_GEN1_COMPILE_FULL_SPINE=1 make bootstrap-loop-probe` | ✅ inventory-scale gen-1 emit (`compiler_lib_spine_smoke`, `emit_path=native`) — opt-in CI `BOOTSTRAP_M4_FULL_SPINE_PROBE_GATE=1` ([#2770](https://github.com/PurHur/php-compiler/issues/2770), [#2664](https://github.com/PurHur/php-compiler/issues/2664)) |
