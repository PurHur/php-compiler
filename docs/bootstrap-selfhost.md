...
**Project north star:** The **compiler fully compiles itself** — native AOT from `lib/` (no `vendor/` at cold boot), then compiles PHP and rebuilds the next compiler revision without Zend. **M2 spine:** **1298** / **1681** literal `require_once` (**1298/1681**) with real **`bin/vm.php`** (`src/cli_driver.php` literal; `src/cli.php` via `cli_spine_shim`; `lib/VM/HashTable.php` promoted — [#2543](https://github.com/PurHur/php-compiler/issues/2543)) ([#2134](https://github.com/PurHur/php-compiler/issues/2134); `php script/bootstrap-spine-count.php`). **M4:** gen-1 link + revision probe partial; **gen-2→gen-3 full-spine recompile** still 🚧 — native inventory driver when ready, **`emit_path=zend partial`** Zend fallback when blocked. **Living tracker:** [#1492](https://github.com/PurHur/php-compiler/issues/1492) (was [#1056](https://github.com/PurHur/php-compiler/issues/1056)) · **re-root doc:** [self-host-target.md](self-host-target.md) · **generation ladder:** [bootstrap-generations.md](bootstrap-generations.md) · public status: [development-status § North star](https://purhur.github.io/php-compiler/development-status.html#north-star-self-host). Parent tracking: [#78](https://github.com/PurHur/php-compiler/issues/78) (roadmap), [#212](https://github.com/PurHur/php-compiler/issues/212) (closed umbrella).
...
| M2 lib spine smoke lint | `php bin/compile.php -l test/selfhost/compiler_lib_spine_smoke/main.php` or `make bootstrap-selfhost-lib-spine-smoke-lint` | ✅ literal includes via `compileIncludeOp` (no SourceBundler mega-bundle, [#8391](https://github.com/PurHur/php-compiler/issues/8391)) |
| M2 spine coverage drift | `php script/check-selfhost-spine-coverage-sync.php` | ✅ **1681** Phase A inventory files covered (no ratio-deferred); `SELFHOST_SPINE_COVERAGE_SYNC_GATE=1` ([#1945](https://github.com/PurHur/php-compiler/issues/1945), [#2840](https://github.com/PurHur/php-compiler/issues/2840))
...
| M4 gen-1 full-spine emit | `./script/bootstrap-loop-gen1-full-spine-emit.sh` or `make bootstrap-loop-gen1-full-spine-emit` | 🚧 gen-1→gen-2 **1298/1681** spine (`build/bootstrap-loop-gen2-full-spine`) — heavy; may need Docker + patches |
...
| M4 gen-2 strict emit gate | `BOOTSTRAP_M4_GEN2_STRICT=1 ./script/bootstrap-loop-gen1-link.sh` | manual/opt-in probe ([#2115](https://github.com/PurHur/php-compiler/issues/2115)); refuses Zend fallback (`emit_path=zend_fallback_would_be_used`); auto-enables compile-driver link env |
