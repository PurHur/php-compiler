...
| Phase B lib AOT lint | `php bin/compile.php -l lib/*.php` (with `script/php-env.sh`) | ✅ **6344/6344** top-level `lib/*.php` units ([#534](https://github.com/PurHur/php-compiler/pull/534)) |
...
| Phase C native run | `make bootstrap-aot-link` or `./script/bootstrap-aot-link.sh` | ✅ **6344/6344** link targets OK |
...
| M3/M4 **genuine native emit** gate | `BOOTSTRAP_M3_REQUIRE_NATIVE_EMIT=1` / `BOOTSTRAP_M4_REQUIRE_NATIVE_EMIT=1` | Refuse a **prelinked sidecar blob COPY** standing in for a native emit ([#21860](https://github.com/PurHur/php-compiler/issues/21860)). Default-off so existing gates keep running; **default-on under `north-star4-verify --strict`**. Without these, `emit_path=native-prelinked-sidecar` is accepted and the ladder still reports OK — and byte-comparing gen-0/gen-2/gen-3 then proves nothing, because they are the same blob. |
