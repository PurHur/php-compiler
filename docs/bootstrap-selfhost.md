...
| Compiler unit probe native emit | `make bootstrap-selfhost-compiler-unit-probe-strict` or `BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT=1` + compile-driver env | ✅ `emit_path=native` compiles `compiler_unit_probe_compile.php` via dedicated M3 emit helper ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); opt-in inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` — `compile_driver.php` only ([#2879](https://github.com/PurHur/php-compiler/issues/2879)); partial Zend emit when `BOOTSTRAP_M3_LINK_COMPILE_DRIVER=1` without strict |
...
| M3 Runtime compile smoke probe | `./script/bootstrap-selfhost-runtime-compile-smoke.sh` or `make bootstrap-selfhost-runtime-compile-smoke` | ✅ strict — `emit_path=native` via `runtime_m3_emit_native_entry.php` ([#2610](https://github.com/PurHur/php-compiler/issues/2610), [#2827](https://github.com/PurHur/php-compiler/issues/2827)); opt-in inventory driver: `BOOTSTRAP_M3_USE_INVENTORY_EMIT_DRIVER=1` ([#2879](https://github.com/PurHur/php-compiler/issues/2879)) |
...
| M4 bootstrap-loop gen-1 link | `./script/bootstrap-loop-gen1-link.sh` or `make bootstrap-loop-gen1-link` | ✅ gen-1 link + gen-2 native emit (`emit_path=native`) via inventory `bootstrap_loop_smoke/compile_driver.php` by default ([#2893](https://github.com/PurHur/php-compiler/issues/2893)); **Zend fallback** (`emit_path=zend partial`) when native blocked; `BOOTSTRAP_M3_EMIT_HELPER_TU=1` bisects emit-helper TU; set `BOOTSTRAP_M4_LINK_COMPILE_DRIVER=0` to bisect compile-driver |
...
| M4 gen-2→gen-3 bin/compile.php revision | `./script/bootstrap-selfhost-full-revision-probe.sh` or `make bootstrap-selfhost-full-revision-probe` | ✅ gen-2 argv-compiles `bin/compile.php` → gen-3 driver + native smoke compile/run ([#2880](https://github.com/PurHur/php-compiler/issues/2880)); alias `make bootstrap-loop-gen2-recompile-bin-compile` |
...
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | ✅ **ladder** — M2 + **M3 HelloWorld strict** (same env as `make bootstrap-selfhost-helloworld`, #2612) + gen-1→gen-2 + gen-2→gen-3 spine + **full-revision argv probe** ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2611](https://github.com/PurHur/php-compiler/issues/2611), [#2898](https://github.com/PurHur/php-compiler/issues/2898)); `--dry-run` skips gen-3 + full-revision |
