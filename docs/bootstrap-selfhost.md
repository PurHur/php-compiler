...
| Compiler unit probe native emit | `make bootstrap-selfhost-compiler-unit-probe-strict` or `BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT=1` + compile-driver env | ✅ `emit_path=native` compiles `compiler_unit_probe_compile.php` via M3 emit helper ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); partial Zend emit when `BOOTSTRAP_M3_LINK_COMPILE_DRIVER=1` without strict |
