...
| Compiler unit probe native link | `./script/bootstrap-selfhost-compiler-unit-probe.sh` or `make bootstrap-selfhost-compiler-unit-probe` | ✅ `build/selfhost-compiler-unit-probe` prints `compiler_unit_probe bundle OK` (link-only default); `BOOTSTRAP_COMPILER_UNIT_PROBE_GATE=1` default-on in `ci-local.sh` llvm tail ([#2221](https://github.com/PurHur/php-compiler/issues/2221)); set `0` to skip |
| Compiler unit probe strict emit | `make bootstrap-selfhost-compiler-unit-probe-strict` or `BOOTSTRAP_M3_COMPILER_UNIT_PROBE_STRICT=1 ./script/bootstrap-selfhost-compiler-unit-probe.sh` | ✅ `emit_path=native` compiles `compiler_unit_probe_compile.php` via `lib/Compiler.php` emit spine ([#2618](https://github.com/PurHur/php-compiler/issues/2618)); refuses Zend fallback on strict path |
