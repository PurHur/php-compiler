...
| M5 bootstrap driver smoke | `make bootstrap-selfhost-driver-smoke` or `BOOTSTRAP_M5_DRIVER_GATE=1` in `ci-local.sh` llvm tail | ✅ native gen-2 compile + run (`compiler smoke`) via `build/selfhost-helloworld-compile` — no Zend on compile step ([#1521](https://github.com/PurHur/php-compiler/issues/1521)); opt-in CI gate default **off** |
