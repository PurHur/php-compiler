...
| M4 bootstrap-loop gen-1 link | `./script/bootstrap-loop-gen1-link.sh` or `make bootstrap-loop-gen1-link` | ✅ gen-1 link + gen-2 native emit when `BOOTSTRAP_M4_LINK_COMPILE_DRIVER=1` (runtime execute defaults on — [#2599](https://github.com/PurHur/php-compiler/issues/2599)); Zend fallback only without link driver |
| M4 gen-2 strict emit gate | `BOOTSTRAP_M4_GEN2_STRICT=1 BOOTSTRAP_M4_LINK_COMPILE_DRIVER=1 ./script/bootstrap-loop-gen1-link.sh` | opt-in ([#2115](https://github.com/PurHur/php-compiler/issues/2115)); refuses Zend fallback (`emit_path=zend_fallback_would_be_used`) |
| M4 bootstrap-loop probe | `./script/bootstrap-loop-probe.sh` or `make bootstrap-loop-probe` | 🚧 **ladder** — `--dry-run`: M2 + M3 partial + gen-2 native slice; full: + M3 strict ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2075](https://github.com/PurHur/php-compiler/issues/2075)) |
