...
| Parser unit probe native link | `./script/bootstrap-selfhost-parser-unit-probe.sh` or `make bootstrap-selfhost-parser-unit-probe` | ✅ `build/selfhost-parser-unit-probe` prints `parser_unit_probe bundle OK` (link-only; `Runtime::parse` on fixture under Zend via PHPUnit); CI gate opt-in ([#2417](https://github.com/PurHur/php-compiler/issues/2417)) |
...
| PHPTypes unit probe native link | `./script/bootstrap-selfhost-types-unit-probe.sh` or `make bootstrap-selfhost-types-unit-probe` | ✅ `build/selfhost-types-unit-probe` prints `types_unit_probe bundle OK` (link-only; `PHPTypes\Type` constants + union/intersection `fromTypeDecl` under Zend via PHPUnit) |
...
| M2 lib spine smoke lint | `php bin/compile.php -l test/selfhost/compiler_lib_spine_smoke/main.php` | ✅ **655/657** units ([#1492](https://github.com/PurHur/php-compiler/issues/1492), [#2001](https://github.com/PurHur/php-compiler/issues/2001), [#2126](https://github.com/PurHur/php-compiler/issues/2126))
| M2 spine coverage drift | `php script/check-selfhost-spine-coverage-sync.php` | ✅ 655/657 in spine; **2** deferred (native-link [#1960](https://github.com/PurHur/php-compiler/issues/1960), [#2126](https://github.com/PurHur/php-compiler/issues/2126)); `SELFHOST_SPINE_COVERAGE_SYNC_GATE=1` ([#1945](https://github.com/PurHur/php-compiler/issues/1945))
...
| M3 compile-smoke strict CI gate | `BOOTSTRAP_M3_COMPILE_SMOKE_STRICT_GATE=1 ./script/ci-local.sh` | opt-in ([#1937](https://github.com/PurHur/php-compiler/issues/1937)); runs probe with `BOOTSTRAP_M3_COMPILE_SMOKE_STRICT=1` + compile-driver link (default **off** until `emit_path=native` stable; refuses `emit_path=zend_fallback_would_be_used`) |
...
| M3 Runtime compile smoke strict CI gate | `BOOTSTRAP_RUNTIME_COMPILE_SMOKE_STRICT_GATE=1 ./script/ci-local.sh` | opt-in ([#2294](https://github.com/PurHur/php-compiler/issues/2294)); runs probe with `BOOTSTRAP_M3_RUNTIME_COMPILE_SMOKE_STRICT=1` + compile-driver link (default **off** until `emit_path=native` stable; refuses `emit_path=zend_fallback_would_be_used`) |
| M4 bootstrap-loop gen-1 link | `./script/bootstrap-loop-gen1-link.sh` or `make bootstrap-loop-gen1-link` | 🚧 **partial** — gen-1 link ✅; gen-2 **Zend** emit (`emit_path=zend partial`) until native gen-2 ([#1498](https://github.com/PurHur/php-compiler/issues/1498), [#2075](https://github.com/PurHur/php-compiler/issues/2075)); opt-in `BOOTSTRAP_M4_LINK_COMPILE_DRIVER=1` for `emit_path=native` |
| M4 gen-2 strict emit gate | `BOOTSTRAP_M4_GEN2_STRICT=1 BOOTSTRAP_M4_LINK_COMPILE_DRIVER=1 ./script/bootstrap-loop-gen1-link.sh` | opt-in ([#2115](https://github.com/PurHur/php-compiler/issues/2115)); refuses Zend fallback (`emit_path=zend_fallback_would_be_used`); default **off** until native gen-2 stable ([#2075](https://github.com/PurHur/php-compiler/issues/2075)) |
...
| M3 unit probe presenter | `make north-star3-verify` or `./script/north-star3-verify.sh` | ✅ 008 VM + three compile-spine unit probes when scripts exist ([#2360](https://github.com/PurHur/php-compiler/issues/2360)); probes land in [#2216](https://github.com/PurHur/php-compiler/issues/2216), [#2332](https://github.com/PurHur/php-compiler/issues/2332), [#2354](https://github.com/PurHur/php-compiler/issues/2354); opt-in CI [#2396](https://github.com/PurHur/php-compiler/issues/2396) |
