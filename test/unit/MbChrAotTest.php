...
/**
 * AOT: mb_chr() leftover of #30759 (#33536) — TypeError/argc gates.
 *
 * @see php-src ext/mbstring/mbstring.c PHP_FUNCTION(mb_chr)
 *
 * @group aot-lint
 */
final class MbChrAotTest extends TestCase
{
    ...
    {
        $runtime = new Runtime();
        $code = file_get_contents(dirname(__DIR__).'/repro/issue_33536_mb_chr_aot.php');
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_33536_mb_chr_aot.php'));
    ...
    public function testAotTypeErrorAndArgcMatchVm(): void
    {
        ...
        $src = $root.'/test/repro/issue_33536_mb_chr_aot.php';
        $runtime = new Runtime();
        ...
        $bin = sys_get_temp_dir().'/phpc_mb_chr_'.getmypid().'.bin';
    ...
    {
        $src = (string) file_get_contents(dirname(__DIR__, 2).'/ext/mbstring/mb_chr.php');
