...
/**
 * phpc init --profile sessionsweb scaffold (issue #1886).
 */
final class PhpcInitSessionsWebTest extends TestCase
{
    public function testInitSessionsWebProfileLintClean(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_sessionsweb_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'sessionsweb', $work], $work);
            $this->assertSame(0, $init['exit'], $init['stderr']);

            foreach (['phpc.json', 'example.php', 'README.md'] as $relative) {
            ...
            $lint = $this->runPhpc(['lint', $work.'/example.php'], $repoRoot);
            $this->assertSame(0, $lint['exit'], $lint['stderr']."\n".$lint['stdout']);

            $second = $this->runPhpc(['init', '--profile', 'sessionsweb', $work], $work);
    ...
    public function testInitRejectsSessionsWebHyphenProfile(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_sessions_web_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'sessions-web', $work], $repoRoot);
