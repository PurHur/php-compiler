...
/**
 * AOT: SplFileObject::fflush via live stream handle (#33354).
 *
 * @see php-src ext/spl/spl_directory.c zim_SplFileObject_fflush
 *
 * @group llvm
 * @group aot
 */
final class SplFileObjectFflush33354AotTest extends TestCase
{
    public function testAotMatchesZendFflush(): void
    {
        $root = dirname(__DIR__, 2);
        $repro = $root.'/test/repro/splfileobject_fflush_aot.php';
        ...
        $zend = implode("\n", $zendOut)."\n";
        $this->assertStringContainsString('flush=true', $zend);
        $this->assertStringContainsString("got='hi'", $zend);

        if (!LlvmToolchain::hasLibrary($root)) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        $bin = sys_get_temp_dir().'/phpc_issue_33354_'.getmypid().'.bin';
        $cacheDir = sys_get_temp_dir().'/phpc_helper_cache_33354_'.getmypid();
    ...
    {
        $root = dirname(__DIR__, 2);
        $this->assertFileDoesNotExist($root.'/runtime/spl_fileobject_fflush.c');
        $helper = (string) file_get_contents($root.'/lib/VM/SplFileObjectJitHelper.php');
        $this->assertStringContainsString('compileFflush', $helper);
        $this->assertStringContainsString('#33354', $helper);
        $this->assertStringContainsString('JitFflush::invoke', $helper);
        $call = (string) file_get_contents($root.'/lib/JIT/Call/SplFileObjectMethod.php');
        $this->assertStringContainsString("'fflush'", $call);
        $ctx = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'fflush'", $ctx);
        $this->assertStringContainsString('#33354', $ctx);
    }
}
