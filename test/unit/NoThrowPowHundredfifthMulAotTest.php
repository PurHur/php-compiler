 ...
 * {@code 105} → {@code (hundredfourth×n)} chained smul
 ...
 * Peer: ** 2–** 104 (#37273…/#37693), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**105} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**105 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredfifthMulAotTest extends TestCase
{
    public function testPowHundredfifthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfifth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredfifth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfifth($src, 'work', ['-1']);
    }

    public function testPowHundredfifthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow105_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow105_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 105), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow105_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow105_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredfifth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow105_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow105_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
