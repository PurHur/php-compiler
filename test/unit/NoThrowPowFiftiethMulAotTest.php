 ...
 * {@code 50} → {@code (fortyeighth*sq)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftiethMulAotTest extends TestCase
{
    public function testPowFiftiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftieth($src, 'work', ['1125899906842624', '1125899906842624', '0']);
    }

    public function testBuiltinPowFiftieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftieth($src, 'work', ['1125899906842624']);
    }

    public function testPowFiftiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow50_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow50_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 50), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow50_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow50_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['1125899906842624'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow50_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow50_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
