 ...
 * {@code 56} → {@code (fiftyfifth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftysixthMulAotTest extends TestCase
{
    public function testPowFiftysixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftysixth($src, 'work', ['72057594037927936', '72057594037927936', '0']);
    }

    public function testBuiltinPowFiftysixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftysixth($src, 'work', ['72057594037927936']);
    }

    public function testPowFiftysixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow56_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow56_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 56), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow56_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow56_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['72057594037927936'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftysixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow56_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow56_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
