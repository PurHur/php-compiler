...
/**
 * AOT: openssl_csr_new() leftover of #6421 (#33527) — TypeError/argc gates.
 *
 * @see php-src ext/openssl/openssl.c PHP_FUNCTION(openssl_csr_new)
 *
 * @group aot-lint
 */
final class OpensslCsrNewAotTest extends TestCase
{
    ...
    {
        $runtime = new Runtime();
        $code = file_get_contents(dirname(__DIR__).'/repro/issue_33527_openssl_csr_new_aot.php');
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_33527_openssl_csr_new_aot.php'));
    ...
    public function testAotTypeErrorAndArgcMatchVm(): void
    {
        ...
        $src = $root.'/test/repro/issue_33527_openssl_csr_new_aot.php';
        $runtime = new Runtime();
        ...
        $bin = sys_get_temp_dir().'/phpc_ossl_csr_new_'.getmypid().'.bin';
