 ...
 * {@code 99} → {@code (ninetyeighth×n)} chained smul
 ...
 * Peer: ** 2–** 98 (#37273…/#37670), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**99} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**99 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetyninthMulAotTest extends TestCase
{
    public function testPowNinetyninthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyninth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowNinetyninth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyninth($src, 'work', ['-1']);
    }

    public function testPowNinetyninthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow99_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow99_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 99), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow99_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow99_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetyninth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow99_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow99_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
