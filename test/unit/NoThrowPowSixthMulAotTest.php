 ...
 * {@code 6} → {@code ($n*$n*$n)*($n*$n*$n)} with chained smul overflow→float —
 ...
 * ** 5 fifth (#37281), ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowSixthMulAotTest extends TestCase
{
    public function testPowSixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSixth($src, 'work', ['729', '64', '0']);
    }

    public function testBuiltinPowSixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSixth($src, 'work', ['64']);
    }

    public function testPowSixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow6_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow6_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 6), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow6_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow6_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow6_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow6_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
