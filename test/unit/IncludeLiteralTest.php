...
final class IncludeLiteralTest extends TestCase
{
    ...
    public function testAotNestedIncludeInheritsCallerScope(): void
    {
        ...
        $entry = realpath(__DIR__.'/../compliance/cases/language/include_scope_inherit/nested_entry.php');
        $this->assertNotFalse($entry);
        $outfile = sys_get_temp_dir().'/phpc_nested_inc_aot_'.bin2hex(random_bytes(6));
    ...
    /**
     * AOT require return into typed property constructor (MiniWebApp config.php, #764).
     *
     * @group llvm
     */
    public function testAotRequireReturnIntoTypedPropertyCtor(): void
    {
        if (!LlvmToolchain::isReady(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM toolchain not available');
        }
        $entry = realpath(__DIR__.'/../compliance/cases/language/require_return_ctor/entry.php');
        $this->assertNotFalse($entry);
        $outfile = sys_get_temp_dir().'/phpc_req_ctor_aot_'.bin2hex(random_bytes(6));
        $repoRoot = dirname(__DIR__, 2);
        $descriptorSpec = [
            0 => ['pipe', 'r'],
            1 => ['pipe', 'w'],
