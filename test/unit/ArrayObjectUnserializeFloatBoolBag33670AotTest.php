...
final class ArrayObjectUnserializeFloatBoolBag33670AotTest extends TestCase
{
    ...
    {
        $this->assertAotMatchesZend(__DIR__.'/../repro/arrayobject_unserialize_float_bool_bag_33670.php');
    }

    public function testFillHelperAcceptsFloatBoolNull(): void
    {
        $root = dirname(__DIR__, 2);
        $fill = (string) file_get_contents($root.'/ext/standard/UnserializeSplArrayFillNestedJitHelper.php');
        $this->assertStringContainsString('phpc_native_ht_set_string_key_bool(', $fill);
        $this->assertStringContainsString('phpc_native_ht_set_string_key_double_str(', $fill);
        $this->assertStringContainsString('phpc_native_ht_set_string_key_null(', $fill);
        $this->assertStringContainsString('#33670', $fill);
        $this->assertFileExists($root.'/ext/standard/phpc_native_ht_set_string_key_bool.php');
        $this->assertFileExists($root.'/ext/standard/phpc_native_ht_set_string_key_null.php');
        $this->assertFileExists($root.'/ext/standard/phpc_native_ht_set_string_key_double_str.php');
    }
