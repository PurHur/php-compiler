...
/**
 * link() VM/AOT smoke (issue #3589).
 */
final class LinkBuiltinTest extends TestCase
{
...
PHP;
    ...
    protected function setUp(): void
    {
        ...
        $base = dirname(__DIR__, 2).'/test/compliance/cases/stdlib/link_fixture';
        if (!is_dir($base)) {
            mkdir($base, 0777, true);
        }
        foreach (['src.txt', 'hardlink.txt', 'nope.txt'] as $name) {
            $path = $base.'/'.$name;
            if (is_file($path)) {
                unlink($path);
            }
    ...
    private function runAotBinary(): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_link_');
        $out = $tmp.'_bin';
