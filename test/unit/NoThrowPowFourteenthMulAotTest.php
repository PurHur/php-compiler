 ...
 * {@code 14} → {@code (((($n*$n*$n)*($n*$n*$n))*$n)*((($n*$n*$n)*($n*$n*$n))*$n))}
 ...
 * Peer: ** 2–** 13 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowFourteenthMulAotTest extends TestCase
{
    public function testPowFourteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFourteenth($src, 'work', ['4782969', '16384', '0']);
    }

    public function testBuiltinPowFourteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFourteenth($src, 'work', ['16384']);
    }

    public function testPowFourteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow14_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow14_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 14), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow14_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow14_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowFourteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow14_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow14_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
