 ...
 * {@code 60} → {@code (fiftyninth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowSixtiethMulAotTest extends TestCase
{
    public function testPowSixtiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSixtieth($src, 'work', ['1152921504606846976', '1152921504606846976', '0']);
    }

    public function testBuiltinPowSixtieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSixtieth($src, 'work', ['1152921504606846976']);
    }

    public function testPowSixtiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow60_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow60_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 60), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow60_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow60_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['1152921504606846976'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowSixtieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow60_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow60_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
