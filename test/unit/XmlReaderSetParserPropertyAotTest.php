 ...
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_XMLReader_setParserProperty
 *
 * @group aot-lint
 */
final class XmlReaderSetParserPropertyAotTest extends TestCase
{
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_setparserproperty_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_setparserproperty_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_setparserproperty_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_spp_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertStringContainsString("'xmlreader::setparserproperty' => true", $jit);
        $method = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderMethod.php');
        $this->assertStringContainsString('trySetParserProperty', $method);
        $user = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderUserScript.php');
        $this->assertStringContainsString('function trySetParserProperty', $user);
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/lib/AOT/runtime/xmlreader_set_parser_property.c');
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/runtime/xmlreader_set_parser_property.c');
    }
}
