 ...
 * {@code 100} → {@code (ninetyninth×n)} chained smul
 ...
 * Peer: ** 2–** 99 (#37273…/#37673), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**100} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**100 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredthMulAotTest extends TestCase
{
    public function testPowHundredthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowHundredth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredth($src, 'work', ['1']);
    }

    public function testPowHundredthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow100_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow100_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 100), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow100_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow100_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow100_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow100_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
