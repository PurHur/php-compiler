...
/**
 * AOT: openssl_pkey_get_private() leftover of #6295 (#33508) — TypeError/argc gates.
 *
 * @see php-src ext/openssl/openssl.c PHP_FUNCTION(openssl_pkey_get_private)
 *
 * @group aot-lint
 */
final class OpensslPkeyGetPrivateAotTest extends TestCase
{
    ...
    {
        $runtime = new Runtime();
        $code = file_get_contents(dirname(__DIR__).'/repro/issue_33508_openssl_pkey_get_private_aot.php');
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_33508_openssl_pkey_get_private_aot.php'));
    ...
    public function testAotTypeErrorAndArgcMatchVm(): void
    {
        ...
        $src = $root.'/test/repro/issue_33508_openssl_pkey_get_private_aot.php';
        $runtime = new Runtime();
        ...
        $bin = sys_get_temp_dir().'/phpc_ossl_pkey_get_private_'.getmypid().'.bin';
