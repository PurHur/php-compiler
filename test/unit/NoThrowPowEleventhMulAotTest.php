 ...
 * {@code 11} → {@code ((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n)))*$n} with
 ...
 * Peer: ** 2–** 10 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowEleventhMulAotTest extends TestCase
{
    public function testPowEleventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowEleventh($src, 'work', ['177147', '-2048', '0']);
    }

    public function testBuiltinPowEleventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowEleventh($src, 'work', ['2048']);
    }

    public function testPowEleventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow11_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow11_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 11), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow11_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow11_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEleventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow11_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow11_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
