...
/**
 * Host compile of Ast/Cli units that previously failed under SPINE_CHUNK
 * (preg_replace_callback / NestedJIT OOM) after method demote (#36387).
 *
 * @group aot-lint
 */
final class AstCliSpineChunkEmit36387Test extends TestCase
{
    /**
     * @return list<string>
     */
    ...
    public function testObjectOnlyEmitSucceedsUnderSpineChunk(string $rel): void
    {
        ...
        $tmp = sys_get_temp_dir().'/phpc-ast-cli-36387-'.bin2hex(random_bytes(4));
        mkdir($tmp, 0755, true);
        $entry = $tmp.'/entry.php';
        $out = $tmp.'/unit.o';
