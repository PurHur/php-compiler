 ...
 * @group aot
 */
final class Issue34193ReflectionExtensionGetINIEntriesLeftoversAotTest extends TestCase
{
    private const EXPECT = 'filter=2 openssl=2 mbstring=11 session=30 filter.default=1 filter.default_flags_null=1 openssl.cafile_null=1 mbstring.language=1 session.name=1';

    public function testRuntimeBakesLeftoverExtensions(): void
    {
    ...
    public function testAotGetINIEntriesLeftoversMatchZend(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/issue_34193_reflection_extension_getinientries_leftovers_aot.php';
    ...
    public function testZendGetINIEntriesLeftoversBaseline(): void
    {
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/issue_34193_reflection_extension_getinientries_leftovers_aot.php';
    ...
    public function testVmGetINIEntriesLeftoversMatchZend(): void
    {
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/issue_34193_reflection_extension_getinientries_leftovers_aot.php';
