 ...
 * {@code 72} → {@code (seventyfirst*n)} chained smul
 ...
 * Peer: ** 2–** 71 (#37273…/#37579), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**72} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**72 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventysecondMulAotTest extends TestCase
{
    public function testPowSeventysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventysecond($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSeventysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventysecond($src, 'work', ['1']);
    }

    public function testPowSeventysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow72_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow72_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 72), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow72_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow72_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow72_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow72_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
