...
/**
 * AOT: openssl_pkey_get_public() leftover of #20240 (#33499) — TypeError/argc gates.
 *
 * @see php-src ext/openssl/openssl.c PHP_FUNCTION(openssl_pkey_get_public)
 *
 * @group aot-lint
 */
final class OpensslPkeyGetPublicAotTest extends TestCase
{
    ...
    {
        $runtime = new Runtime();
        $code = file_get_contents(dirname(__DIR__).'/repro/issue_33499_openssl_pkey_get_public_aot.php');
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_33499_openssl_pkey_get_public_aot.php'));
    ...
    public function testAotTypeErrorAndArgcMatchVm(): void
    {
        ...
        $src = $root.'/test/repro/issue_33499_openssl_pkey_get_public_aot.php';
        $runtime = new Runtime();
        ...
        $bin = sys_get_temp_dir().'/phpc_ossl_pkey_get_public_'.getmypid().'.bin';
