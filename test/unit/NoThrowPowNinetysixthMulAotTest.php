 ...
 * {@code 96} → {@code (ninetyfifth×n)} chained smul
 ...
 * Note: {@code (±2)**96} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**96 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetysixthMulAotTest extends TestCase
{
    public function testPowNinetysixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetysixth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowNinetysixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetysixth($src, 'work', ['1']);
    }

    public function testPowNinetysixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow96_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow96_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 96), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow96_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow96_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetysixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow96_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow96_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
