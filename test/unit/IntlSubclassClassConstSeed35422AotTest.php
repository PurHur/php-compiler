<?php

declare(strict_types=1);

use PHPUnit\Framework\TestCase;

/**
 * AOT: intl subclass ClassConstFetch seeds (#35422 peer #35389 / #35401).
 *
 * php-src: ext/intl/calendar.stub.php / breakiterator.stub.php — subclass consts.
 *
 * @group llvm
 */
final class IntlSubclassClassConstSeed35422AotTest extends TestCase
{
    public function testSeedHookExists(): void
    {
        $src = (string) file_get_contents(
            __DIR__.'/../../lib/JIT/Builtin/Type/Object_.php'
        );
        $this->assertStringContainsString("'intlgregoriancalendar' === \$lcname", $src);
        $this->assertStringContainsString("'intlrulebasedbreakiterator' === \$lcname", $src);
        $this->assertStringContainsString('partsIteratorConstants()', $src);
        $this->assertStringContainsString('#35422', $src);
    }

    public function testAotIntlSubclassConstsMatch(): void
    {
        $repro = dirname(__DIR__).'/repro/aot_intl_subclass_const_seed.php';
        $this->assertFileExists($repro);
        $bin = sys_get_temp_dir().'/isc_cc_35422_'.getmypid();
        $compile = $this->runCmd([
            PHP_BINARY,
            dirname(__DIR__, 2).'/bin/compile.php',
            '-o',
            $bin,
            $repro,
        ]);
        $this->assertSame(0, $compile['code'], $compile['out']);
        $this->assertFileExists($bin);
        try {
            $aot = $this->runCmd([$bin]);
            $this->assertSame(0, $aot['code'], $aot['out']);
            $this->assertSame(
                "GREG_FIELD_YEAR=1\nRB_DONE=-1\nCP_DONE=-1\nPARTS_KEY_SEQUENTIAL=0\n",
                $aot['out']
            );
        } finally {
            @unlink($bin);
        }
    }

    /** @return array{code:int,out:string} */
    private function runCmd(array $cmd): array
    {
        $desc = [1 => ['pipe', 'w'], 2 => ['pipe', 'w']];
        $proc = proc_open($cmd, $desc, $pipes, dirname(__DIR__, 2));
        $this->assertIsResource($proc);
        $out = stream_get_contents($pipes[1]).stream_get_contents($pipes[2]);
        fclose($pipes[1]);
        fclose($pipes[2]);
        $code = proc_close($proc);

        return ['code' => $code, 'out' => $out];
    }
}
