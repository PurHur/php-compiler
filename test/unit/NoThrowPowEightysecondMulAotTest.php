 ...
 * {@code 82} → {@code (eightyfirst*n)} chained smul
 ...
 * Peer: ** 2–** 81 (#37273…/#37610), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**82} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**82 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightysecondMulAotTest extends TestCase
{
    public function testPowEightysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightysecond($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowEightysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightysecond($src, 'work', ['1']);
    }

    public function testPowEightysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow82_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow82_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 82), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow82_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow82_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow82_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow82_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
