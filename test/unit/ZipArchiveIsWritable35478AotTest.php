 ...
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_isWritable / zim_ZipArchive_setReadOnly
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveIsWritable35478AotTest extends TestCase
{
    public function testIsWritableAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_iswritable_aot.php';
        $this->assertSame($this->runVm($src), $this->runAot($src));
    }

    public function testProxyRegisteredForIsWritable(): void
    {
        $root = dirname(__DIR__, 2);
        $src = (string) file_get_contents($root.'/ext/zip/Module.php');
        $this->assertStringContainsString("'isWritable'", $src);
        $this->assertStringContainsString("'setReadOnly'", $src);
        $helper = (string) file_get_contents($root.'/ext/zip/ZipArchiveJitHelper.php');
        $this->assertStringContainsString("'is_writable'", $helper);
        $this->assertStringContainsString("'set_readonly'", $helper);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_iw_'.getmypid().'_'.md5($src);
