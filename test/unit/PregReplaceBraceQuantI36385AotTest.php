...
/**
 * `/X{n,}/i` delimiter + brace-quant find must match Zend under thin AOT (#36385).
 *
 * php-src: ext/pcre/php_pcre.c (modifiers + brace quantifiers).
 */
final class PregReplaceBraceQuantI36385AotTest extends \PHPUnit\Framework\TestCase
{
    public function testDelimitedBodyCloseAllowsIModifier(): void
    {
        $this->assertSame(
            1,
            PregAotFastPath::replaceFindNext('/t{3,}/i', 'tttAAA', 0)
        );
        $this->assertSame(0, PregAotFastPath::takeLastReplacePos());
        $this->assertSame(3, PregAotFastPath::takeLastReplaceBodyLen());

        $this->assertSame(
            1,
            PregAotFastPath::replaceFindNext('/a{3,}/i', 'TTTAAA', 0)
        );
        $this->assertSame(3, PregAotFastPath::takeLastReplacePos());
        $this->assertSame(3, PregAotFastPath::takeLastReplaceBodyLen());
    }

    public function testChainedBraceQuantIReplaceMatchesZendOnVmAndAot(): void
    {
        $path = __DIR__ . '/../repro/preg_replace_brace_quant_i_36385.php';
        $zend = $this->runZendFile($path);
        $this->assertSame("700|650\n", $zend);
    ...
    {
        $bin = tempnam(sys_get_temp_dir(), 'aot36385brace_');
