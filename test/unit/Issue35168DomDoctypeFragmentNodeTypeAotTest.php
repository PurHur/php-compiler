...
/**
 * #35168 — AOT createDocumentType / createDocumentFragment nodeType matches Zend (no SIGSEGV).
 *
 * @group llvm
 */
final class Issue35168DomDoctypeFragmentNodeTypeAotTest extends TestCase
{
    public function testAotDoctypeAndFragmentNodeTypeMatchesZend(): void
    {
        $src = dirname(__DIR__).'/repro/issue_dom_doctype_fragment_nodetype_aot.php';
        $this->assertFileExists($src);

        $zend = $this->runPhp($src);
        $vm = $this->runVm($src);
        $this->assertSame($zend, $vm, 'VM must match Zend');
        $this->assertStringContainsString('doctype_type=10', $zend);
        $this->assertStringContainsString('frag_type=11', $zend);

        $bin = sys_get_temp_dir().'/phpc_35168_'.getmypid().'.bin';
        $compile = $this->runCompile($src, $bin);
        $this->assertSame(0, $compile['code'], "AOT compile failed:\n".$compile['out']);
        $aot = $this->runBin($bin);
        @unlink($bin);

        $this->assertSame($zend, $aot['out'], 'AOT must match Zend DocumentType/Fragment nodeType');
