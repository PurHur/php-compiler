 ...
 * {@code 8} → {@code (($n*$n)*($n*$n))*(($n*$n)*($n*$n))} with chained smul
 ...
 * Peer: ** 2–** 7 (#37273/#37277/#37278/#37281/#37283/#37287), ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowEighthMulAotTest extends TestCase
{
    public function testPowEighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowEighth($src, 'work', ['6561', '256', '0']);
    }

    public function testBuiltinPowEighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowEighth($src, 'work', ['256']);
    }

    public function testPowEighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow8_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow8_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 8), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow8_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow8_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow8_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow8_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
