 ...
 * {@code 73} → {@code (seventysecond*n)} chained smul
 ...
 * Peer: ** 2–** 72 (#37273…/#37583), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**73} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**73 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventythirdMulAotTest extends TestCase
{
    public function testPowSeventythirdOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventythird($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSeventythird(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventythird($src, 'work', ['-1']);
    }

    public function testPowSeventythirdOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow73_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow73_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 73), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow73_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow73_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventythird(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow73_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow73_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
