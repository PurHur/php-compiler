 ...
 * {@code 24} → {@code (((($n*$n*$n)*($n*$n*$n))*(($n*$n*$n)*($n*$n*$n)))*((($n*$n*$n)*($n*$n*$n))*(($n*$n*$n)*($n*$n*$n))))}
 ...
 * Peer: ** 2–** 23 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347/#37351/#37354/#37361/#37364/#37366/#37370),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowTwentyfourthMulAotTest extends TestCase
{
    public function testPowTwentyfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowTwentyfourth($src, 'work', ['282429536481', '16777216', '0']);
    }

    public function testBuiltinPowTwentyfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowTwentyfourth($src, 'work', ['16777216']);
    }

    public function testPowTwentyfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow24_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow24_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 24), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow24_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow24_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['16777216'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowTwentyfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow24_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow24_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
