 ...
 * {@code 54} → {@code (fiftythird*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyfourthMulAotTest extends TestCase
{
    public function testPowFiftyfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyfourth($src, 'work', ['18014398509481984', '18014398509481984', '0']);
    }

    public function testBuiltinPowFiftyfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyfourth($src, 'work', ['18014398509481984']);
    }

    public function testPowFiftyfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow54_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow54_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 54), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow54_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow54_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['18014398509481984'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow54_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow54_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
