 ...
 * {@code 101} → {@code (hundredth×n)} chained smul
 ...
 * Peer: ** 2–** 100 (#37273…/#37680), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**101} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**101 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredfirstMulAotTest extends TestCase
{
    public function testPowHundredfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfirst($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfirst($src, 'work', ['-1']);
    }

    public function testPowHundredfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow101_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow101_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 101), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow101_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow101_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow101_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow101_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
