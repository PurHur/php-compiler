 ...
 * {@code 92} → {@code (ninetyfirst*n)} chained smul
 ...
 * Peer: ** 2–** 91 (#37273…/#37645), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**92} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**92 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetysecondMulAotTest extends TestCase
{
    public function testPowNinetysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetysecond($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowNinetysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetysecond($src, 'work', ['1']);
    }

    public function testPowNinetysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow92_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow92_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 92), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow92_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow92_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow92_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow92_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
