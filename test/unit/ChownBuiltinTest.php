...
/**
 * chown() / lchown() VM/AOT smoke (issue #3241).
 */
final class ChownBuiltinTest extends TestCase
{
    private const CODE = <<<'PHP'
$base = 'test/compliance/cases/stdlib/chmod_fixture';
$path = $base . '/chown_unit_data.txt';
if (!is_dir($base)) {
...
$path = 'test/compliance/cases/stdlib/chmod_fixture/chown_aot_unit_data.txt';
if (file_put_contents($path, 'x')) {
    $st = stat($path);
    $uid = (int) ($st['uid'] ?? 0);
    if (chown($path, $uid)) {
...
PHP;
    ...
    protected function setUp(): void
    {
        ...
        foreach (['chown_unit_data.txt', 'chown_aot_unit_data.txt', 'chown_aot_data.txt', 'chown_data.txt'] as $name) {
            $path = $base.'/'.$name;
            if (is_file($path)) {
                @unlink($path);
            }
    ...
    private function runAotBinary(): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_chown_');
        $out = $tmp.'_bin';
