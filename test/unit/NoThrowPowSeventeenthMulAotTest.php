 ...
 * {@code 17} → {@code ((((($n*$n)*($n*$n))*(($n*$n)*($n*$n)))*((($n*$n)*($n*$n))*(($n*$n)*($n*$n))))*$n}
 ...
 * Peer: ** 2–** 16 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowSeventeenthMulAotTest extends TestCase
{
    public function testPowSeventeenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSeventeenth($src, 'work', ['129140163', '-131072', '0']);
    }

    public function testBuiltinPowSeventeenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSeventeenth($src, 'work', ['131072']);
    }

    public function testPowSeventeenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow17_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow17_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 17), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow17_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow17_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventeenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow17_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow17_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
