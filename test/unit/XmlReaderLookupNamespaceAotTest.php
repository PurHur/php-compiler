 ...
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_XMLReader_lookupNamespace
 *
 * @group aot-lint
 */
final class XmlReaderLookupNamespaceAotTest extends TestCase
{
    private const EXPECTED = "'urn:x'\nNULL\n'urn:x'\n'urn:q'\n";
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_lookupnamespace_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_lookupnamespace_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_lookupnamespace_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_lookupns_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertStringContainsString("'xmlreader::lookupnamespace' => true", $jit);
        $method = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderMethod.php');
        $this->assertStringContainsString('tryLookupNamespace', $method);
        $user = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderUserScript.php');
        $this->assertStringContainsString('function tryLookupNamespace', $user);
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/lib/AOT/runtime/xmlreader_lookup_namespace.c');
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/runtime/xmlreader_lookup_namespace.c');
    }
}
