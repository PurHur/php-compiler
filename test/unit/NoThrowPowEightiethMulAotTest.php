 ...
 * {@code 80} → {@code (seventyninth*n)} chained smul
 ...
 * Peer: ** 2–** 79 (#37273…/#37602), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**80} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**80 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightiethMulAotTest extends TestCase
{
    public function testPowEightiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightieth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowEightieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightieth($src, 'work', ['1']);
    }

    public function testPowEightiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow80_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow80_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 80), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow80_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow80_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow80_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow80_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
