...
/**
 * AOT: DOMDocument validate/schemaValidate/relaxNG/xinclude/registerNodeClass (#35540).
 *
 * @see php-src ext/dom/document.c
 *
 * @group llvm
 * @group aot
 */
final class DomDocumentValidate35540AotTest extends TestCase
{
    public function testValidateFamilyAotMatchesVm(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }

        $src = __DIR__.'/../repro/aot_dom_document_validate_null.php';
    ...
    public function testProxyWiredInDomInstanceMethodJit(): void
    {
        ...
        foreach ([
            ...
            $this->assertStringContainsString("'".$key."' => true", $jit);
        }
        $invoke = (string) file_get_contents($root.'/ext/dom/VmDomInstanceInvoke.php');
        $this->assertStringContainsString("'schemavalidate'", $invoke);
        $this->assertStringContainsString("'registernodeclass'", $invoke);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/dom_val_35540_'.getmypid().'_'.md5($src);
