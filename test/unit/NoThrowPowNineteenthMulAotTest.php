 ...
 * {@code 19} → {@code ((((($n*$n*$n)*($n*$n*$n))*($n*$n*$n))*((($n*$n*$n)*($n*$n*$n))*($n*$n*$n)))*$n}
 ...
 * Peer: ** 2–** 18 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347/#37351),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowNineteenthMulAotTest extends TestCase
{
    public function testPowNineteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowNineteenth($src, 'work', ['1162261467', '-524288', '0']);
    }

    public function testBuiltinPowNineteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowNineteenth($src, 'work', ['524288']);
    }

    public function testPowNineteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow19_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow19_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 19), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow19_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow19_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNineteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow19_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow19_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
