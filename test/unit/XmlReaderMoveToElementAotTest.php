 ...
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_XMLReader_moveToElement
 *
 * @group aot-lint
 */
final class XmlReaderMoveToElementAotTest extends TestCase
{
    private const EXPECTED = "mta=true val='2'\nmte=true name=root nt=1\nmte2=false\n";
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_movetoelement_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_movetoelement_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_movetoelement_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_mte_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/lib/AOT/runtime/xmlreader_movetoelement.c');
    }
}
