 ...
 * {@code 83} → {@code (eightysecond*n)} chained smul
 ...
 * Peer: ** 2–** 82 (#37273…/#37613), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**83} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**83 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightythirdMulAotTest extends TestCase
{
    public function testPowEightythirdOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightythird($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowEightythird(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightythird($src, 'work', ['-1']);
    }

    public function testPowEightythirdOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow83_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow83_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 83), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow83_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow83_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightythird(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow83_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow83_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
