 ...
 * {@code 13} → {@code ((($n*$n*$n)*($n*$n*$n))*(($n*$n*$n)*($n*$n*$n)))*$n} with
 * chained smul overflow→float — omit {@code llvm.pow.f64} (#36386).
 *
 * php-src: Zend/zend_operators.c pow_function / zend_pow / mul_function;
 * ext/standard/math.c PHP_FUNCTION(pow).
 * Peer: ** 2–** 12 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowThirteenthMulAotTest extends TestCase
{
    public function testPowThirteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowThirteenth($src, 'work', ['1594323', '-8192', '0']);
    }

    public function testBuiltinPowThirteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowThirteenth($src, 'work', ['8192']);
    }

    public function testPowThirteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow13_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow13_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 13), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow13_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow13_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowThirteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow13_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow13_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
