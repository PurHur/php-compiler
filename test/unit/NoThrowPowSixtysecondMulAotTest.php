 ...
 * {@code 62} → {@code (sixtyfirst*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowSixtysecondMulAotTest extends TestCase
{
    public function testPowSixtysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSixtysecond($src, 'work', ['4611686018427387904', '4611686018427387904', '0']);
    }

    public function testBuiltinPowSixtysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSixtysecond($src, 'work', ['4611686018427387904']);
    }

    public function testPowSixtysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow62_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow62_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 62), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow62_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow62_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['4611686018427387904'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowSixtysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow62_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow62_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
