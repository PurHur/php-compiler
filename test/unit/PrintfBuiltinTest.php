...
/**
 * printf() VM/AOT smoke (issue #3681).
 */
final class PrintfBuiltinTest extends TestCase
{
    private const CODE = <<<'PHP'
printf("%d %s\n", 42, 'ok');
PHP;

    private const EXPECT = '42 ok';
    ...
    public function testVmReturnLength(): void
    {
        $code = <<<'PHP'
$n = printf("%d %s\n", 42, 'ok');
var_dump($n);
PHP;
        $this->assertSame("42 ok\nint(6)", $this->runBin('bin/vm.php', $code));
    }
    ...
    private function runAotBinary(): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_printf_');
        $out = $tmp.'_bin';
    ...
    {
        $repo = dirname(__DIR__, 2);
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_printf_vm_');
        $this->assertNotFalse($tmp);
        file_put_contents($tmp, "<?php\n".$code);
        $proc = proc_open(
            ['php', $repo.'/'.$bin, $tmp],
