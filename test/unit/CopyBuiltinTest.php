 ...
 * copy() VM/AOT smoke.
 */
final class CopyBuiltinTest extends TestCase
{
...
PHP;
    ...
    {
        parent::setUp();
        $base = dirname(__DIR__, 2).'/test/compliance/cases/stdlib/copy_fixture';
        if (!is_dir($base)) {
            mkdir($base, 0777, true);
        }
        foreach (['source.txt', 'dest.txt'] as $name) {
            $path = $base.'/'.$name;
            if (is_file($path)) {
                unlink($path);
            }
    ...
    private function runAotBinary(): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_copy_');
        $out = $tmp.'_bin';
