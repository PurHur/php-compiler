 ...
 * {@code 98} → {@code (ninetyseventh×n)} chained smul
 ...
 * Peer: ** 2–** 97 (#37273…/#37667), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**98} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**98 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetyeighthMulAotTest extends TestCase
{
    public function testPowNinetyeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyeighth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowNinetyeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyeighth($src, 'work', ['1']);
    }

    public function testPowNinetyeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow98_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow98_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 98), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow98_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow98_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetyeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow98_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow98_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
