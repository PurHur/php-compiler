...
/**
 * AOT: ZipArchive::setPassword NestedJIT (#35500 leftover of #35496 / #19873).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_setPassword
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveSetPassword35500AotTest extends TestCase
{
    public function testSetPasswordAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_setpassword_aot.php';
    ...
    public function testIrDispatchPresent(): void
    {
        ...
        $this->assertStringContainsString("'spw'", $jit);
        $this->assertStringContainsString('function setPassword', $jit);
        $call = (string) file_get_contents($root.'/lib/JIT/Call/ZipArchiveMethod.php');
        $this->assertStringContainsString("'setpassword'", $call);
        $ctx = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'setPassword'", $ctx);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_spw_'.getmypid().'_'.md5($src);
