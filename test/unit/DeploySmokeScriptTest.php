...
final class DeploySmokeScriptTest extends TestCase
{
    ...
    public function testDeploySmokeExample003SkipsWhenBlocked(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $script = $repoRoot.'/script/deploy-smoke.sh';
        ...
        $proc = proc_open(['bash', $script, '--example', '003'], $descriptorSpec, $pipes, $repoRoot, $this->baseEnv());
        $this->assertIsResource($proc);
        fclose($pipes[0]);
        $stdout = stream_get_contents($pipes[1]);
        $stderr = stream_get_contents($pipes[2]);
        ...
        $this->assertStringContainsString('#612', $combined);
    }
