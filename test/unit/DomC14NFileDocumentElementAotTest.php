...
/**
 * AOT: DOMNode::C14NFile() after loadXML / createElement (#32964).
 *
 * Untyped documentElement temps must remap to DOMNode::C14NFile (peer C14N / isSameNode).
 * Typed createElement receivers must fold via host C14N (no DomRegistry ObjectEntry).
 *
...
final class DomC14NFileDocumentElementAotTest extends TestCase
{
    ...
    public function testVmCreateElementC14NFile(): void
    {
        $runtime = new Runtime();
        $code = file_get_contents(
            dirname(__DIR__).'/repro/issue_32964_dom_c14nfile_createelement_aot.php'
        );
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_32964_dom_c14nfile_createelement_aot.php'));
        $out = (string) ob_get_clean();
        $this->assertSame(self::EXPECTED, $out);
    }

    public function testAotCreateElementC14NFile(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/issue_32964_dom_c14nfile_createelement_aot.php';
        $bin = sys_get_temp_dir().'/phpc_c14nfile_32964ce_'.getmypid();
        $compile = escapeshellarg(PHP_BINARY).' '.escapeshellarg($root.'/bin/compile.php')
            .' -o '.escapeshellarg($bin).' '.escapeshellarg($src);
        exec($compile.' 2>&1', $out, $rc);
        $this->assertSame(0, $rc, implode("\n", $out));
