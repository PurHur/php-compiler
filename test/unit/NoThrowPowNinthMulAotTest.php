 ...
 * {@code 9} → {@code (($n*$n*$n)*($n*$n*$n))*($n*$n*$n)} with chained smul
 ...
 * Peer: ** 2–** 8 (#37273/#37277/#37278/#37281/#37283/#37287/#37291), ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowNinthMulAotTest extends TestCase
{
    public function testPowNinthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowNinth($src, 'work', ['19683', '-512', '0']);
    }

    public function testBuiltinPowNinth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowNinth($src, 'work', ['512']);
    }

    public function testPowNinthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow9_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow9_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 9), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow9_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow9_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow9_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow9_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
