 ...
 * {@code 87} → {@code (eightysixth*n)} chained smul
 ...
 * Peer: ** 2–** 86 (#37273…/#37626), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**87} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**87 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyseventhMulAotTest extends TestCase
{
    public function testPowEightyseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyseventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowEightyseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyseventh($src, 'work', ['-1']);
    }

    public function testPowEightyseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow87_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow87_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 87), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow87_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow87_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow87_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow87_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
