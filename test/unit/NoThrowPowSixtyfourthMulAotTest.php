 ...
 * {@code 64} → {@code (sixtythird*n)} chained smul
 ...
 * Peer: ** 2–** 63 (#37273…/#37528), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**64} and larger magnitudes promote to float (signed i64).
 *
 * @group aot-lint
 */
final class NoThrowPowSixtyfourthMulAotTest extends TestCase
{
    public function testPowSixtyfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyfourth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSixtyfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyfourth($src, 'work', ['1']);
    }

    public function testPowSixtyfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow64_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow64_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 64), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow64_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow64_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixtyfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow64_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow64_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
