...
/**
 * AOT: `$obj instanceof $className` for non-literal strings (#32766 follow-up).
 *
...
final class InstanceOfVarClassAotTest extends TestCase
{
    ...
    public function testInstanceOfHelperUsesLengthAwareStringCompare(): void
    {
        $src = (string) file_get_contents(dirname(__DIR__, 2).'/lib/JIT/InstanceOfHelper.php');
        $this->assertStringContainsString('asciiCaseInsensitiveIdentical', $src);
        $this->assertStringNotContainsString('ABI_STRCASECMP', $src);
        $this->assertStringNotContainsString('ABI_STRNCASECMP', $src);
    }
}
