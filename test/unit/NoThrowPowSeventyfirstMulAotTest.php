 ...
 * {@code 71} → {@code (seventieth*n)} chained smul
 ...
 * Peer: ** 2–** 70 (#37273…/#37565), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**71} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**71 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyfirstMulAotTest extends TestCase
{
    public function testPowSeventyfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfirst($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSeventyfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfirst($src, 'work', ['-1']);
    }

    public function testPowSeventyfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow71_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow71_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 71), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow71_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow71_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow71_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow71_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
