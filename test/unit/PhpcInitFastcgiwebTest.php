...
/**
 * phpc init --profile fastcgiweb scaffold (issue #2342).
 */
final class PhpcInitFastcgiwebTest extends TestCase
{
    public function testInitFastcgiwebProfileLintClean(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_fastcgiweb_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'fastcgiweb', $work], $work);
            ...
            $canonical = $repoRoot.'/examples/009-FastCGIWeb';
            $this->assertSame(
            ...
            $second = $this->runPhpc(['init', '--profile', 'fastcgiweb', $work], $work);
    ...
    public function testInitRejectsFastcgiwebTypoProfile(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_fastcgiweb_bad_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'fast-cgi-web', $work], $repoRoot);
