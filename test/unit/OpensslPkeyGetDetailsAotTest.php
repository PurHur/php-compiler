...
/**
 * AOT: openssl_pkey_get_details() leftover of #20240 (#33496) — TypeError/argc gates.
 *
 * @see php-src ext/openssl/openssl.c PHP_FUNCTION(openssl_pkey_get_details)
 *
 * @group aot-lint
 */
final class OpensslPkeyGetDetailsAotTest extends TestCase
{
    ...
    {
        $runtime = new Runtime();
        $code = file_get_contents(dirname(__DIR__).'/repro/issue_33496_openssl_pkey_get_details_aot.php');
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_33496_openssl_pkey_get_details_aot.php'));
    ...
    public function testAotTypeErrorAndArgcMatchVm(): void
    {
        ...
        $src = $root.'/test/repro/issue_33496_openssl_pkey_get_details_aot.php';
        $runtime = new Runtime();
        ...
        $bin = sys_get_temp_dir().'/phpc_ossl_pkey_details_'.getmypid().'.bin';
