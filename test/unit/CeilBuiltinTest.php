...
/**
 * ceil() VM/AOT smoke (#27003 — AOT must not return 0 via VmMath recursion).
 *
 * Multi-call UnaryMinus-arg sequences can still mis-wire under AOT after a prior
 * ceil() (second negative literal → 0); same class as fmod (#15736 / #26994).
 * Single-call negatives are covered separately below.
 */
final class CeilBuiltinTest extends TestCase
{
...
PHP;
    ...
    private const EXPECT = "2\n2\n0\n";

    private const NEG_CODE = <<<'PHP'
echo ceil(-1.7), "\n";
PHP;

    private const NEG_EXPECT = "-1\n";
    ...
    private function runAotBinary(string $code): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_ceil_');
        $out = $tmp.'_bin';
