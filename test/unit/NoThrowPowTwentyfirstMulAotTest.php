 ...
 * {@code 21} → {@code ((((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n)))*((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n))))*$n}
 ...
 * Peer: ** 2–** 20 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347/#37351/#37354/#37361),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowTwentyfirstMulAotTest extends TestCase
{
    public function testPowTwentyfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowTwentyfirst($src, 'work', ['10460353203', '-2097152', '0']);
    }

    public function testBuiltinPowTwentyfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowTwentyfirst($src, 'work', ['2097152']);
    }

    public function testPowTwentyfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow21_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow21_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 21), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow21_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow21_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['2097152'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowTwentyfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow21_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow21_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
