 ...
 * {@code 90} → {@code (eightyninth*n)} chained smul
 ...
 * Peer: ** 2–** 89 (#37273…/#37634), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**90} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**90 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetiethMulAotTest extends TestCase
{
    public function testPowNinetiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetieth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowNinetieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetieth($src, 'work', ['1']);
    }

    public function testPowNinetiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow90_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow90_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 90), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow90_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow90_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow90_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow90_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
