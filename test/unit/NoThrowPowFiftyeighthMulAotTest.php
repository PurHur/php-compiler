 ...
 * {@code 58} → {@code (fiftyseventh*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyeighthMulAotTest extends TestCase
{
    public function testPowFiftyeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyeighth($src, 'work', ['288230376151711744', '288230376151711744', '0']);
    }

    public function testBuiltinPowFiftyeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyeighth($src, 'work', ['288230376151711744']);
    }

    public function testPowFiftyeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow58_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow58_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 58), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow58_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow58_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['288230376151711744'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow58_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow58_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
