 ...
 * {@code 16} → {@code (((($n*$n)*($n*$n))*(($n*$n)*($n*$n)))*((($n*$n)*($n*$n))*(($n*$n)*($n*$n))))}
 ...
 * Peer: ** 2–** 15 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowSixteenthMulAotTest extends TestCase
{
    public function testPowSixteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSixteenth($src, 'work', ['43046721', '65536', '0']);
    }

    public function testBuiltinPowSixteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSixteenth($src, 'work', ['65536']);
    }

    public function testPowSixteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow16_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow16_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 16), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow16_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow16_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow16_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow16_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
