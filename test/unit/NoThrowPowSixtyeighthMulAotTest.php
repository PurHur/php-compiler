 ...
 * {@code 68} → {@code (sixtyseventh*n)} chained smul
 ...
 * Peer: ** 2–** 67 (#37273…/#37552), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**68} and larger magnitudes promote to float (signed i64).
 *
 * @group aot-lint
 */
final class NoThrowPowSixtyeighthMulAotTest extends TestCase
{
    public function testPowSixtyeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyeighth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSixtyeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyeighth($src, 'work', ['1']);
    }

    public function testPowSixtyeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow68_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow68_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 68), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow68_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow68_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixtyeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow68_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow68_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
