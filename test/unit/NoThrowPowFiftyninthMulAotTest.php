 ...
 * {@code 59} → {@code (fiftyeighth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyninthMulAotTest extends TestCase
{
    public function testPowFiftyninthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyninth($src, 'work', ['576460752303423488', '-576460752303423488', '0']);
    }

    public function testBuiltinPowFiftyninth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyninth($src, 'work', ['576460752303423488']);
    }

    public function testPowFiftyninthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow59_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow59_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 59), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow59_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow59_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['576460752303423488'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyninth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow59_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow59_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
