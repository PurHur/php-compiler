 ...
 * {@code 4} → {@code ($n*$n)*($n*$n)} with chained smul overflow→float — omit
 ...
 * Peer: ** 2 square (#37273), ** 3 cube (#37277), ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowFourthMulAotTest extends TestCase
{
    public function testPowFourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFourth($src, 'work', ['625', '81', '0']);
    }

    public function testBuiltinPowFourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(6), "\n";
        PHP;
        $this->assertPowFourth($src, 'work', ['1296']);
    }

    public function testPowFourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow4_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow4_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 4), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow4_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow4_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowFourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow4_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow4_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
