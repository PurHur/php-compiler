...
/**
 * AOT: ZipArchive multi addFromString keeps every entry (#35454).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_addFromString
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveMultiAddAotTest extends TestCase
{
    public function testMultiAddAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_multi_add_aot.php';
        $this->assertSame($this->runVm($src), $this->runAot($src));
    }

    public function testHelperUsesSecondEntrySlot(): void
    {
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_multi_'.getmypid().'_'.md5($src);
