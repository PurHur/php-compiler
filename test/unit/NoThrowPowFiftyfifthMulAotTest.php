 ...
 * {@code 55} → {@code (fiftyfourth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyfifthMulAotTest extends TestCase
{
    public function testPowFiftyfifthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyfifth($src, 'work', ['36028797018963968', '-36028797018963968', '0']);
    }

    public function testBuiltinPowFiftyfifth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyfifth($src, 'work', ['36028797018963968']);
    }

    public function testPowFiftyfifthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow55_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow55_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 55), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow55_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow55_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['36028797018963968'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyfifth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow55_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow55_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
