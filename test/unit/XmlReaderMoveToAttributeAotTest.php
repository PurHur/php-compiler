...
/**
 * AOT: XMLReader::moveToAttribute / moveToElement leftover of getAttribute (#35940 / #35941 / #27299).
 *
...
final class XmlReaderMoveToAttributeAotTest extends TestCase
{
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_movetoattribute_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_movetoattribute_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_movetoattribute_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_mta_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertStringContainsString("PROP_ATTR = '__xr_attr'", $user);
