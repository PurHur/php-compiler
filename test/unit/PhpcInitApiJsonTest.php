...
/**
 * phpc init --profile apijson scaffold (issue #2000).
 */
final class PhpcInitApiJsonTest extends TestCase
{
    public function testInitApiJsonProfileLintClean(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_apijson_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'apijson', $work], $work);
            ...
            $canonical = $repoRoot.'/examples/004-ApiJson';
            $this->assertSame(
            ...
            $second = $this->runPhpc(['init', '--profile', 'apijson', $work], $work);
    ...
    public function testInitRejectsApiJsonTypoProfile(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_apijson_bad_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'api-json', $work], $repoRoot);
