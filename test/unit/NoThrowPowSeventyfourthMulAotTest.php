 ...
 * {@code 74} → {@code (seventythird*n)} chained smul
 ...
 * Peer: ** 2–** 73 (#37273…/#37587), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**74} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**74 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyfourthMulAotTest extends TestCase
{
    public function testPowSeventyfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfourth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSeventyfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfourth($src, 'work', ['1']);
    }

    public function testPowSeventyfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow74_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow74_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 74), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow74_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow74_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow74_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow74_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
