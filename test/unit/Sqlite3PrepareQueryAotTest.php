...
final class Sqlite3PrepareQueryAotTest extends TestCase
{
    ...
    public function testProxyRegisteredForPrepareQuery(): void
    {
        $root = dirname(__DIR__, 2);
        $ctx = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'prepare', 'query'", $ctx);
        $this->assertStringContainsString('Sqlite3StmtMethod', $ctx);
        $this->assertStringContainsString('Sqlite3ResultMethod', $ctx);
        $dispatch = (string) file_get_contents($root.'/lib/JIT/Call/Sqlite3Method.php');
        $this->assertStringContainsString("'prepare'", $dispatch);
        $this->assertStringContainsString("'query'", $dispatch);
        ...
        $this->assertFileDoesNotExist($root.'/lib/AOT/runtime/sqlite3_prepare.c');
