 ...
 * {@code 77} → {@code (seventysixth*n)} chained smul
 ...
 * Peer: ** 2–** 76 (#37273…/#37593), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**77} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**77 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyseventhMulAotTest extends TestCase
{
    public function testPowSeventyseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyseventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSeventyseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyseventh($src, 'work', ['-1']);
    }

    public function testPowSeventyseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow77_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow77_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 77), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow77_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow77_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow77_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow77_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
