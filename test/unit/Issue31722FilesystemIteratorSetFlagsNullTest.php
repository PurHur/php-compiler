...
/**
 * FilesystemIterator::setFlags(null) — soft-null E_DEPRECATED then flags=0 (#31722).
 *
 * php-src: ext/spl/spl_directory.c — zim_FilesystemIterator_setFlags
 */
final class Issue31722FilesystemIteratorSetFlagsNullTest extends TestCase
{
    ...
    public function testVmStrictTypesSetFlagsNullTypeError(): void
    {
        ...
        $this->assertSame(
            "TypeError: FilesystemIterator::setFlags(): Argument #1 (\$flags) must be of type int, null given\n",
            $out
        );
    }
    ...
    {
        return "DEP:FilesystemIterator::setFlags(): Passing null to parameter #1 (\$flags) of type int is deprecated\n"
            ."flags=0\n";
    }
...
$tmpdir = sys_get_temp_dir() . '/phpc_31722_' . getmypid();
@mkdir($tmpdir);
...
declare(strict_types=1);
error_reporting(E_ALL);
$tmpdir = sys_get_temp_dir() . '/phpc_31722s_' . getmypid();
@mkdir($tmpdir);
file_put_contents("$tmpdir/a.txt", 'x');
try {
    $it = new FilesystemIterator($tmpdir);
...
PHP;
    ...
    private function runBin(string $bin, string $code): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_31722_');
