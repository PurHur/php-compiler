...
/**
 * AOT: ZipArchive renameIndex NestedJIT (#35473 leftover of #35450).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_renameIndex
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveRenameIndex35473AotTest extends TestCase
{
    public function testRenameIndexAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_renameindex_aot.php';
        $this->assertSame($this->runVm($src), $this->runAot($src));
    }

    public function testProxyRegisteredForRenameIndex(): void
    {
        $root = dirname(__DIR__, 2);
        $src = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'renameIndex'", $src);
        $helper = (string) file_get_contents($root.'/ext/zip/ZipArchiveJitHelper.php');
        $this->assertStringContainsString("'rename_index'", $helper);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_ri_'.getmypid().'_'.md5($src);
