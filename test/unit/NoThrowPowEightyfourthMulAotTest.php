 ...
 * {@code 84} → {@code (eightythird*n)} chained smul
 ...
 * Peer: ** 2–** 83 (#37273…/#37615), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**84} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**84 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyfourthMulAotTest extends TestCase
{
    public function testPowEightyfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfourth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowEightyfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfourth($src, 'work', ['1']);
    }

    public function testPowEightyfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow84_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow84_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 84), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow84_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow84_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow84_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow84_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
