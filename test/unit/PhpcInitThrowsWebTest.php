...
/**
 * phpc init --profile throwsweb scaffold (issue #2092).
 */
final class PhpcInitThrowsWebTest extends TestCase
{
    public function testInitThrowsWebProfileLintClean(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_throwsweb_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'throwsweb', $work], $work);
            ...
            $canonical = $repoRoot.'/examples/007-ThrowsWeb';
            $this->assertSame(
            ...
            $second = $this->runPhpc(['init', '--profile', 'throwsweb', $work], $work);
    ...
    public function testInitRejectsThrowsWebTypoProfile(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_throwsweb_bad_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'throws-web', $work], $repoRoot);
