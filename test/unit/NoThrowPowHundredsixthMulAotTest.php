 ...
 * {@code 106} → {@code (hundredfifth×n)} chained smul
 ...
 * Peer: ** 2–** 105 (#37273…/#37697), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**106} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**106 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredsixthMulAotTest extends TestCase
{
    public function testPowHundredsixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredsixth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowHundredsixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredsixth($src, 'work', ['1']);
    }

    public function testPowHundredsixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow106_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow106_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 106), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow106_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow106_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredsixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow106_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow106_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
