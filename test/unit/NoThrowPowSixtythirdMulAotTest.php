 ...
 * {@code 63} → {@code (sixtysecond*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowSixtythirdMulAotTest extends TestCase
{
    public function testPowSixtythirdOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtythird($src, 'work', ['-9223372036854775808', '0', '1', '-1']);
    }

    public function testBuiltinPowSixtythird(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-2), "\n";
        PHP;
        $this->assertPowSixtythird($src, 'work', ['-9223372036854775808']);
    }

    public function testPowSixtythirdOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow63_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow63_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-2, 63), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow63_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow63_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['-9223372036854775808'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowSixtythird(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow63_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow63_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
