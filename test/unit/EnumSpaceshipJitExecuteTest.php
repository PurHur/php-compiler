...
/**
 * MCJIT execute for enum <=> (#4849, Zend/zend_enum.c).
 *
 * @group llvm
 */
final class EnumSpaceshipJitExecuteTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (!LlvmToolchain::isReady($this->repoRoot)) {
            ...
            $this->markTestSkipped($reason.' — enum spaceship JIT execute needs LLVM (#4849)');
        }
        if (!$this->jitRuntimeProbeGreen()) {
            $this->markTestSkipped('jit-runtime-probe failed — MCJIT execute unavailable (#4849)');
        }
    }

    public function testEnumSpaceshipMcjitOutput(): void
    {
        $this->assertMcjitOutput(
            $this->phptFixtureCode('enum_spaceship_jit.phpt'),
            "1\n1\n0\n1\n"
        );
    }
