 ...
 *
 * NestedJIT/AOT bridges stay Sscanf → StringSscanfByRef / StringSscanfArray / StringVfscanf.
 * Runtime owner declares module-locally (getNamedFunction first) so leftover
 * Type empty decls cannot mint sscanf.1 / vfscanf.1 (#31894 / #32122).
 */
final class TypeDeadSscanfAbiRuntimeShrinkTest extends TestCase
{
    ...
    {
        $owner = (string) file_get_contents(__DIR__.'/../../lib/JIT/Builtin/Sscanf.php');
        $this->assertStringContainsString('#32935', $owner);
        ...
        foreach ([$byRef, $array, $vfscanf] as $src) {
            $this->assertStringContainsString('#32935', $src);
            $this->assertStringContainsString('getNamedFunction', $src);
        }
