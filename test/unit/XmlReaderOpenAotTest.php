 ...
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_xmlreader_open
 *
 * @group aot-lint
 */
final class XmlReaderOpenAotTest extends TestCase
{
    public function testVmOpen(): void
    {
        $runtime = new Runtime();
        $src = dirname(__DIR__).'/repro/xmlreader_open_aot.php';
     ...
     * @group aot
     */
    public function testAotOpenMatchVm(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/xmlreader_open_aot.php';
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_open_'.getmypid().'.bin';
    ...
    public function testOpenRegisteredInUserScriptAot(): void
    {
        $jit = (string) file_get_contents(dirname(__DIR__, 2).'/lib/JIT/XmlReaderInstanceMethodJit.php');
        $this->assertStringContainsString("'xmlreader::open' => true", $jit);
