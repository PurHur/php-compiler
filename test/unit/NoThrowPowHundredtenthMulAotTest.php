 ...
 * {@code 110} → {@code (hundredninth×n)} chained smul
 ...
 * Peer: ** 2–** 109 (#37273…/#37714), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**110} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**110 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredtenthMulAotTest extends TestCase
{
    public function testPowHundredtenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredtenth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowHundredtenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredtenth($src, 'work', ['1']);
    }

    public function testPowHundredtenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow110_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow110_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 110), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow110_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow110_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredtenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow110_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow110_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
