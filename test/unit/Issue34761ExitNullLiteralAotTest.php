 ...
 * @group aot
 */
final class Issue34761ExitNullLiteralAotTest extends TestCase
{
    ...
    public function testAotAssignedNullStillMatchesZend(): void
    {
        $src = sys_get_temp_dir().'/phpc_34761_assigned_'.getmypid().'.php';
        file_put_contents($src, "<?php \$x = null; exit(\$x);\n");
        try {
            $this->assertAotSourceMatches($src, '', 0);
        } finally {
            @unlink($src);
        }
    }
    ...
    private function assertAotMatches(string $repro, string $expectedOut, int $expectedRc): void
    {
        ...
        $this->assertAotSourceMatches($root.'/test/repro/'.$repro, $expectedOut, $expectedRc);
    }

    private function assertAotSourceMatches(string $src, string $expectedOut, int $expectedRc): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/phpc_34761_'.getmypid().'_'.md5($src).'.bin';
