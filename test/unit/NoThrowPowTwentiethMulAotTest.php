 ...
 * {@code 20} → {@code (((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n)))*((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n))))}
 ...
 * Peer: ** 2–** 19 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347/#37351/#37354),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowTwentiethMulAotTest extends TestCase
{
    public function testPowTwentiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowTwentieth($src, 'work', ['3486784401', '1048576', '0']);
    }

    public function testBuiltinPowTwentieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowTwentieth($src, 'work', ['1048576']);
    }

    public function testPowTwentiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow20_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow20_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 20), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow20_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow20_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['1048576'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowTwentieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow20_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow20_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
