 ...
 * {@code 88} → {@code (eightyseventh*n)} chained smul
 ...
 * Peer: ** 2–** 87 (#37273…/#37628), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**88} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**88 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyeighthMulAotTest extends TestCase
{
    public function testPowEightyeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyeighth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowEightyeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyeighth($src, 'work', ['1']);
    }

    public function testPowEightyeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow88_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow88_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 88), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow88_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow88_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow88_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow88_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
