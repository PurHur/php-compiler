 ...
 * {@code 70} → {@code (sixtyninth*n)} chained smul
 ...
 * Peer: ** 2–** 69 (#37273…/#37561), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**70} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**70 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventiethMulAotTest extends TestCase
{
    public function testPowSeventiethOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventieth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSeventieth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventieth($src, 'work', ['1']);
    }

    public function testPowSeventiethOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow70_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow70_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 70), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow70_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow70_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventieth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow70_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow70_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
