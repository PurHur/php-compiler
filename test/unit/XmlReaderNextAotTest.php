 ...
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_XMLReader_next
 *
 * @group aot-lint
 */
final class XmlReaderNextAotTest extends TestCase
{
    private const EXPECTED = "true|root\ntrue|a\ntrue|b\ntrue|c\ntrue|root\n";
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_next_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_next_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_next_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_next_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertStringContainsString("'xmlreader::next' => true", $jit);
        $method = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderMethod.php');
        $this->assertStringContainsString('tryNext', $method);
        $user = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderUserScript.php');
        $this->assertStringContainsString('function tryNext', $user);
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/lib/AOT/runtime/xmlreader_next.c');
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/runtime/xmlreader_next.c');
    }
}
