...
/**
 * phpc init --profile fileupload scaffold (issue #2004).
 */
final class PhpcInitFileUploadTest extends TestCase
{
    public function testInitFileUploadProfileLintClean(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_fileupload_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'fileupload', $work], $work);
            ...
            $canonical = $repoRoot.'/examples/006-FileUploadWeb';
            $this->assertSame(
            ...
            $second = $this->runPhpc(['init', '--profile', 'fileupload', $work], $work);
    ...
    public function testInitRejectsFileUploadTypoProfile(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_fileupload_bad_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', '--profile', 'file-upload', $work], $repoRoot);
