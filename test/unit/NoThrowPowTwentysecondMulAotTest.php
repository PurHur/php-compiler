 ...
 * {@code 22} → {@code ((((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n)))*$n)*(((($n*$n*$n)*($n*$n))*(($n*$n*$n)*($n*$n)))*$n))}
 ...
 * Peer: ** 2–** 21 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347/#37351/#37354/#37361/#37364),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowTwentysecondMulAotTest extends TestCase
{
    public function testPowTwentysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowTwentysecond($src, 'work', ['31381059609', '4194304', '0']);
    }

    public function testBuiltinPowTwentysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowTwentysecond($src, 'work', ['4194304']);
    }

    public function testPowTwentysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow22_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow22_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 22), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow22_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow22_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['4194304'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowTwentysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow22_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow22_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
