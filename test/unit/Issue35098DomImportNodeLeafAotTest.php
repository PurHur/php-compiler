...
/**
 * #35098 — AOT importNode(Comment/CDATA/PI) copies the leaf (not #text).
 *
 * @group llvm
 */
final class Issue35098DomImportNodeLeafAotTest extends TestCase
{
    public function testAotImportNodeLeafNodesMatchZend(): void
    {
        ...
        $this->assertStringContainsString('comment_type=8', $zend);
        $this->assertStringContainsString('comment_xml=<r><!--c--></r>', $zend);
        $this->assertStringContainsString('cdata_type=4', $zend);
        $this->assertStringContainsString('cdata_xml=<r><![CDATA[x]]></r>', $zend);
        $this->assertStringContainsString('pi_type=7', $zend);
        $this->assertStringContainsString('pi_xml=<r><?pi data?></r>', $zend);
        ...
        $this->assertSame($zend, $aot['out'], 'AOT must match Zend importNode leaf nodes');
