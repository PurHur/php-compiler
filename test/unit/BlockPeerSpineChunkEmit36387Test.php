...
/**
 * Host compile of lib/Block.php under SPINE_CHUNK — NestedJIT assign trap
 * (hashtable→native-long) is capacity-gated via method demote (#36387).
 *
 * @group aot-lint
 */
final class BlockPeerSpineChunkEmit36387Test extends TestCase
{
    public function testObjectOnlyEmitSucceedsUnderSpineChunk(): void
    {
        $root = dirname(__DIR__, 2);
        $src = $root.'/lib/Block.php';
        $this->assertFileExists($src);

        $tmp = sys_get_temp_dir().'/phpc-block-36387-'.bin2hex(random_bytes(4));
        mkdir($tmp, 0755, true);
        $entry = $tmp.'/entry.php';
        $out = $tmp.'/unit.o';
        ...
        exec($cmd, $lines, $rc);
        $this->assertSame(0, $rc, "lib/Block.php\n".implode("\n", $lines));
