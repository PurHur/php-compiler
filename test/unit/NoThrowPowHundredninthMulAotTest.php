 ...
 * {@code 109} → {@code (hundredeighth×n)} chained smul
 ...
 * Peer: ** 2–** 108 (#37273…/#37712), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**109} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**109 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredninthMulAotTest extends TestCase
{
    public function testPowHundredninthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredninth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredninth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredninth($src, 'work', ['-1']);
    }

    public function testPowHundredninthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow109_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow109_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 109), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow109_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow109_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredninth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow109_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow109_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
