 ...
 * {@code 86} → {@code (eightyfifth*n)} chained smul
 ...
 * Peer: ** 2–** 85 (#37273…/#37623), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**86} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**86 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightysixthMulAotTest extends TestCase
{
    public function testPowEightysixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightysixth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowEightysixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightysixth($src, 'work', ['1']);
    }

    public function testPowEightysixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow86_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow86_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 86), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow86_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow86_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightysixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow86_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow86_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
