 ...
 * {@code 78} → {@code (seventyseventh*n)} chained smul
 ...
 * Peer: ** 2–** 77 (#37273…/#37596), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**78} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**78 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyeighthMulAotTest extends TestCase
{
    public function testPowSeventyeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyeighth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSeventyeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyeighth($src, 'work', ['1']);
    }

    public function testPowSeventyeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow78_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow78_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 78), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow78_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow78_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow78_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow78_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
