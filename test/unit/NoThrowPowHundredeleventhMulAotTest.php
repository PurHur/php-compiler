 ...
 * {@code 111} → {@code (hundredtenth×n)} chained smul
 ...
 * Peer: ** 2–** 110 (#37273…/#37715), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**111} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**111 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredeleventhMulAotTest extends TestCase
{
    public function testPowHundredeleventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredeleventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredeleventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredeleventh($src, 'work', ['-1']);
    }

    public function testPowHundredeleventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow111_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow111_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 111), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow111_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow111_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredeleventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow111_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow111_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
