...
/**
 * AOT: ZipArchive::addGlob/addPattern NestedJIT (#35537 leftover of #35531).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_addGlob / addPattern
 *
 * @group llvm
 * @group aot
 */
...
{
    public function testAddGlobAddPatternAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_addglob_aot.php';
    ...
    public function testProxyAndHelperOpPresent(): void
    {
        ...
        $this->assertStringContainsString("'ag'", $helper);
        $this->assertStringContainsString("'ap'", $helper);
        $this->assertStringContainsString("'aps'", $helper);
        $jit = (string) file_get_contents($root.'/ext/zip/JitZipArchive.php');
        $this->assertStringContainsString('function addGlob', $jit);
        $this->assertStringContainsString('function addPattern', $jit);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_ag_'.getmypid().'_'.md5($src);
