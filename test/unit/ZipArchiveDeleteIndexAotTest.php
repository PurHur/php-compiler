...
/**
 * AOT: ZipArchive deleteIndex NestedJIT (#35455 leftover of #35450).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_deleteIndex
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveDeleteIndexAotTest extends TestCase
{
    public function testDeleteIndexAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_deleteindex_aot.php';
        $this->assertSame($this->runVm($src), $this->runAot($src));
    }

    public function testProxyRegisteredForDeleteIndex(): void
    {
        $root = dirname(__DIR__, 2);
        $src = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'deleteIndex'", $src);
        $helper = (string) file_get_contents($root.'/ext/zip/ZipArchiveJitHelper.php');
        $this->assertStringContainsString("'delete_index'", $helper);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_di_'.getmypid().'_'.md5($src);
