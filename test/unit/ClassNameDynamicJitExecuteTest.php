...
/**
 * MCJIT execute for `$object::class` and literal `Class::class` (#4954).
 *
 * @group llvm
 */
final class ClassNameDynamicJitExecuteTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        if (!LlvmToolchain::isReady($this->repoRoot)) {
            ...
            $this->markTestSkipped($reason.' — class name dynamic JIT execute needs LLVM (#4954)');
        }
        if (!$this->jitRuntimeProbeOk()) {
            $this->markTestSkipped('jit-runtime-probe failed — MCJIT execute unavailable (#4954)');
        }
    }

    public function testDynamicAndLiteralClassPseudoConstMatchesVm(): void
    {
        ...
        $script = $this->repoRoot.'/var/class_name_dynamic_jit_execute.php';
...
PHP);
        ...
        $this->assertSame($expected, $jitOut['stdout']);
    }
