 ...
 * {@code 12} → {@code (($n*$n*$n)*($n*$n*$n))*(($n*$n*$n)*($n*$n*$n))} with
 ...
 * Peer: ** 2–** 11 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowTwelfthMulAotTest extends TestCase
{
    public function testPowTwelfthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowTwelfth($src, 'work', ['531441', '4096', '0']);
    }

    public function testBuiltinPowTwelfth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowTwelfth($src, 'work', ['4096']);
    }

    public function testPowTwelfthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow12_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow12_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 12), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow12_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow12_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowTwelfth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow12_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow12_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
