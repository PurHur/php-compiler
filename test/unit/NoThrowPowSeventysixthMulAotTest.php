 ...
 * {@code 76} → {@code (seventyfifth*n)} chained smul
 ...
 * Peer: ** 2–** 75 (#37273…/#37590), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**76} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**76 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventysixthMulAotTest extends TestCase
{
    public function testPowSeventysixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventysixth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSeventysixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventysixth($src, 'work', ['1']);
    }

    public function testPowSeventysixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow76_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow76_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 76), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow76_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow76_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventysixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow76_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow76_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
