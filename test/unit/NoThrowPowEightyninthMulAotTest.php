 ...
 * {@code 89} → {@code (eightyeighth*n)} chained smul
 ...
 * Peer: ** 2–** 88 (#37273…/#37631), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**89} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**89 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyninthMulAotTest extends TestCase
{
    public function testPowEightyninthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyninth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowEightyninth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyninth($src, 'work', ['-1']);
    }

    public function testPowEightyninthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow89_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow89_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 89), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow89_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow89_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyninth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow89_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow89_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
