 ...
 * {@code 103} → {@code (hundredsecond×n)} chained smul
 ...
 * Peer: ** 2–** 102 (#37273…/#37688), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**103} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**103 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredthirdMulAotTest extends TestCase
{
    public function testPowHundredthirdOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredthird($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredthird(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredthird($src, 'work', ['-1']);
    }

    public function testPowHundredthirdOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow103_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow103_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 103), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow103_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow103_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredthird(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow103_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow103_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
