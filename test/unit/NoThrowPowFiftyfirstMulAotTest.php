 ...
 * {@code 51} → {@code (fiftieth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyfirstMulAotTest extends TestCase
{
    public function testPowFiftyfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyfirst($src, 'work', ['2251799813685248', '-2251799813685248', '0']);
    }

    public function testBuiltinPowFiftyfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyfirst($src, 'work', ['2251799813685248']);
    }

    public function testPowFiftyfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow51_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow51_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 51), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow51_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow51_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['2251799813685248'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow51_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow51_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
