 ...
 * {@code 7} → {@code (($n*$n*$n)*($n*$n*$n))*$n} with chained smul
 ...
 * Peer: ** 2–** 6 (#37273/#37277/#37278/#37281/#37283), ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowSeventhMulAotTest extends TestCase
{
    public function testPowSeventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowSeventh($src, 'work', ['2187', '-128', '0']);
    }

    public function testBuiltinPowSeventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowSeventh($src, 'work', ['128']);
    }

    public function testPowSeventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow7_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow7_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 7), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow7_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow7_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow7_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow7_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
