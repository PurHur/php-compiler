...
/**
 * AOT: mb_convert_case(TITLE) illegal UTF-8 → `?`, not U+0080 (#34344).
 *
...
final class MbConvertCaseTitleIllegalUtf8AotTest extends TestCase
{
    ...
    public function testHelperKeepsIllegalUtf8Substitute(): void
    {
    ...
    public function testExistingTitleRuntimeAotStillMatchesZend(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/mb_convert_title_illegal_'.getmypid().'_'.md5($src);
