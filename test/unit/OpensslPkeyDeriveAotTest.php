...
final class OpensslPkeyDeriveAotTest extends TestCase
{
    ...
    public function testCallNoLongerThrowsJitUnimplemented(): void
    {
        ...
        $this->assertStringContainsString('isCompileTimeDeriveScalarSoftFail', (string) file_get_contents(
    ...
    /**
     * Residual of #32852: compile-time null/bool/int/float soft-fail to false (#26689), not LogicException.
     *
     * @group llvm
     * @group aot
     */
    public function testAotSoftFailScalarsMatchVm(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }
        ...
        $root = dirname(__DIR__, 2);
        $src = $root.'/test/repro/issue_32852_openssl_pkey_derive_softfail_aot.php';
        ...
        $this->assertNotFalse($code);
        ob_start();
        $runtime->run($runtime->parseAndCompile($code, 'issue_32852_openssl_pkey_derive_softfail_aot.php'));
        $vmOut = (string) ob_get_clean();
        $this->assertSame($expected, $vmOut);

        $bin = sys_get_temp_dir().'/phpc_ossl_pkey_derive_sf_'.getmypid().'.bin';
        $compile = 'env PHP_COMPILER_HELPER_RUNTIME_O=1 '.escapeshellarg(PHP_BINARY).' '
            .escapeshellarg($root.'/bin/compile.php')
            .' -o '.escapeshellarg($bin).' '.escapeshellarg($src).' 2>&1';
        exec($compile, $compileOut, $compileRc);
