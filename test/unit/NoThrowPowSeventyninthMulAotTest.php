 ...
 * {@code 79} → {@code (seventyeighth*n)} chained smul
 ...
 * Peer: ** 2–** 78 (#37273…/#37599), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**79} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**79 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyninthMulAotTest extends TestCase
{
    public function testPowSeventyninthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyninth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSeventyninth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyninth($src, 'work', ['-1']);
    }

    public function testPowSeventyninthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow79_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow79_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 79), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow79_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow79_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyninth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow79_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow79_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
