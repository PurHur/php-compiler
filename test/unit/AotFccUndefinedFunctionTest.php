...
/**
 * AOT: undefined-function FCC must raise catchable Error with preserved case (#27106).
 *
 * @group llvm
 * @group aot
 */
final class AotFccUndefinedFunctionTest extends TestCase
{
    public function testAotUndefinedFunctionFccErrorMatchesVm(): void
    {
        $root = dirname(__DIR__, 2);
        $source = $root.'/test/repro/maintainer_gap_fcc_undefined_case.php';
        $vmOut = $this->runCommand([PHP_BINARY, $root.'/bin/vm.php', $source], $root);
        $this->assertSame("Call to undefined function FooBar()\n", $vmOut);

        $out = $root.'/build/test-aot-fcc-undefined-function';
