 ...
 * {@code 15} → {@code ((((($n*$n*$n)*($n*$n*$n))*$n)*((($n*$n*$n)*($n*$n*$n))*$n))*$n)}
 ...
 * Peer: ** 2–** 14 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowFifteenthMulAotTest extends TestCase
{
    public function testPowFifteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFifteenth($src, 'work', ['14348907', '-32768', '0']);
    }

    public function testBuiltinPowFifteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFifteenth($src, 'work', ['32768']);
    }

    public function testPowFifteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow15_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow15_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 15), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow15_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow15_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowFifteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow15_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow15_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
