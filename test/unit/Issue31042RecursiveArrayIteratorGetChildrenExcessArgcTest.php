...
final class Issue31042RecursiveArrayIteratorGetChildrenExcessArgcTest extends TestCase
{
    ...
    {
        $code = file_get_contents(__DIR__.'/../repro/issue_31042_recursivearrayiterator_getchildren_excess_argc.php');
        $this->assertNotFalse($code);
        $rt = new Runtime();
        $block = $rt->parseAndCompile($code, 'issue_31042_recursivearrayiterator_getchildren_excess_argc.php');
        ob_start();
        $rt->run($block);
        $out = ob_get_clean();
        ...
        $this->assertStringContainsString('hasChildren_ok: OK', $out);
        $this->assertStringContainsString('getChildren_ok: OK', $out);
