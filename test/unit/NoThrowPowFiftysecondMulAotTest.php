 ...
 * {@code 52} → {@code (fiftyfirst*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftysecondMulAotTest extends TestCase
{
    public function testPowFiftysecondOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftysecond($src, 'work', ['4503599627370496', '4503599627370496', '0']);
    }

    public function testBuiltinPowFiftysecond(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftysecond($src, 'work', ['4503599627370496']);
    }

    public function testPowFiftysecondOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow52_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow52_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 52), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow52_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow52_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['4503599627370496'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftysecond(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow52_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow52_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
