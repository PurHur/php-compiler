 ...
 * {@code 18} → {@code (((($n*$n*$n)*($n*$n*$n))*($n*$n*$n))*((($n*$n*$n)*($n*$n*$n))*($n*$n*$n)))}
 ...
 * Peer: ** 2–** 17 (#37273/#37277/#37278/#37281/#37283/#37287/#37291/#37326/#37330/#37335/#37336/#37339/#37343/#37344/#37345/#37347),
 * ** 0 / ** 1 (#37269).
 *
 * @group aot-lint
 */
final class NoThrowPowEighteenthMulAotTest extends TestCase
{
    public function testPowEighteenthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowEighteenth($src, 'work', ['387420489', '262144', '0']);
    }

    public function testBuiltinPowEighteenth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowEighteenth($src, 'work', ['262144']);
    }

    public function testPowEighteenthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow18_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow18_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 18), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow18_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow18_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEighteenth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow18_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow18_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
