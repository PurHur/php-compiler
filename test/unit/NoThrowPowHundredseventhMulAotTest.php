 ...
 * {@code 107} → {@code (hundredsixth×n)} chained smul
 ...
 * Peer: ** 2–** 106 (#37273…/#37703), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**107} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**107 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredseventhMulAotTest extends TestCase
{
    public function testPowHundredseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredseventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowHundredseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredseventh($src, 'work', ['-1']);
    }

    public function testPowHundredseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow107_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow107_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 107), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow107_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow107_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow107_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow107_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
