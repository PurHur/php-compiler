 ...
 * {@code 81} → {@code (eightieth*n)} chained smul
 ...
 * Peer: ** 2–** 80 (#37273…/#37607), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**81} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**81 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyfirstMulAotTest extends TestCase
{
    public function testPowEightyfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfirst($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowEightyfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfirst($src, 'work', ['-1']);
    }

    public function testPowEightyfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow81_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow81_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 81), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow81_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow81_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow81_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow81_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
