...
final class BootstrapGen0ChunksOrchestratorTest extends TestCase
{
    ...
    public function testChunkPlanLibAndExtCarryWavesAndAutoload(): void
    {
        ...
        foreach ($plan['chunks'] as $chunk) {
            ...
            // In-repo entries-dir: depth-correct __DIR__ relative. Outside the repo (/tmp):
            // absolute require_once (relative ../../../vendor would be wrong) (#36387).
            $this->assertTrue(
                (bool) preg_match("#require_once __DIR__ \\. '/(\\.\\./)+vendor/autoload\\.php';#", $body)
                || str_contains($body, "require_once '".$root.'/vendor/autoload.php'."';")
                || str_contains($body, 'require_once '.var_export($root.'/vendor/autoload.php', true).';'),
                "autoload require missing in {$chunk['chunk_id']}:\n".$body
            );
    ...
    public function testChunkPlanDefersOnlyMeasuredPostDemoteFails(): void
    {
        $root = dirname(__DIR__, 2);
        $script = $root.'/script/bootstrap-gen0-chunk-plan.php';
        ...
        mkdir($tmp.'/entries', 0755, true);
        $planPath = $tmp.'/plan.json';
        // Demote-covered oversize hubs emit; VmDateTimeNative stays deferred (host CFG null Op).
        // SoapClientState::$cacheWsdl uses a literal so soap is emit-eligible (#36387).
        $cmd = escapeshellarg(PHP_BINARY).' '.escapeshellarg($script)
        ...
        foreach ($plan['chunks'] as $chunk) {
            ...
            if (!empty($chunk['deferred'])) {
                ...
                $this->assertSame('host_cfg_null_op', $chunk['defer_reason'] ?? '');
                $deferredPaths[] = (string) ($chunk['defer_path'] ?? '');
            }
        }
        $this->assertSame(['ext/standard/VmDateTimeNative.php'], $deferredPaths);
        $this->removeTree($tmp);
    }
