...
/**
 * Host compile of FinalClassConstCheck under SPINE_CHUNK — sparse cfgChildren
 * must not TypeError in prelude / sibling-inline walks (#36387).
 *
 * @group aot-lint
 */
final class FinalClassConstCheckSpineChunkEmit36387Test extends TestCase
{
    public function testObjectOnlyEmitSucceedsUnderSpineChunk(): void
    {
        $root = dirname(__DIR__, 2);
        $src = $root.'/lib/Compiler/FinalClassConstCheck.php';
        $this->assertFileExists($src);

        $tmp = sys_get_temp_dir().'/phpc-fcc-36387-'.bin2hex(random_bytes(4));
        mkdir($tmp, 0755, true);
        $entry = $tmp.'/entry.php';
        $out = $tmp.'/fcc.o';
