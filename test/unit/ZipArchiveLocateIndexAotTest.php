...
/**
 * AOT: ZipArchive getNameIndex NestedJIT (#35440 leftover of #35437).
 *
 * @see php-src ext/zip/php_zip.c zim_ZipArchive_getNameIndex
 *
 * @group llvm
 * @group aot
 */
final class ZipArchiveLocateIndexAotTest extends TestCase
{
    public function testGetNameIndexAotMatchesVm(): void
    {
        ...
        $src = __DIR__.'/../repro/ziparchive_locate_index_aot.php';
        $this->assertSame($this->runVm($src), $this->runAot($src));
    }

    public function testProxyRegisteredForGetNameIndex(): void
    {
        $root = dirname(__DIR__, 2);
        $src = (string) file_get_contents($root.'/lib/JIT/Context.php');
        $this->assertStringContainsString("'getNameIndex'", $src);
        $helper = (string) file_get_contents($root.'/ext/zip/ZipArchiveJitHelper.php');
        $this->assertStringContainsString("'name_index'", $helper);
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/zip_gni_'.getmypid().'_'.md5($src);
