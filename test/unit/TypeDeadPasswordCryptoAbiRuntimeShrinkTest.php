...
/**
 * Drop leftover always-on password_hash/verify/crypt ABI shells from Builtin\Type (#32855).
 *
 * NestedJIT/AOT bridges stay PasswordCryptoRuntime (peer #32851 libcrypt/random_bytes).
...
final class TypeDeadPasswordCryptoAbiRuntimeShrinkTest extends TestCase
{
    ...
    public function testPasswordCryptoRuntimeDeclaresAbisModuleLocally(): void
    {
        $runtime = (string) file_get_contents(__DIR__.'/../../lib/JIT/Builtin/PasswordCryptoRuntime.php');
        $this->assertStringContainsString('#32855', $runtime);
        $this->assertStringContainsString('getNamedFunction($name)', $runtime);
        $this->assertStringContainsString('module->addFunction($name', $runtime);
        $this->assertStringNotContainsString('lookupFunction($name)', $runtime);
        foreach ($this->droppedAbis() as $sym) {
            $this->assertStringContainsString($sym, $runtime);
        }
    }

    public function testTypeInitializeStillEnsureLinksPasswordCryptoOwner(): void
    {
        $type = (string) file_get_contents(__DIR__.'/../../lib/JIT/Builtin/Type.php');
    ...
    public function testPhpHelpersRemainForDroppedUserScriptBuiltins(): void
    {
        ...
        $this->assertFileExists(__DIR__.'/../../lib/JIT/Builtin/StringPasswordCrypto.php');
        $this->assertFileDoesNotExist(
            dirname(__DIR__, 2).'/lib/AOT/runtime/phpc_password_hash.c'
        );
        $this->assertFileDoesNotExist(
            dirname(__DIR__, 2).'/lib/AOT/runtime/phpc_password_crypto.c'
        );
    }
}
