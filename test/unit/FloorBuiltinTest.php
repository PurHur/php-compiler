...
/**
 * floor() VM/AOT smoke (#27004 — AOT must not return 0 via VmMath recursion).
 *
 * Multi-call UnaryMinus-arg sequences can still mis-wire under AOT after a prior
 * floor() (second negative literal → 0); same class as fmod (#15736 / #26994).
 * Single-call negatives are covered separately below.
 */
final class FloorBuiltinTest extends TestCase
{
...
PHP;
    ...
    private function runAotBinary(string $code): string
    {
        ...
        $tmp = tempnam(sys_get_temp_dir(), 'phpc_floor_');
        $out = $tmp.'_bin';
