 ...
 * {@code 75} → {@code (seventyfourth*n)} chained smul
 ...
 * Peer: ** 2–** 74 (#37273…/#37589), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**75} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**75 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSeventyfifthMulAotTest extends TestCase
{
    public function testPowSeventyfifthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfifth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSeventyfifth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSeventyfifth($src, 'work', ['-1']);
    }

    public function testPowSeventyfifthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow75_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow75_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 75), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow75_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow75_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSeventyfifth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow75_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow75_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
