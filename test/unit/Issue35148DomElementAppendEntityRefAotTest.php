...
/**
 * #35148 — AOT Element::appendChild(createEntityReference) matches Zend (no SIGSEGV).
 *
 * @group llvm
 */
final class Issue35148DomElementAppendEntityRefAotTest extends TestCase
{
    public function testAotElementAppendEntityRefMatchesZend(): void
    {
        $src = dirname(__DIR__).'/repro/issue_dom_element_append_entityref_aot.php';
        $this->assertFileExists($src);

        $zend = $this->runPhp($src);
        $vm = $this->runVm($src);
        $this->assertSame($zend, $vm, 'VM must match Zend');
        $this->assertStringContainsString('type=5', $zend);
        $this->assertStringContainsString('xml=<r>&amp;</r>', $zend);

        $bin = sys_get_temp_dir().'/phpc_35148_'.getmypid().'.bin';
        $compile = $this->runCompile($src, $bin);
        $this->assertSame(0, $compile['code'], "AOT compile failed:\n".$compile['out']);
        $aot = $this->runBin($bin);
        @unlink($bin);

        $this->assertSame($zend, $aot['out'], 'AOT must match Zend Element appendChild(EntityReference)');
