 ...
 * {@code 57} → {@code (fiftysixth*n)} chained smul
 ...
 * @group aot-lint
 */
final class NoThrowPowFiftyseventhMulAotTest extends TestCase
{
    public function testPowFiftyseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(0), "\n";
        PHP;
        $this->assertPowFiftyseventh($src, 'work', ['144115188075855872', '-144115188075855872', '0']);
    }

    public function testBuiltinPowFiftyseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(2), "\n";
        PHP;
        $this->assertPowFiftyseventh($src, 'work', ['144115188075855872']);
    }

    public function testPowFiftyseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo gettype($v), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow57_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow57_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(2, 57), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow57_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow57_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
            ...
            exec(escapeshellarg($bin), $runOut, $runRc);
            $this->assertSame(0, $runRc);
            $this->assertSame(['144115188075855872'], $runOut);
        } finally {
            putenv('PHP_COMPILER_DUMP_IR');
            putenv('PHP_COMPILER_CACHE');
            @unlink($path);
     ...
     */
    private function assertPowFiftyseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow57_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow57_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
