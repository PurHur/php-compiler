...
/**
 * Host compile of units that NestedJIT segfaulted / OOMed under SPINE_CHUNK before
 * ext\* + top-level Compiler/VM/JIT demote (#36387).
 *
 * @group aot-lint
 */
final class ExtAndMegaHubSpineChunkEmit36387Test extends TestCase
{
    /**
     * @return list<string>
     */
    ...
    {
        return [
            // bcmath class/JIT pack members that sat in the rc=139 batch.
    ...
    public function testObjectOnlyEmitSucceedsUnderSpineChunk(string $rel): void
    {
        ...
        $tmp = sys_get_temp_dir().'/phpc-ext-mega-36387-'.bin2hex(random_bytes(4));
        mkdir($tmp, 0755, true);
        $entry = $tmp.'/entry.php';
        $out = $tmp.'/unit.o';
