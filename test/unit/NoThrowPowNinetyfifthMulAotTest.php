 ...
 * {@code 95} → {@code (ninetyfourth×n)} chained smul
 ...
 * Note: {@code (±2)**95} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**95 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetyfifthMulAotTest extends TestCase
{
    public function testPowNinetyfifthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyfifth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowNinetyfifth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyfifth($src, 'work', ['-1']);
    }

    public function testPowNinetyfifthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow95_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow95_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 95), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow95_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow95_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetyfifth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow95_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow95_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
