...
/**
 * AOT: XMLReader::getAttributeNs leftover of getAttribute (#35925 / #35918 / #27299).
 *
 * @see php-src ext/xmlreader/php_xmlreader.c zim_XMLReader_getAttributeNs
 *
 * @group aot-lint
 */
final class XmlReaderGetAttributeNsAotTest extends TestCase
{
    private const EXPECTED = "'42'\nNULL\n";
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_getattributens_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_getattributens_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $src = $root.'/test/repro/xmlreader_getattributens_aot.php';
        $vm = [];
        exec(
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_gans_'.getmypid().'.bin';
    ...
    public function testRegisteredInUserScriptAot(): void
    {
        ...
        $this->assertStringContainsString("'xmlreader::getattributens' => true", $jit);
        $method = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderMethod.php');
        $this->assertStringContainsString('tryGetAttributeNs', $method);
        $user = (string) file_get_contents(dirname(__DIR__, 2).'/ext/xmlreader/JitXmlReaderUserScript.php');
        $this->assertStringContainsString('function tryGetAttributeNs', $user);
        $this->assertFileDoesNotExist(dirname(__DIR__, 2).'/lib/AOT/runtime/xmlreader_get_attribute_ns.c');
    }
}
