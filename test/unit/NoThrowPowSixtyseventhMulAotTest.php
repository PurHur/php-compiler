 ...
 * {@code 67} → {@code (sixtysixth*n)} chained smul
 ...
 * Peer: ** 2–** 66 (#37273…/#37543), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**67} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**67 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowSixtyseventhMulAotTest extends TestCase
{
    public function testPowSixtyseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyseventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowSixtyseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtyseventh($src, 'work', ['-1']);
    }

    public function testPowSixtyseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow67_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow67_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 67), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow67_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow67_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixtyseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow67_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow67_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
