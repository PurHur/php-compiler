...
/**
 * AOT: SQLite3 escapeString NestedJIT (#35977 leftover of #35972).
 *
 * @see php-src ext/sqlite3/sqlite3.c zim_SQLite3_escapeString
 *
 * @group llvm
 * @group aot
 */
final class Sqlite3EscapeStringAotTest extends TestCase
{
    public function testEscapeStringAotMatchesVm(): void
    {
        if (!LlvmToolchain::hasLibrary(dirname(__DIR__, 2))) {
            $this->markTestSkipped('LLVM 9 toolchain not available');
        }

        $src = __DIR__.'/../repro/sqlite3_escapestring_aot.php';
    ...
    public function testProxyRegisteredForEscapeString(): void
    {
        ...
        $this->assertStringContainsString('function escapeString(', $jit);
        $this->assertStringContainsString('#35977', $jit);
        $this->assertFileDoesNotExist($root.'/lib/AOT/runtime/sqlite3_escapestring.c');
        $this->assertFileDoesNotExist($root.'/runtime/sqlite3_escapestring.c');
    }
    ...
    {
        $root = dirname(__DIR__, 2);
        $bin = sys_get_temp_dir().'/sq3_es_'.getmypid().'_'.md5($src);
