...
/**
 * phpc init scaffold (issue #312).
 */
final class PhpcInitTest extends TestCase
{
    public function testInitCreatesLintCleanProject(): void
    {
        $repoRoot = dirname(__DIR__, 2);
        $work = sys_get_temp_dir().'/phpc_init_'.bin2hex(random_bytes(6));
        $this->assertTrue(mkdir($work));

        try {
            $init = $this->runPhpc(['init', $work], $work);
            $this->assertSame(0, $init['exit'], $init['stderr']);
            $this->assertFileExists($work.'/phpc.json');
            $this->assertFileExists($work.'/public/index.php');
            $this->assertFileExists($work.'/README.md');
            ...
            $this->assertStringContainsString('Hello', $run['stdout']);

            $second = $this->runPhpc(['init', $work], $work);
            $this->assertNotSame(0, $second['exit']);
            $this->assertStringContainsString('--force', $second['stderr']);

            $force = $this->runPhpc(['init', '--force', $work], $work);
            $this->assertSame(0, $force['exit'], $force['stderr']);
        } finally {
