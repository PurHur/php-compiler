...
{
    private const EXPECTED = "DOMElement:r\n";
    ...
    public function testVm(): void
    {
        ...
        try {
            $runtime = new Runtime();
            $code = file_get_contents(dirname(__DIR__).'/repro/xmlreader_expand_aot.php');
            $this->assertNotFalse($code);
            ob_start();
            $runtime->run($runtime->parseAndCompile($code, 'xmlreader_expand_aot.php'));
    ...
    public function testAotMatchesVm(): void
    {
        ...
        $bin = sys_get_temp_dir().'/phpc_xr_exp_'.getmypid().'.bin';
