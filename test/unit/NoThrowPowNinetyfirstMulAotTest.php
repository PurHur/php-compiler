 ...
 * {@code 91} → {@code (ninetieth*n)} chained smul
 ...
 * Peer: ** 2–** 90 (#37273…/#37641), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**91} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**91 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetyfirstMulAotTest extends TestCase
{
    public function testPowNinetyfirstOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyfirst($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowNinetyfirst(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyfirst($src, 'work', ['-1']);
    }

    public function testPowNinetyfirstOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow91_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow91_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 91), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow91_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow91_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetyfirst(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow91_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow91_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
