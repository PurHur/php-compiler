 ...
 * {@code 85} → {@code (eightyfourth*n)} chained smul
 ...
 * Peer: ** 2–** 84 (#37273…/#37618), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**85} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**85 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowEightyfifthMulAotTest extends TestCase
{
    public function testPowEightyfifthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfifth($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowEightyfifth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowEightyfifth($src, 'work', ['-1']);
    }

    public function testPowEightyfifthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow85_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow85_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 85), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow85_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow85_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowEightyfifth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow85_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow85_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
