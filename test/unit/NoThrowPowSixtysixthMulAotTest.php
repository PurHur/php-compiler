 ...
 * {@code 66} → {@code (sixtyfifth*n)} chained smul
 ...
 * Peer: ** 2–** 65 (#37273…/#37537), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**66} and larger magnitudes promote to float (signed i64).
 *
 * @group aot-lint
 */
final class NoThrowPowSixtysixthMulAotTest extends TestCase
{
    public function testPowSixtysixthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtysixth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowSixtysixth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowSixtysixth($src, 'work', ['1']);
    }

    public function testPowSixtysixthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow66_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow66_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 66), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow66_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow66_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowSixtysixth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow66_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow66_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
