 ...
 * {@code 104} → {@code (hundredthird×n)} chained smul
 ...
 * Peer: ** 2–** 103 (#37273…/#37691), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**104} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**104 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredfourthMulAotTest extends TestCase
{
    public function testPowHundredfourthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfourth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowHundredfourth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredfourth($src, 'work', ['1']);
    }

    public function testPowHundredfourthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow104_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow104_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 104), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow104_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow104_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredfourth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow104_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow104_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
