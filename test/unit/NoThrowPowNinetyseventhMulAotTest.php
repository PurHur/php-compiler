 ...
 * {@code 97} → {@code (ninetysixth×n)} chained smul
 ...
 * Peer: ** 2–** 96 (#37273…/#37665), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**97} and larger magnitudes promote to float (signed i64).
 * Odd exponent: {@code (-1)**97 === -1}.
 *
 * @group aot-lint
 */
final class NoThrowPowNinetyseventhMulAotTest extends TestCase
{
    public function testPowNinetyseventhOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyseventh($src, 'work', ['0', '1', '-1']);
    }

    public function testBuiltinPowNinetyseventh(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowNinetyseventh($src, 'work', ['-1']);
    }

    public function testPowNinetyseventhOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow97_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow97_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 97), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow97_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow97_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowNinetyseventh(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow97_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow97_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
