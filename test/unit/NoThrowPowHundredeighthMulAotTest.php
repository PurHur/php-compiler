 ...
 * {@code 108} → {@code (hundredseventh×n)} chained smul
 ...
 * Peer: ** 2–** 107 (#37273…/#37709), ** 0 / ** 1 (#37269).
 * Note: {@code (±2)**108} and larger magnitudes promote to float (signed i64).
 * Even exponent: {@code (-1)**108 === 1}.
 *
 * @group aot-lint
 */
final class NoThrowPowHundredeighthMulAotTest extends TestCase
{
    public function testPowHundredeighthOmitsFpowUsesSmul(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredeighth($src, 'work', ['0', '1', '1']);
    }

    public function testBuiltinPowHundredeighth(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        echo work(-1), "\n";
        PHP;
        $this->assertPowHundredeighth($src, 'work', ['1']);
    }

    public function testPowHundredeighthOverflowPromotesToFloat(): void
    {
        $src = <<<'PHP'
        <?php
        declare(strict_types=1);
        ...
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow108_ov_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow108_ov_'.getmypid().'.bin';
    ...
    public function testRuntimeExponentKeepsPow(): void
    {
        ...
        echo work(-1, 108), "\n";
        PHP;
        $path = sys_get_temp_dir().'/phpc_pow108_rt_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow108_rt_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
     ...
     */
    private function assertPowHundredeighth(string $src, string $fn, array $expectedRun): void
    {
        $path = sys_get_temp_dir().'/phpc_pow108_'.getmypid().'.php';
        $bin = sys_get_temp_dir().'/phpc_pow108_'.getmypid().'.bin';
        file_put_contents($path, $src);
        try {
            putenv('PHP_COMPILER_DUMP_IR=1');
            putenv('PHP_COMPILER_CACHE=0');
