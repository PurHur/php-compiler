...
if (!function_exists('openssl_dh_compute_key')) {
    fwrite(STDERR, "MISSING: openssl_dh_compute_key\n");
    exit(1);
}
...
PEM);
if (false === $private) {
    fwrite(STDERR, "openssl_pkey_get_private failed\n");
    exit(1);
}
...
$shared = openssl_dh_compute_key($peerPub, $private);
if (false === $shared) {
    fwrite(STDERR, "openssl_dh_compute_key returned false\n");
    exit(1);
}
if ($shared !== $expected) {
    fwrite(STDERR, 'shared secret mismatch: '.bin2hex($shared)."\n");
    exit(1);
}
