...
/**
 * Repro #26715 — ssh2_forward_listen/accept registration + optional live smoke.
 */
...
$code = <<<'PHP'
<?php
echo 'ssh2_forward_listen=', function_exists('ssh2_forward_listen') ? 1 : 0, "\n";
echo 'ssh2_forward_accept=', function_exists('ssh2_forward_accept') ? 1 : 0, "\n";
...
echo 'listen_is_object=', is_object($listener) ? 1 : 0, "\n";
echo 'listen_class=', is_object($listener) ? get_class($listener) : '', "\n";
ssh2_disconnect($session);
echo "ok\n";
PHP;

$tmp = sys_get_temp_dir() . '/phpc_ssh2_forward_' . getmypid() . '.php';
