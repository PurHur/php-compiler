...
$server->addSoapHeader(new SoapHeader('http://example.com/meta', 'Stamp', 'v1', false));
...
$server2 = new SoapServer(null, ['uri' => 'http://example.com/echo']);
$server2->setPersistence(SOAP_PERSISTENCE_REQUEST);
$server2->setClass('CounterSvc');
ob_start();
$server2->handle($req);
$a = ob_get_clean();
ob_start();
$server2->handle($req);
$b = ob_get_clean();
echo 'req_mode=', (is_string($a) && is_string($b) && str_contains($a, '>1</') && str_contains($b, '>1</')) ? 1 : 0, "\n";
