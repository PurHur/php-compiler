...
/**
 * M5 HelloWorld compile driver native entry (#2681).
 *
 * Same emit spine as helloworld_m3_emit_native_entry.php; link with
 * PHP_COMPILER_M3_EMIT_LOG_PREFIX=helloworld_compile_smoke so logs match
 * test/selfhost/compiler_helloworld_smoke/compile_driver.php (not compile_smoke_m3_emit).
 *
...
if (!is_string($sourceFile) || '' === $sourceFile || !is_string($outFile) || '' === $outFile) {
    echo "helloworld_compile_smoke: set PHP_COMPILER_M3_SOURCE and PHP_COMPILER_M3_OUT\n";
    exit(1);
}
