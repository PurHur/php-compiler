...
/**
 * Native M3 compiler unit probe emit binary (issue #2618).
 *
 * Same emit spine as compile_smoke_m3_emit_native_entry.php; link-time sidecar targets
 * test/selfhost/compiler_unit_probe/compiler_unit_probe_compile.php.
 *
 *   PHP_COMPILER_M3_SOURCE=test/selfhost/compiler_unit_probe/compiler_unit_probe_compile.php \
 *   PHP_COMPILER_M3_OUT=build/compiler-unit-probe-aot ./build/selfhost-compiler-unit-probe-emit
 */
...
if (!is_string($sourceFile) || '' === $sourceFile || !is_string($outFile) || '' === $outFile) {
    echo "compiler_unit_probe_m3_emit_entry: set PHP_COMPILER_M3_SOURCE and PHP_COMPILER_M3_OUT\n";
    exit(1);
}
