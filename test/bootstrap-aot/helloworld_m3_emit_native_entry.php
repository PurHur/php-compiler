...
/**
 * Native M3 HelloWorld emit binary (issues #1056, #1493, #1768).
 *
 * Same emit spine as compile_smoke_m3_emit_native_entry.php: Runtime parseAndCompile + standalone
 * without bundling helloworld_compile_smoke FUNCDEF (LLVM 9 link crash on compile_driver bundle).
 *
 *   PHP_COMPILER_M3_SOURCE=examples/000-HelloWorld/example.php PHP_COMPILER_M3_OUT=… ./build/selfhost-helloworld-emit
 */
...
if (!is_string($sourceFile) || '' === $sourceFile || !is_string($outFile) || '' === $outFile) {
    echo "helloworld_m3_emit_entry: set PHP_COMPILER_M3_SOURCE and PHP_COMPILER_M3_OUT\n";
    exit(1);
}
