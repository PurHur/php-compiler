...
$m3Source = getenv('PHP_COMPILER_M3_SOURCE');
$m3Out = getenv('PHP_COMPILER_M3_OUT');
if (is_string($m3Source) && '' !== $m3Source && is_string($m3Out) && '' !== $m3Out) {
    echo "helloworld_compile_smoke: native driver not in selfhost bundle yet (NEXT_LOWER: parseAndCompile under PHP_COMPILER_SELFHOST_AOT)\n";
    exit(1);
}
