...
/**
 * M3 compile-smoke native compile driver (issues #1056, #1937, #1977).
 * Gate: php bin/compile.php -l test/selfhost/compiler_compile_smoke/compile_driver.php
 *
 ...
 *   PHP_COMPILER_M3_SOURCE=… PHP_COMPILER_M3_OUT=… ./build/selfhost-compile-smoke-compile
 */
...
if ('compile' === (string) getenv('PHP_COMPILER_M3_COMPILE_MODE')) {
    ...
    if ('1' !== (string) getenv('PHP_COMPILER_M3_RUNTIME_COMPILE')) {
        echo "compile_smoke_probe: emit path blocked (gate: set BOOTSTRAP_M3_RUNTIME_COMPILE=1 and PHP_COMPILER_M3_RUNTIME_COMPILE=1)\n";
        exit(1);
    }
    ...
    if (!is_string($sourceFile) || '' === $sourceFile || !is_string($outFile) || '' === $outFile) {
        echo "compile_smoke_probe: emit path blocked (set PHP_COMPILER_M3_SOURCE and PHP_COMPILER_M3_OUT for compile mode)\n";
        exit(1);
    }
    exit(\PHPCompiler\BootstrapAot\compile_smoke_m3_emit($sourceFile, $outFile));
}

echo "compiler_compile_smoke_compile_driver ready\n";
