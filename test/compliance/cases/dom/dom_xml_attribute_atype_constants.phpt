--TEST--
DOM XML_ATTRIBUTE_* atype globals match php-src (issue #24098, ext/dom/php_dom.stub.php)
--FILE--
<?php
$expect = [
...
// Bare names (not only constant()) so AOT matches VM (#21137 / #24098).
$got = [
...
$dom = $buckets['dom'] ?? [];
foreach ($expect as $name => $want) {
    if (!array_key_exists($name, $dom) || $dom[$name] !== $want) {
        $ok = 0;
        echo 'bad_bucket=', $name, "\n";
    }
}
// Node type and atype share numeric 2 under different names.
if (XML_ATTRIBUTE_NODE !== 2 || XML_ATTRIBUTE_ID !== 2) {
    $ok = 0;
    echo "bad=overlap\n";
}
echo 'ok=', $ok, "\n";
echo 'cdata=', XML_ATTRIBUTE_CDATA, "\n";
echo 'id=', XML_ATTRIBUTE_ID, "\n";
echo 'entity=', XML_ATTRIBUTE_ENTITY, "\n";
echo 'notation=', XML_ATTRIBUTE_NOTATION, "\n";
?>
--EXPECT--
ok=1
cdata=1
