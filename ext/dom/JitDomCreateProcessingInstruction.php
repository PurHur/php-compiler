 ...
 * Slot layout: nodeName/target = PI target, data/nodeValue/textContent = PI data,
 * tagName = `#pi` discriminator so saveXML can dump `<?target data?>` without
 * fetching an unset tagName (SIGSEGV, sibling #32315).
 *
 * php-src: ext/dom/document.c PHP_METHOD(DOMDocument, createProcessingInstruction)
 * → xmlNewDocPI; saveXML → xmlNodeDump → `<?target data?>` / empty data `<?target?>`
 */
final class JitDomCreateProcessingInstruction
{
    ...
    public static function invoke(Context $context, JITVariable ...$args): Value
    {
        ...
        $dataIsLit = true;
        if (null !== $dataArg) {
            $maybe = JitStringBuiltinArg::compileTimeLiteral($dataArg) ?? $dataArg->compileTimeString;
            if (null === $maybe) {
                $dataIsLit = false;
            } else {
                $dataLit = $maybe;
        ...
        if (null !== $targetLit && $dataIsLit) {
            return self::materialize($context, $targetLit, $dataLit);
        }

        return self::materializeFromRuntime($context, $args[1], $dataArg);
    }
    ...
    public static function materialize(Context $context, string $target, string $data = ''): Value
    {
        ...
        self::storeStringLiteral($context, $obj, self::PROP_TAG_NAME, self::TAG_PI);
    ...
    private static function materializeFromRuntime(
        Context $context,
        JITVariable $targetArg,
        ?JITVariable $dataArg
    ): Value {
        $targetStr = self::loadStringArg($context, $targetArg);
