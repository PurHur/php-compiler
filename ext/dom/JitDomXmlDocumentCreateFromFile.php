...
/**
 * LLVM lowering for Dom\XMLDocument::createFromFile() (#35804).
 *
 * php-src: ext/dom/xml_document.c — load_from_helper(DOM_LOAD_FILE)
 *
 * Compile-time path literals fold through {@see JitDomXmlDocumentCreateFromString}
 * (peer HTML #27300 / leftover ExternalMethod NULL).
 */
final class JitDomXmlDocumentCreateFromFile
{
        ...
        if (null !== $pathLit && '' !== $pathLit) {
            ...
            if (false === $contents) {
                TypeErrorRaise::emitRaise(
                    $context,
                    "Cannot open file '".$pathLit."'"
                );
            } else {
                $sourceArg = self::stringVariableFromLiteral($context, $contents);
                if (null !== $optionsArg) {
                    return JitDomXmlDocumentCreateFromString::invoke($context, $sourceArg, $optionsArg);
                }

                return JitDomXmlDocumentCreateFromString::invoke($context, $sourceArg);
            }
        }
    ...
    private static function invokeViaHelper(
        ...
        if (JitDomDocumentMethodKernel::shouldUse($context)) {
            JitDomDocumentMethodKernel::ensureXmlDocumentCreateFromFileBridge($context);
        } else {
            DomXmlDocumentCreateFromStringRuntime::ensureLinked($context);
        }

        $document = $context->builder->call(
            $context->lookupFunction(DomXmlDocumentCreateFromStringRuntime::ABI_FILE),
            $pathStr,
            $options
        );
