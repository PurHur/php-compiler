...
final class JitDomGetElementById
{
    ...
    private static function invokeLookup(Context $context, JITVariable ...$args): Value
    {
        ...
        if (DomDocumentMethodUserScriptLlvm::shouldUse($context)) {
            ...
            return self::lookupUserScriptFromCache($context, $args[1], $idStr);
        }
    ...
    private static function lookupUserScriptFromCache(
        Context $context,
        JITVariable $idArg,
        Value $idStr
    ): Value {
        ...
        $idLit = JitStringBuiltinArg::compileTimeLiteral($idArg) ?? $idArg->compileTimeString;
        $parsed = JitDomLoadHTMLUserScript::lastCompileTimeParsed();
        if (null !== $idLit && null !== $parsed && $idLit === $parsed['id']) {
