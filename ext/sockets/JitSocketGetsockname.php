...
/**
 * LLVM lowering for socket_getsockname() via SocketCreateJitHelper (#31293).
 *
...
final class JitSocketGetsockname
{
    public static function invoke(Context $context, JITVariable ...$args): Value
    {
        ...
        $ok = $context->builder->call(
            $context->lookupFunction('__compiler_socket_getsockname'),
            $handle
        );
        ...
        $failBb = BasicBlockHelper::append($context, 'socket_getsockname_fail');
        $okBb = BasicBlockHelper::append($context, 'socket_getsockname_ok');
        $doneBb = BasicBlockHelper::append($context, 'socket_getsockname_done');
    ...
    private static function socketHandle(Context $context, JITVariable $arg): Value
    {
        if (JITVariable::TYPE_OBJECT === $arg->type) {
            return JitGetObjectId::invoke($context, $arg, 'socket_getsockname');
        }
        ...
        TypeErrorRaise::emitRaise(
            $context,
            'socket_getsockname(): Argument #1 ($socket) must be of type Socket, mixed given'
        );
