...
/**
 * FILTER_* ids and name map (php-src ext/filter/filter.c, filter_private.h; #5839, #11419, #24065).
 *
...
{
    /**
     * filter_list() names in php-src order (ext/filter/filter.c php_filter_list).
     *
     * @var list<string>
     */
    ...
    public const NAME_TO_ID = [
        'int' => VmFilter::FILTER_VALIDATE_INT,
        'boolean' => VmFilter::FILTER_VALIDATE_BOOLEAN,
        'float' => VmFilter::FILTER_VALIDATE_FLOAT,
        'validate_regexp' => VmFilter::FILTER_VALIDATE_REGEXP,
        'validate_domain' => VmFilter::FILTER_VALIDATE_DOMAIN,
        ...
        'validate_mac' => VmFilter::FILTER_VALIDATE_MAC,
        'string' => VmFilter::FILTER_SANITIZE_STRING,
        'stripped' => VmFilter::FILTER_SANITIZE_STRING,
        'encoded' => VmFilter::FILTER_SANITIZE_ENCODED,
        'special_chars' => VmFilter::FILTER_SANITIZE_SPECIAL_CHARS,
        'full_special_chars' => VmFilter::FILTER_SANITIZE_FULL_SPECIAL_CHARS,
        'unsafe_raw' => VmFilter::FILTER_UNSAFE_RAW,
        'email' => VmFilter::FILTER_SANITIZE_EMAIL,
        'url' => VmFilter::FILTER_SANITIZE_URL,
        'number_int' => VmFilter::FILTER_SANITIZE_NUMBER_INT,
        'number_float' => VmFilter::FILTER_SANITIZE_NUMBER_FLOAT,
        'add_slashes' => VmFilter::FILTER_SANITIZE_ADD_SLASHES,
        'callback' => VmFilter::FILTER_CALLBACK,
    ];

    /**
     * Extension constants registered into the filter bucket (php-src register_filter_symbols).
     *
     * Profile-gated: FILTER_THROW_ON_FAILURE + FILTER_FLAG_GLOBAL_RANGE bit layout (#24065).
     *
    ...
    public static function registeredConstants(): array
    {
        $out = [
            ...
            'FILTER_FLAG_GLOBAL_RANGE' => VmFilter::filterFlagGlobalRange(),
            'FILTER_FLAG_HOSTNAME' => VmFilter::FILTER_FLAG_HOSTNAME,
            'FILTER_FLAG_EMAIL_UNICODE' => VmFilter::FILTER_FLAG_EMAIL_UNICODE,
        ];
        // PHP 8.5+: FILTER_THROW_ON_FAILURE takes 0x10000000; GLOBAL_RANGE moves to 0x20000000.
        if (CompilerVersion::supportsFilterThrowOnFailure()) {
            // Insert after FILTER_NULL_ON_FAILURE to match php-src register order.
            $reordered = [];
            foreach ($out as $name => $value) {
                $reordered[$name] = $value;
                if ('FILTER_NULL_ON_FAILURE' === $name) {
                    $reordered['FILTER_THROW_ON_FAILURE'] = VmFilter::FILTER_THROW_ON_FAILURE;
                }
            }
    ...
    {
        return array_keys(self::registeredConstants());
    }

    public static function isRegisteredName(string $name): bool
    {
        return isset(self::registeredConstants()[$name]);
    }
    ...
    {
        $value = self::registeredConstants()[$name] ?? null;
        if (null === $value) {
            return null;
        }
