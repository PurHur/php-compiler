...
/** LLVM lowering for header_register_callback() (issue #3759). */
final class JitHeaderRegisterCallback
{
    public static function invoke(Context $context, JITVariable ...$args): Value
    {
        ...
        if (null === $call) {
            throw new \LogicException(
                'header_register_callback() callback must be a compile-time function name or closure in this compiler build'
            );
        }
        if (Builtin::LOAD_TYPE_STANDALONE !== $context->loadType) {
            ...
            if (null === $literal || '' === $literal) {
                throw new \LogicException(
                    'header_register_callback() JIT requires a compile-time function name string in this compiler build; use bin/vm.php for closures'
                );
            }
        }
        $saved = $context->builder->getInsertBlock();
        $context->builder->positionAtEnd($context->headerPreFlushBlock);
        try {
            self::emitPreFlushCall($context, $call);
        } finally {
            if (null !== $saved) {
        ...
        return $context->getTypeFromString('int32')->constInt(1, false);
    }
    ...
    private static function emitPreFlushCall(Context $context, Call $call): void
    {
    ...
    private static function unwrapNative(Call $call): Native
    {
        ...
        throw new \LogicException(
            'header_register_callback() callback must be a closure in this compiler build'
        );
    }
}
