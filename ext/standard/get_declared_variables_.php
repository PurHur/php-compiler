<?php

declare(strict_types=1);

namespace PHPCompiler\ext\standard;

use PHPCompiler\Frame;
use PHPCompiler\Func\Internal;
use PHPCompiler\JIT\Context;
use PHPCompiler\JIT\ScopeBuiltinHelper;
use PHPCompiler\JIT\Variable as JITVariable;
use PHPLLVM\Value;

/**
 * get_declared_variables() — phantom vs php-src (#24223, re-#4780). Kept for spine compile only;
 * not registered in Module.php. Use get_defined_vars().
 */
final class get_declared_variables_ extends Internal
{
    public function __construct()
    {
        parent::__construct('get_declared_variables');
    }

    public function execute(Frame $frame): void
    {
        if (\count($frame->calledArgs) > 0) {
            throw new \LogicException('get_declared_variables() takes no arguments');
        }
        if (null === $frame->returnVar) {
            return;
        }
        $frame->returnVar->array(VmScope::getDeclaredVariables($frame));
    }

    public function call(Context $context, JITVariable ...$args): Value
    {
        if (\count($args) > 0) {
            throw new \LogicException('get_declared_variables() takes no arguments');
        }

        return ScopeBuiltinHelper::getDeclaredVariables($context);
    }
}
