...
/** LLVM JIT/AOT helper for str_ireplace() with array $search and/or array|string $replace (#10599). */
final class JitStrIreplaceMulti
{
    public static function replace(
        ...
        if (null !== $searchNeedles && null !== $replaceOperand && null !== $subjectLit) {
            ...
            $result = 1 === \count($searchNeedles) && \is_string($replaceOperand)
                ? VmString::strIreplace($searchNeedles[0], $replaceOperand, $subjectLit, $count)
                : VmString::strIreplaceMulti($searchNeedles, $replaceOperand, $subjectLit, $count);
            if (null !== $countSlot) {
        ...
        if (self::isRuntimeArrayArg($searchArg) || self::isRuntimeArrayArg($replaceArg)) {
            throw new \LogicException(
                'str_ireplace() runtime array $search/$replace is not supported in this compiler build'
            );
        }

        return JitStrIreplace::replace(
            $context,
            JitStringArg::lower($context, $searchArg, 'str_ireplace() search'),
            JitStringArg::lower($context, $replaceArg, 'str_ireplace() replace'),
            JitStringArg::lower($context, $subjectArg, 'str_ireplace() subject'),
            $countSlot
        );
    }
