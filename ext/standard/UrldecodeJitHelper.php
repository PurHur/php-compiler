...
/**
 * urldecode()/rawurldecode() for compiled JIT/AOT modules (#14726, #35057, php-in-PHP).
 *
 * Self-contained NestedJIT helper — must not NestedJIT {@see VmString} (thin AOT
 * segfaults when UrldecodeJitHelper only forwards; peer {@see Base64JitHelper} #35051 /
 * {@see UrlencodeJitHelper} #35057 / {@see Bin2hexJitHelper} #20452).
 *
 * Length via `\strlen`; look-ahead uses `$i+2 < $len` then `$data[$i+1]` / `$data[$i+2]`
 * (peer {@see Hex2binJitHelper}). No native ord()/chr()/hexdec().
...
final class UrldecodeJitHelper
{
    ...
    /** @return int negative when not a hex digit */
