...
/** LLVM lowering for image_type_to_extension() (ext/standard/image.c; #6091). */
final class JitImageTypeToExtension
{
    /** @var array<int, array{0: string, 1: string}> dotted / undotted extension */
    private const EXTENSIONS = [
        VmImage::IMAGETYPE_GIF => ['.gif', 'gif'],
        VmImage::IMAGETYPE_JPEG => ['.jpeg', 'jpeg'],
        VmImage::IMAGETYPE_PNG => ['.png', 'png'],
        VmImage::IMAGETYPE_SWF => ['.swf', 'swf'],
        VmImage::IMAGETYPE_SWC => ['.swf', 'swf'],
        VmImage::IMAGETYPE_PSD => ['.psd', 'psd'],
        VmImage::IMAGETYPE_BMP => ['.bmp', 'bmp'],
        VmImage::IMAGETYPE_WBMP => ['.bmp', 'bmp'],
        VmImage::IMAGETYPE_TIFF_II => ['.tiff', 'tiff'],
        VmImage::IMAGETYPE_TIFF_MM => ['.tiff', 'tiff'],
        VmImage::IMAGETYPE_IFF => ['.iff', 'iff'],
        VmImage::IMAGETYPE_JPC => ['.jpc', 'jpc'],
        VmImage::IMAGETYPE_JP2 => ['.jp2', 'jp2'],
        VmImage::IMAGETYPE_JPX => ['.jpx', 'jpx'],
        VmImage::IMAGETYPE_JB2 => ['.jb2', 'jb2'],
        VmImage::IMAGETYPE_XBM => ['.xbm', 'xbm'],
        VmImage::IMAGETYPE_ICO => ['.ico', 'ico'],
        VmImage::IMAGETYPE_WEBP => ['.webp', 'webp'],
        VmImage::IMAGETYPE_AVIF => ['.avif', 'avif'],
        VmImage::IMAGETYPE_HEIF => ['.heif', 'heif'],
    ];

    public static function invoke(Context $context, JITVariable $imageTypeArg, Value $includeDot): Value
    {
        $imageType = self::lowerImageType($context, $imageTypeArg);
        $dottedExt = self::lookupExtension($context, $imageType, true);
        $plainExt = self::lookupExtension($context, $imageType, false);
        $extPtr = $context->builder->select($includeDot, $dottedExt, $plainExt);

        $strPtrTy = $context->getTypeFromString('__string__*');
        $isNull = $context->builder->icmp(Builder::INT_EQ, $extPtr, $strPtrTy->constNull());

        $slot = JitValueBox::alloc($context);
        $ptr = JitValueBox::pointer($context, $slot);
        $failBlock = BasicBlockHelper::append($context, 'imgext_fail');
        $okBlock = BasicBlockHelper::append($context, 'imgext_ok');
        $doneBlock = BasicBlockHelper::append($context, 'imgext_done');
    ...
    private static function lookupExtension(Context $context, Value $imageType, bool $withDot): Value
    {
        $strPtrTy = $context->getTypeFromString('__string__*');
        $null = $strPtrTy->constNull();
        $result = $null;
        $i64 = $context->getTypeFromString('int64');
        foreach (self::EXTENSIONS as $type => [$dotted, $plain]) {
            $eq = $context->builder->icmp(
                Builder::INT_EQ,
                $imageType,
                $i64->constInt($type, false)
            );
            $candidate = self::literalString($context, $withDot ? $dotted : $plain);
    ...
    private static function lowerImageType(Context $context, JITVariable $arg): Value
    {
        ...
        return JitLongArg::lower($context, $arg, 'image_type_to_extension() image_type');
    }

    private static function lowerStringOperand(Context $context, JITVariable $arg): Value
    {
        $strPtr = JitStringArg::lower($context, $arg, 'image_type_to_extension() image_type');
    ...
    {
        return sprintf(
            'image_type_to_extension(): Argument #1 ($image_type) must be of type int, %s given',
            $given
        );
    }
