...
/**
 * array_diff_assoc() — diff with key match and loose value equality (php-src ext/standard/array.c, #3129).
 */
final class array_diff_assoc extends Internal
{
    public function execute(Frame $frame): void
    {
        ...
        VmArrayAssocSetOps::guardSetOpOperands($frame, $frame->calledArgs, 'array_diff_assoc');
        ...
        $others = VmArrayAssocSetOps::collectOtherHashTables($frame->calledArgs, 'array_diff_assoc');
        $out = new HashTable();
        foreach ($firstHt->iterateKeyed(true) as [$key, $value]) {
            if (VmArrayAssocSetOps::pairInAnyOther($key, $value, $others)) {
                continue;
            }
            $stored = new Variable();
    ...
    public function call(Context $context, JITVariable ...$args): Value
    {
        ...
        foreach ($args as $i => $arg) {
            ...
            if (null !== $bad) {
                ...
                if (0 === $i) {
                    throw new \TypeError(
                        'array_diff_assoc(): Argument #1 ($array) must be of type array, '.$bad.' given'
                    );
                }
                throw new \TypeError(
                    'array_diff_assoc(): Argument #'.((int) $i + 1).' must be of type array, '.$bad.' given'
                );
            }
            ...
            if (0 === $i) {
                JitArrayElem::requireArrayParam($context, $arg, 'array_diff_assoc', 1, 'array');
            } else {
                JitArrayElem::requireArrayArgNum($context, $arg, 'array_diff_assoc', $i + 1);
            }
        }
