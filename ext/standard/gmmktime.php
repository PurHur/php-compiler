...
/** gmmktime() — UTC Unix timestamp from parts (VM VmDate; JIT StringGmmktime, #7001). */
final class gmmktime extends Internal
{
    public function __construct()
    {
        parent::__construct('gmmktime');
    }
    ...
    public function execute(Frame $frame): void
    {
        ...
        if ($argc < 1) {
            throw new \ArgumentCountError('gmmktime() expects at least 1 argument, 0 given');
        }
        if ($argc > 6) {
            throw new \ArgumentCountError('gmmktime() expects at most 6 arguments, '.$argc.' given');
        }
    ...
    public function call(Context $context, JITVariable ...$args): Value
    {
        $argc = \count($args);
        if ($argc < 1 || $argc > 6) {
            throw new \LogicException('gmmktime() expects between one and six arguments in this compiler build');
        }

        return JitGmmktime::invoke(
            $context,
            $args[0],
            $args[1] ?? null,
            $args[2] ?? null,
