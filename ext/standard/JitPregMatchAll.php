...
/** LLVM lowering for preg_match_all() match count (no by-ref $matches in JIT v1; ensureLinked #33188). */
final class JitPregMatchAll
{
    ...
    public static function invoke(Context $context, Value $pattern, Value $subject): Value
    {
        ...
        $raw = $context->builder->call(
            $context->lookupFunction('__compiler_preg_match_all'),
            $pattern,
            $subject
        );
        ...
        $failBlock = BasicBlockHelper::append($context, 'preg_match_all_fail_'.$id);
        $okBlock = BasicBlockHelper::append($context, 'preg_match_all_ok_'.$id);
