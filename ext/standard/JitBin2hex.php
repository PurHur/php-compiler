...
final class JitBin2hex
{
    public static function convert(Context $context, Value $strPtr): Value
    {
        ...
        $emptyBlock = $prev->insertBasicBlock('bin2hex_empty');
        $workBlock = $prev->insertBasicBlock('bin2hex_work');
        $doneBlock = $emptyBlock->insertBasicBlock('bin2hex_done');
        ...
        $outLen = $context->builder->mulNoSignedWrap($len, $two);
        ...
        $idxSlot = $context->builder->alloca($i32, 1, 'bin2hex_idx');
        $context->builder->store($zero, $idxSlot);

        $loopHead = $workBlock->insertBasicBlock('bin2hex_head');
        $loopBody = $loopHead->insertBasicBlock('bin2hex_body');
        $loopDone = $loopBody->insertBasicBlock('bin2hex_loop_done');
        ...
        $outPos = $context->builder->mulNoSignedWrap($idx, $two);
        $context->builder->store(
