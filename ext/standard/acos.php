...
/**
 * acos() for integer or float arguments (ext/standard/math.c parity).
 */
final class acos extends Internal
{
    public function execute(Frame $frame): void
    {
        ...
        $frame->returnVar->float(\acos(self::toFloat($v)));
    }
    ...
    private static function toJitDouble(Context $context, JITVariable $arg, $double): Value
    {
        switch ($arg->type) {
            case JITVariable::TYPE_NATIVE_LONG:
                $v = JitLongArg::lower($context, $arg, 'acos() argument');

                return $context->builder->siToFp($v, $double);
            case JITVariable::TYPE_NATIVE_DOUBLE:
                return $context->helper->loadValue($arg);
            default:
                throw new \LogicException('acos() only supports integers and floats in this compiler build');
        }
    }
