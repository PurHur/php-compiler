 ...
 *
 * Runtime emission is required for thin AOT (#27660): NestedJIT of ExplodeJitHelper cannot
 * construct HashTable / call VmString (peers #26956, #27078, #27345). Compile-time literals
 * still use {@see buildPackedStrings()}.
 */
...
final class JitExplode
{
    ...
    private static function explodeUnlimited(Context $context, Value $delimiter, Value $haystack): Value
    {
        ...
        $foundI64 = self::findDelimiterOffsetI64($context, $haystack, $delimiter, $offset);
        $notFound = self::isDelimiterNotFound($context, $foundI64);
    ...
    private static function findDelimiterOffsetI64(
        Context $context,
        Value $haystack,
        Value $delimiter,
        Value $offset
    ): Value {
        // Must be a CALL — inlining VmStringCompare::findOffset into explode loops
        // corrupts phis when the loop branches back to the find entry (#27660).
        self::ensureFindDelimFunction($context);

        return $context->builder->call(
            $context->lookupFunction('phpc_explode_find_delim'),
            $haystack,
            $delimiter,
            $offset
        );
    ...
    private static function ensureFindDelimFunction(Context $context): void
    {
        ...
        $i64 = $context->getTypeFromString('int64');
        $ft = $context->context->functionType($i64, false, $strPtr, $strPtr, $i64);
        $fn = null !== $existing
            ? $existing
    ...
    private static function isDelimiterNotFound(Context $context, Value $foundI64): Value
    {
        $i64 = $context->getTypeFromString('int64');
