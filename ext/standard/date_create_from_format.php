...
/**
 * date_create_from_format() — procedural DateTime factory from format string (#6172).
 *
 * php-src: ext/date/php_date.c — PHP_FUNCTION(date_create_from_format)
 */
final class date_create_from_format extends Internal
{
    public function __construct()
    {
        parent::__construct('date_create_from_format');
    }
    ...
    public function execute(Frame $frame): void
    {
        ...
        if ($argc < 2 || $argc > 3) {
            throw new \ArgumentCountError(\sprintf(
                'date_create_from_format() expects at least 2 arguments, %d given',
                $argc
            ));
        }
        ...
        if ($argc >= 3) {
            ...
            if (Variable::TYPE_NULL !== $tzVar->type) {
                $timezone = DateTimeSupport::requireDateTimeZone(
                    $frame->calledArgs[2],
                    'date_create_from_format(): Argument #3 ($timezone)'
                );
            }
        }
        ...
        if (null === $created) {
            $frame->vmContext->errors->triggerError(
                'date_create_from_format(): Failed to parse time string ('.$time.')',
    ...
    {
        throw new \LogicException('date_create_from_format() is not implemented for JIT in this compiler build (issue #6172)');
    }
}
