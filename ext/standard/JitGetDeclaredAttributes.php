...
/** LLVM lowering for get_declared_attributes() (issue #6450). */
final class JitGetDeclaredAttributes
{
    public static function invoke(Context $context): Value
    {
        $names = $context->type->object->allDeclaredAttributeClassNames();
        if ([] === $names && null !== $context->runtime->vmContext) {
            $table = VmReflection::declaredAttributesTable($context->runtime->vmContext);
        ...
        return self::wrapHashTable(
            $context,
            self::emitAttributeClassNames($context, $names)
        );
    }
     ...
     */
    private static function emitAttributeClassNames(Context $context, array $names): Value
    {
