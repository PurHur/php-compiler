...
{
    private const FUNCTION = 'class_has_method';

    private const OBJECT_OR_CLASS_TYPE_ERROR =
        'class_has_method(): Argument #1 ($object_or_class) must be of type object|string, %s given';
    ...
    public function execute(Frame $frame): void
    {
        ...
        $method = VmString::coerceStringBuiltinArg($frame->calledArgs[1], self::FUNCTION, 1, 'method');
        ...
        $exists = null !== $entry && VmReflection::classHasMethodForReflection($entry, $ctx, $method);
    ...
    public function call(Context $context, JITVariable ...$args): Value
    {
        ...
        if (null !== $classLiteral && null !== $methodLiteral) {
            return ReflectionBuiltinHelper::classHasMethodLiteral($context, $classLiteral, $methodLiteral);
        }

        throw new \LogicException(
            self::FUNCTION.'() class and method names must be string literals in JIT in this compiler build'
        );
    }
