...
/**
 * htmlentities() — encode HTML entities (subset of PHP; issue #2472).
 *
 * VM: {@see VmString::htmlentities()}; JIT/AOT: same LLVM path as htmlspecialchars().
 */
        ...
        if (null !== $literal && 1 === $argc) {
            return $context->builder->load(
                $context->constantStringFromString(
                    VmString::htmlentities($literal, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8', true)
                )
            );
        }
