...
{
    private const FUNCTION = 'class_has_property';

    private const OBJECT_OR_CLASS_TYPE_ERROR =
        'class_has_property(): Argument #1 ($object_or_class) must be of type object|string, %s given';
    ...
    public function execute(Frame $frame): void
    {
        ...
        $property = VmString::coerceStringBuiltinArg($frame->calledArgs[1], self::FUNCTION, 1, 'property');
        ...
        $exists = null !== $entry && VmReflection::classHasPropertyForReflection($entry, $ctx, $property);
    ...
    public function call(Context $context, JITVariable ...$args): Value
    {
        ...
        self::emitJitStringArgChecks($context, $args[1], 1, 'property');
        $classLiteral = JitStringArg::compileTimeLiteral($args[0]);
        $propertyLiteral = JitStringArg::compileTimeLiteral($args[1]);
        if (null !== $classLiteral && null !== $propertyLiteral) {
            return ReflectionBuiltinHelper::classHasPropertyLiteral($context, $classLiteral, $propertyLiteral);
        }

        throw new \LogicException(
            self::FUNCTION.'() class and property names must be string literals in JIT in this compiler build'
        );
    }
