...
final class arsort_ extends Internal
{
    ...
    public function execute(Frame $frame): void
    {
        ...
        if ($argc < 1 || $argc > 2) {
            throw new \LogicException('arsort() requires one or two arguments');
        }
    ...
    private static function resolveJitSortFlags(Context $context, JITVariable $flagsArg): int
    {
        ...
        if (JITVariable::TYPE_NATIVE_LONG === $flagsArg->type) {
            throw new \LogicException(
                'arsort() flags must be a predefined constant in JIT/AOT in this compiler build'
            );
        }
        throw new \LogicException('arsort() flags must be an integer in this compiler build');
    }
