...
final class array_all_key extends Internal
{
    ...
    {
        $strict = VmArrayValueCallback::parseOptionalStrictArg($frame->calledArgs, 'array_all_key');
        if (null === $frame->returnVar) {
            return;
        }
        $ht = VmArray::requireArray($frame->calledArgs[0]->resolveIndirect(), 'array_all_key');
        $callback = $frame->calledArgs[1];
        VmArrayValueCallback::requireCallback($frame, $callback, 'array_all_key');
    ...
    public function call(Context $context, JITVariable ...$args): Value
    {
        ...
        if (3 === $argc) {
            ...
            $strictI1 = $this->jitBool($context, $args[2], 'array_all_key() strict');

            return $this->boxStandaloneBoolJitResult(
                $context,
                ArrayFindHelper::buildAllArray($context, $args[0], $args[1], null, $strictI1, true)
            );
        }
        if (!ArrayFindCallbackPolicy::isJitNullCallback($args[1])
            && !ArrayFindCallbackPolicy::isJitLowerable($args[1])) {
        ...
        return $this->boxStandaloneBoolJitResult(
            $context,
            ArrayFindHelper::buildAllArray($context, $args[0], $args[1], null, null, true)
        );
    }
}
