...
/** LLVM lowering for stream_copy_to_string() via __compiler_stream_copy_to_string (#6547). */
final class JitStreamCopyToString
{
    /** Z_PARAM_LONG_OR_NULL parity for $maxlength (ext/standard/streams.c). */
    public static function lowerLengthArg(Context $context, JITVariable $arg): Value
    {
        ...
        return JitLongArg::lower($context, $arg, 'stream_copy_to_string() maxlength');
    }

    /** Z_PARAM_LONG parity for $offset (#6547). */
    public static function lowerOffsetArg(Context $context, JITVariable $arg): Value
    {
        ...
        return JitLongArg::lower($context, $arg, 'stream_copy_to_string() offset');
    }
    ...
    private static function lowerBoxedNullableLengthArg(Context $context, JITVariable $arg): Value
    {
        ...
        $nullBlock = BasicBlockHelper::append($context, 'scts_len_null');
        $notNullBlock = BasicBlockHelper::append($context, 'scts_len_not_null');
        $arrayBlock = BasicBlockHelper::append($context, 'scts_len_array');
        $rejectBlock = BasicBlockHelper::append($context, 'scts_len_reject');
        $coerceBlock = BasicBlockHelper::append($context, 'scts_len_coerce');
        $mergeBlock = BasicBlockHelper::append($context, 'scts_len_merge');
        ...
        $numericBlock = BasicBlockHelper::append($context, 'scts_len_numeric');
    ...
    private static function lowerBoxedOffsetArg(Context $context, JITVariable $arg): Value
    {
        ...
        $arrayBlock = BasicBlockHelper::append($context, 'scts_off_array');
        $rejectBlock = BasicBlockHelper::append($context, 'scts_off_reject');
        $coerceBlock = BasicBlockHelper::append($context, 'scts_off_coerce');

        $isArray = $context->builder->icmp(Builder::INT_EQ, $typeByte, $arrayTy);
        $okBlock = BasicBlockHelper::append($context, 'scts_off_ok');
    ...
    {
        return sprintf(
            'stream_copy_to_string(): Argument #3 ($offset) must be of type int, %s given',
            $given
        );
    }
    ...
    public static function invoke(
        ...
        $contents = $context->builder->call(
            $context->lookupFunction('__compiler_stream_copy_to_string'),
            $handleLong,
            $maxlengthLong,
            $offsetLong
        );
        ...
        $failBlock = BasicBlockHelper::append($context, 'stream_copy_to_string_fail');
        $okBlock = BasicBlockHelper::append($context, 'stream_copy_to_string_ok');
        $doneBlock = BasicBlockHelper::append($context, 'stream_copy_to_string_done');
