...
/**
 * LLVM JIT/AOT helpers for dirname() and basename() (byte paths, / and \ separators).
 *
 * Thin AOT NestedJIT of PathJitHelper returns a null __string__* (value-box ABI
 * mismatch — no VmString::basename call in the emitted helper), then segfaults after
 * c:main_before_php (#26905; peer getcwd #26928 / getmypid #26944). Emit path logic
 * as LLVM at the call site. VM SSOT stays {@see VmString}.
...
final class JitPath
{
    ...
    {
        return self::stripSuffixIfPresent($context, self::basename($context, $path), $suffix);
    }
    ...
    /**
     * Locate stream-wrapper "://" without {@see JitStringSearch} — findOffsetI32 was
     * false-positive at offset 0 under thin AOT and collapsed dirname("/a/b") to "/".
     */
    private static function findSchemeSepIndex(
        ...
        $three = $i64->constInt(3, false);
        $minusOne = $i64->constInt(-1, false);
        [, $charPtr] = self::stringFields($context, $str);

        $idxSlot = $context->builder->alloca($i64, 1, 'dirname_scheme_idx_'.$id);
        $context->builder->store($zero, $idxSlot);

        $done = self::block($context, 'dirname_scheme_done_'.$id);
        $head = self::block($context, 'dirname_scheme_head_'.$id);
        $body = self::block($context, 'dirname_scheme_body_'.$id);
        $checkSlash1 = self::block($context, 'dirname_scheme_slash1_'.$id);
        $checkSlash2 = self::block($context, 'dirname_scheme_slash2_'.$id);
        $hit = self::block($context, 'dirname_scheme_hit_'.$id);
        $advance = self::block($context, 'dirname_scheme_advance_'.$id);
        $context->builder->branch($head);

        $context->builder->positionAtEnd($head);
        $idx = $context->builder->load($idxSlot);
        // Need room for ':' '/' '/'
        $limit = $context->builder->sub($end, $three);
        $stop = $context->builder->icmp(Builder::INT_SGT, $idx, $limit);
        $context->builder->branchIf($stop, $done, $body);

        $context->builder->positionAtEnd($body);
        $ch0 = $context->builder->zExt($context->builder->load($context->builder->gep($charPtr, $idx)), $i32);
        $isColon = $context->builder->icmp(Builder::INT_EQ, $ch0, $i32->constInt(ord(':'), false));
        $context->builder->branchIf($isColon, $checkSlash1, $advance);

        $context->builder->positionAtEnd($checkSlash1);
        $ch1 = $context->builder->zExt(
            $context->builder->load($context->builder->gep($charPtr, $context->builder->add($idx, $one))),
            $i32
        );
        $isSlash1 = $context->builder->icmp(Builder::INT_EQ, $ch1, $i32->constInt(ord('/'), false));
        $context->builder->branchIf($isSlash1, $checkSlash2, $advance);

        $context->builder->positionAtEnd($checkSlash2);
        $ch2 = $context->builder->zExt(
            $context->builder->load($context->builder->gep($charPtr, $context->builder->add($idx, $two))),
            $i32
        );
        $isSlash2 = $context->builder->icmp(Builder::INT_EQ, $ch2, $i32->constInt(ord('/'), false));
        $context->builder->branchIf($isSlash2, $hit, $advance);

        $context->builder->positionAtEnd($hit);
        $context->builder->store($idx, $schemeSepSlot);
        $context->builder->branch($done);
        ...
        $context->builder->positionAtEnd($done);
        // schemeSepSlot stays -1 when never hit (caller initializes to -1).
    }
