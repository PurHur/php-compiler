    ...
    public function execute(Frame $frame): void
    {
        ...
        if ($argc < 2 || $argc > 3) {
            throw new \ArgumentCountError(
                'openssl_pkey_export() expects 2 or 3 arguments, '.$argc.' given'
            );
        }
        ...
        $config = 3 === $argc ? $frame->calledArgs[2] : null;
        $exported = VmOpenssl::pkeyExportPem($frame->calledArgs[0], $config, $frame);
        if (false === $exported) {
