...
/**
 * LLVM JIT/AOT helpers for mb_ucfirst() / mb_lcfirst() (php-src ext/mbstring/mbstring.c; #27330, #34259).
 *
 * Compile-time fold for string literals; runtime non-literal args via NestedJIT MbCaseJitHelper
 * (peer {@see JitMbCase} / #34236).
 */
final class JitMbUcfirstLcfirst
{
    ...
    public static function invoke(Context $context, string $function, callable $fold, array $args): Value
    {
        ...
        if (null !== $strLit && null !== $encLit) {
            // Soft-null string is not a compileTimeString — falls through to runtime (#24176).
            if (!MbstringEncodingRegistry::isValid($encLit)) {
    ...
    private static function runtimeEncodingLiteral(array $args, int $argc, Context $context): string
    {
        ...
        if (JITVariable::TYPE_STRING !== $args[1]->type) {
            throw new \LogicException(
                'mb_ucfirst/mb_lcfirst JIT encoding must be a string literal in this compiler build'
            );
        }
