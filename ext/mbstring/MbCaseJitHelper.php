 ...
 * (peer MbStrwidthJitHelper #3495; ucfirst/lcfirst #34259; convert_case UPPER/LOWER #34280).
 *
 * Do **not** call {@see VmMbstring} / {@see Utf8CaseMap} / {@see MbstringState} from these
 * entry points — illegal-UTF-8 substitution through that path SIGSEGVs under thin AOT
 * NestedJIT (#34340; same class as TITLE split #34284). This unit uses only strlen/ord/substr
 * + NestedJIT-safe upper/lower maps (Latin-1 + Cyrillic + Greek + ß; peer MbConvertCaseJitHelper
 * — no PHP chr()).
 *
...
final class MbCaseJitHelper
{
    public static function strtoupperArgv(string $string, string $encoding): string
    {
        ...
        return self::utf8UpperMap($string);
    }

    public static function strtolowerArgv(string $string, string $encoding): string
    {
        if (self::isAsciiFamily($encoding)) {
            return self::asciiLower($string);
        }

        return self::utf8LowerMap($string);
    }
    ...
    public static function ucfirstArgv(string $string, string $encoding): string
    {
        ...
        if (self::isAsciiFamily($encoding)) {
            ...
            return self::asciiUpper($first).$rest;
        }
        $len = self::utf8ByteLenAt($string, 0);
        if ($len <= 0) {
            return $string;
        }
        if (self::isIllegalUtf8Byte($string, 0, $len)) {
            return '?'.\substr($string, 1);
        }
        $cp = self::utf8CpAt($string, 0, $len);
        $head = self::encodeMappedUpper($cp);
        $rest = \substr($string, $len);
    ...
    public static function lcfirstArgv(string $string, string $encoding): string
    {
        ...
        if (self::isAsciiFamily($encoding)) {
            ...
            return self::asciiLower($first).$rest;
        }
        $len = self::utf8ByteLenAt($string, 0);
        if ($len <= 0) {
            return $string;
        ...
        $cp = self::utf8CpAt($string, 0, $len);
        $head = self::encodeMappedLower($cp);
        $rest = \substr($string, $len);
    ...
    private static function asciiUpper(string $source): string
    {
        ...
        $i = 0;
        while ($i < $len) {
            $b = \ord(\substr($source, $i, 1));
            if ($b >= 97 && $b <= 122) {
                $b -= 32;
            }
            $out .= \substr(self::allBytes(), $b, 1);
            ++$i;
        }
    ...
    /** Separate from utf8LowerMap — NestedJIT miscompiles bool-flagged shared walk (#34340). */
    private static function utf8UpperMap(string $string): string
    {
        $out = '';
        $len = \strlen($string);
        $i = 0;
    ...
    private static function utf8LowerMap(string $string): string
    {
        $out = '';
        $len = \strlen($string);
        $i = 0;
    ...
    private static function encodeMappedUpper(int $cp): string
    {
        if (0xDF === $cp) {
            return 'SS';
        }

        return self::encodeUtf8(self::toUpperCp($cp));
    }

    private static function encodeMappedLower(int $cp): string
    {
        if (0xDF === $cp) {
            return 'ss';
        }

        return self::encodeUtf8(self::toLowerCp($cp));
    }
    ...
    private static function toUpperCp(int $cp): int
    {
        ...
        // Greek small → capital (final sigma → Sigma).
