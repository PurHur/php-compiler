 ...
 *
 * Compile-time fold for string literals; runtime encoding via NestedJIT
 * {@see MbEncodingAliasesJitHelper} (#35216 leftover of #30795).
 *
...
final class JitMbEncodingRegistry
{
    ...
    public static function foldEncodingAliases(Context $context, JITVariable ...$args): Value
    {
        ...
        // Runtime encoding — NestedJIT joined string → HT (#35216).
        return self::lowerEncodingAliasesRuntime($context, $args[0]);
    }

    private static function lowerEncodingAliasesRuntime(Context $context, JITVariable $arg): Value
    {
        ...
        $context->builder->call(
            MbEncodingAliasesRuntime::assertEncodingHelper($context),
            $enc
        );
    ...
    /** Empty sentinel / empty joined → []; otherwise explode on {@see MbEncodingAliasesJitHelper::JOIN_DELIM}. */
    private static function hashtableFromJoined(Context $context, Value $joined): Value
    {
        $tag = 'mba';
        $i64 = $context->getTypeFromString('int64');
        $zero = $i64->constInt(0, false);
        $emptyLen = $i64->constInt(\strlen(MbEncodingAliasesJitHelper::EMPTY_ALIASES), false);
        $slen = $context->builder->call($context->lookupFunction('__string__strlen'), $joined);
        // EMPTY_ALIASES sentinel (NestedJIT must not return "").
        $isEmpty = $context->builder->icmp(Builder::INT_EQ, $slen, $emptyLen);
