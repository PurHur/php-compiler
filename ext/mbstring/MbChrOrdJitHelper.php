...
/**
 * mb_chr() / mb_ord() for compiled JIT/AOT modules (#34243 / #34250 leftovers of #33547 / #33536 / #30759).
 *
 * Returns {@see StringStrpos::NOT_FOUND} (-1) from ordArgv on invalid first character so callers can box int|false.
 * chrArgv returns string|false.
 *
 * NestedJIT must not call {@see VmMbstring::ord} / {@see VmMbstring::chr} / native {@see chr} —
 * those silent-return / misbehave under thin AOT NestedJIT. Decode/encode is inlined with strlen/ord/substr
 * and a NestedJIT-safe byte table (peer {@see Hex2binJitHelper} #20452).
 *
 * SSOT (VM / compile-time fold): {@see VmMbstring::ord()} / {@see VmMbstring::chr()}
 * php-src: ext/mbstring/mbstring.c — PHP_FUNCTION(mb_ord) / PHP_FUNCTION(mb_chr)
 *
...
{
    /** NestedJIT-safe 256-byte table (#20452) — index via substr; no native chr(). */
    private const BYTES = "";

    /**
     * mb_chr() — codepoint to character, or false when out of range / surrogate.
     *
     * @return string|false
     */
    public static function chrArgv(int $codepoint, string $encoding)
    {
        if ('ASCII' === $encoding || '8BIT' === $encoding) {
            ...
            return self::byteAt($codepoint);
        }
        // UTF-8 (default NestedJIT encodings)
    ...
    {
        if ($cp < 0x80) {
            return self::byteAt($cp);
        }
        if ($cp < 0x800) {
            return self::byteAt(0xC0 | ($cp >> 6)).self::byteAt(0x80 | ($cp & 0x3F));
        }
        if ($cp < 0x10000) {
            return self::byteAt(0xE0 | ($cp >> 12))
                .self::byteAt(0x80 | (($cp >> 6) & 0x3F))
                .self::byteAt(0x80 | ($cp & 0x3F));
        }

        return self::byteAt(0xF0 | ($cp >> 18))
            .self::byteAt(0x80 | (($cp >> 12) & 0x3F))
            .self::byteAt(0x80 | (($cp >> 6) & 0x3F))
            .self::byteAt(0x80 | ($cp & 0x3F));
    }
    ...
    {
        return \substr(self::BYTES, $code & 0xFF, 1);
    }
