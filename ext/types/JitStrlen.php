...
/** LLVM lowering for strlen() string operand (php-src ext/standard/string.c, #5119). */
final class JitStrlen
{
    public static function lowerStringOperand(Context $context, JITVariable $arg): Value
    {
        ...
        if (JITVariable::TYPE_OBJECT === $arg->type) {
            self::emitTypeErrorAndAbort($context, self::compileTimeObjectGivenLabel($context, $arg));
    ...
    public static function emitTypeErrorAndAbort(Context $context, string $given): void
    {
        TypeErrorRaise::registerDeclarations($context);
        TypeErrorRaise::ensureLinked($context);
        TypeErrorRaise::emitRaise($context, self::typeErrorMessage($given));
